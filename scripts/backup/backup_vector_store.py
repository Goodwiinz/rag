...
class VectorStoreBackup:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.client = QdrantClient(
            host=config["host"], port=config["port"], api_key=config.get("api_key")
        )
    ...
    async def _backup_collection(
        self, collection_name: str, backup_path: Path
    ) -> Dict[str, Any]:
        ...
        with gzip.open(points_file, "wt", encoding="utf-8") as f:
            ...
            for point in all_points:
                ...
                if exported_points % 1000 == 0:
                    logger.info(
                        f"Exported {exported_points}/{points_count} points for {collection_name}"
                    )

        logger.info(
            f"Exported {exported_points} points for collection {collection_name}"
        )
    ...
    def cleanup_old_backups(self):
        ...
        for backup_file in self.backup_dir.glob("qdrant_backup_*.tar.gz"):
            try:
                # Extract timestamp from filename (strip the full .tar.gz suffix;
                # Path.stem only removes .gz, leaving an unparseable .tar fragment)
                bare = backup_file.name.removesuffix(".tar.gz")
                timestamp_str = "_".join(bare.split("_")[-2:])  # "YYYYMMDD_HHMMSS"
    ...
    def _upload_to_gcs(self, archive_path: Path):
        ...
        try:
            ...
            logger.info(
                f"Uploaded backup to GCS: gs://{self.config['gcs_bucket']}/vector-backups/{archive_path.name}"
            )
    ...
    def _upload_to_azure(self, archive_path: Path):
        ...
        try:
            ...
            logger.info(
                f"Uploaded backup to Azure: {self.config['azure_container']}/vector-backups/{archive_path.name}"
            )
    ...
    def _send_slack_notification(self, archive_path: Path, success: bool):
        ...
        try:
            ...
            message = (
                f"{status_emoji} Vector store backup {status_text}\n"
    ...
    def _send_email_notification(self, archive_path: Path, success: bool):
        ...
        try:
            ...
            subject = (
                f"Vector Store Backup {'Success' if success else 'Failed'} - Qdrant"
            )
...
def load_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "cloud_storage_enabled": os.getenv("CLOUD_STORAGE_ENABLED", "").lower()
        == "true",
...
async def main():
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Perform a dry run without creating backup",
    )
    parser.add_argument(
        "--cleanup-only", action="store_true", help="Only cleanup old backups"
    )

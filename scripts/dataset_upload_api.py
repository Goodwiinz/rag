...
class DatasetUploader:
    ...
    async def __aenter__(self) -> "DatasetUploader":
    ...
    async def authenticate(
        ...
        Credentials default to the API_USERNAME / API_PASSWORD environment
        variables and fail-fast if neither is provided.
        """
        username = username or os.environ.get("API_USERNAME")
        password = password or os.environ.get("API_PASSWORD")
        if not username or not password:
            logger.error(
                "Authentication credentials missing: set API_USERNAME/API_PASSWORD"
            )
            return False
        try:
    ...
    async def upload_document(
        self, document: Dict[str, Any], metadata: DocumentMetadata
    ) -> UploadResult:
        ...
        for attempt in range(self.config.max_retries):
            try:
                # Prepare payload (serialize datetime -> ISO string for JSON)
                metadata_dict = asdict(metadata)
                if isinstance(metadata_dict.get("created_at"), datetime):
                    metadata_dict["created_at"] = metadata_dict[
                        "created_at"
                    ].isoformat()
                payload = {
                    "title": document.get("title", ""),
                    "content": document.get("content", ""),
                    "metadata": {**metadata_dict, **document.get("metadata", {})},
                }
                ...
                async with self.session.post(
                    ...
                    if response.status == 201:
                        ...
                        logger.warning(
                            f"Upload attempt {attempt + 1} failed: {response.status}"
                        )
    ...
    async def upload_batch(
        self, documents: List[Dict[str, Any]], metadata_base: DocumentMetadata
    ) -> List[UploadResult]:
        ...
        async def upload_with_semaphore(doc_data: tuple) -> UploadResult:
            ...
            metadata = DocumentMetadata(
                **{
                    **asdict(metadata_base),
                    "original_id": f"{metadata_base.original_id}_{idx}",
                }
            )
    ...
    def process_publaynet_documents(self, data_path: Path) -> List[Dict[str, Any]]:
        ...
        for item in data.get("annotations", []):
            ...
            content_parts.append(
                f"Dimensions: {item.get('width', 0)}x{item.get('height', 0)}"
            )
    ...
    async def upload_dataset(
        ...
        try:
            ...
            for i in range(0, len(documents), self.config.batch_size):
                ...
                logger.info(
                    f"Uploading batch {i//self.config.batch_size + 1}/{(len(documents) + self.config.batch_size - 1)//self.config.batch_size}"
                )
            ...
            upload_stats = {
                ...
                "avg_time_per_document": (
                    total_time / total_uploads if total_uploads > 0 else 0
                ),
                "errors": errors[:10],  # Limit error display
            }

            logger.info(
                f"Upload completed: {successful_uploads}/{total_uploads} successful ({upload_stats['success_rate']:.1f}%)"
            )
            return upload_stats
    ...
    def generate_upload_report(self, results: Dict[str, Any]) -> str:
        ...
        # Overall statistics
        total_docs = sum(
            r.get("total_documents", 0)
            for r in results.values()
            if "total_documents" in r
        )
        total_successful = sum(
            r.get("successful_uploads", 0)
            for r in results.values()
            if "successful_uploads" in r
        )
        total_failed = sum(
            r.get("failed_uploads", 0)
            for r in results.values()
            if "failed_uploads" in r
        )
        ...
        report.append(
            f"  Overall Success Rate: {(total_successful / total_docs * 100) if total_docs > 0 else 0:.1f}%"
        )
        report.append("")
        ...
        for dataset_type, result in results.items():
            ...
            if "error" in result:
                ...
                report.append(
                    ...
                    else f"  Status: ⚠️  PARTIAL"
                )
                report.append(
                    f"  Documents: {result.get('successful_uploads', 0)}/{result.get('total_documents', 0)}"
                )
                report.append(f"  Success Rate: {result.get('success_rate', 0):.1f}%")
                report.append(f"  Total Time: {result.get('total_time', 0):.2f}s")
                report.append(
                    f"  Avg Time/Doc: {result.get('avg_time_per_document', 0):.3f}s"
                )
...
async def main():
    ...
    config = UploadConfig(batch_size=10, max_retries=3, max_concurrent_uploads=5)
    ...
    async with DatasetUploader(config=config) as uploader:
        ...
        if not await uploader.authenticate():
            print(
                "❌ Authentication failed. Please check if the RAG system is running."
            )
            return

...
## 2024-05-24 - Interactive Tag Removal Accessibility
**Learning:** Custom inline `<button>` elements (like '×' icons) inside composite components (like tags or badges) are frequently missing `type="button"` (risking form submission if wrapped in a form context) and explicitly defined focus states. Because they often inherit minimal styles or live inside interactive areas, their default focus rings can be clipped, invisible, or missing, failing keyboard accessibility standards.
**Action:** Whenever adding or auditing inline icon buttons (e.g., removing a tag or dismissing an item), always explicitly define `type="button"` and include `focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2` alongside a `rounded` utility so the focus indicator is visible, distinct, and doesn't look broken.

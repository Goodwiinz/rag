...
## 2026-08-06 - Keyboard Accessible Custom File Uploads
**Learning:** When using `<label>` elements to create custom file upload buttons (by wrapping hidden inputs), they do not automatically receive keyboard focus or trigger on Enter/Space.
**Action:** Always add `tabIndex={0}`, `focus-visible` utility classes, and an `onKeyDown` handler to trigger the click event to ensure keyboard accessibility.

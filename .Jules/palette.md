...
## 2024-07-21 - Pagination Component Keyboard & Landmark Accessibility
**Learning:** Generic pagination component wrappers (like `div`) lack landmark definitions, making it hard for screen reader users to find them quickly. Additionally, icon-only control buttons often have `title`s but lack `aria-label`s, and interactive pagination buttons lack distinct `focus-visible` styles, preventing effective keyboard navigation.
**Action:** When building or modifying pagination components, always wrap the controls in `<nav aria-label="Pagination">`, apply `aria-label`s to all icon-only buttons, use `aria-current="page"` for the active page, and explicitly apply `focus-visible` CSS utilities.

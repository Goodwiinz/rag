...
## 2026-06-24 - Focus-Visible Utility Usage on Custom Buttons
**Learning:** Raw `<button>` elements used in interactive layouts (like lists, graphs, or search results) often lack visual focus states or explicitly specified `type` attributes, creating keyboard accessibility and potential form submission issues.
**Action:** When adding or modifying custom inline `<button>` elements, always explicitly define `type="button"` and add `focus-visible` utility classes (e.g., `focus-visible:outline-none focus-visible:ring-2`) to ensure keyboard accessibility remains functional and clearly indicated.

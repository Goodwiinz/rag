...
export default function DocumentUploadWithAnalytics() {
  ...
  const { trackUpload, trackProcessingStart, trackProcessingComplete, trackProcessingError } = useDocumentAnalytics();
  ...
  const handleUploadStart = (files: File[]) => {
    ...
    trackClick('upload_zone', { fileCount: files.length, totalSize: files.reduce((sum, f) => sum + f.size, 0) });
  };

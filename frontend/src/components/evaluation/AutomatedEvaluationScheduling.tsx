    ...
    logs: [
      {
        ...
        level: 'info',
        message: 'Starting scheduled execution',
      },
      {
        timestamp: '2025-10-17T08:00:15Z',
        level: 'info',
        message: 'Running test suite: Core RAG Functionality',
      },
      {
        timestamp: '2025-10-17T08:18:25Z',
        level: 'info',
        message: 'Test execution completed',
      },
      {
        timestamp: '2025-10-17T08:18:30Z',
        level: 'info',
    ...
    logs: [
      {
        ...
        level: 'info',
        message: 'Starting metric collection',
      },
      {
        timestamp: '2025-10-17T10:01:30Z',
        level: 'warning',
        message: 'Metric calculation taking longer than expected',
      },
      {
      ...
      {
        timestamp: '2025-10-17T10:05:15Z',
        level: 'error',
...
const AutomatedEvaluationScheduling: React.FC<
  AutomatedEvaluationSchedulingProps
> = ({
  onScheduleCreate,
  onScheduleUpdate,
  onScheduleDelete,
  ...
  const [schedules, setSchedules] =
    useState<EvaluationSchedule[]>(mockSchedules);
  const [executions, setExecutions] =
    useState<ScheduleExecution[]>(mockExecutions);
  const [selectedSchedule, setSelectedSchedule] =
    useState<EvaluationSchedule | null>(null);
  ...
  const handleSaveSchedule = useCallback(
    (scheduleData: Partial<EvaluationSchedule>) => {
      if (editingSchedule) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-foreground">
            Configure and manage automated evaluation schedules
          </p>
        </div>
          ...
          >
            ...
            <DialogContent className="max-w-4xl">
              <DialogHeader>
                ...
                <DialogDescription>
                  Configure automated execution of tests, metrics collection,
          ...
          <CardContent className="pt-6">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <div className="text-sm text-foreground">Active Schedules</div>
              </div>
            </div>
          </CardContent>
        </Card>
              ...
              <div>
                <div className="text-2xl font-bold">
                  {upcomingRuns.length > 0 && upcomingRuns[0]?.nextRun
                    ? new Date(upcomingRuns[0].nextRun).toLocaleDateString()
                    : 'N/A'}
                </div>
                <div className="text-sm text-foreground">Next Run</div>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <div className="text-2xl font-bold">
                  {Math.round(
                    schedules.reduce((sum, s) => sum + s.averageDuration, 0) /
                      schedules.length || 0
                  )}
                  m
                </div>
                <div className="text-sm text-foreground">Avg Duration</div>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <div className="flex items-center space-x-4">
            ...
            <Select value={filterType} onValueChange={setFilterType}>
              ...
              <SelectContent>
                ...
                <SelectItem value="metric_collection">
                  Metric Collection
                </SelectItem>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <div className="text-sm">
                        {schedule.schedule.frequency === 'minutes'
                          ? `Every ${schedule.schedule.interval} minutes`
                          : schedule.schedule.frequency === 'hourly'
                            ? 'Every hour'
                            : schedule.schedule.frequency === 'daily'
                              ? `Daily at ${schedule.schedule.time}`
                              : schedule.schedule.frequency === 'weekly'
                                ? `Weekly on ${schedule.schedule.dayOfWeek !== undefined && schedule.schedule.dayOfWeek >= 0 && schedule.schedule.dayOfWeek <= 6 ? ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][schedule.schedule.dayOfWeek] : 'Invalid'} at ${schedule.schedule.time}`
                                : schedule.schedule.frequency === 'monthly'
                                  ? `Monthly on day ${schedule.schedule.dayOfMonth}`
                                  : 'Custom cron'}
                      </div>
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {schedule.target.type.replace('_', ' ')}
                      </div>
                    </div>
                    <div>
                      <div className="text-sm font-medium text-foreground">
                      ...
                      <div className="text-sm">
                        {schedule.runCount} runs • {schedule.successCount}{' '}
                        success • {schedule.failureCount} failures
                      </div>
                      <div className="text-xs text-muted-foreground">
                        Avg: {schedule.averageDuration.toFixed(1)}m • Success
                        rate:{' '}
                        {schedule.runCount > 0
                          ? (
                    ...
                    <div className="mt-4 p-4 border rounded-lg space-y-4">
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            Timeout: {schedule.execution.timeout} minutes
                          </div>
                          <div>
                            Max retries:{' '}
                            {schedule.execution.retryPolicy.maxRetries}
                          </div>
                          <div>
                            Parallel:{' '}
                            {schedule.execution.parallelExecution
                              ? 'Yes'
                              : 'No'}
                          </div>
                          <div>
                            Max concurrency: {schedule.execution.maxConcurrency}
                          </div>
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          {schedule.notifications.onTimeout && (
        ...
        <TabsContent value="executions" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                History of automated and manual executions
              </CardDescription>
            </CardHeader>
            <CardContent>
                        ...
                        <div>
                          ...
                          <div className="text-sm text-foreground">
                            ...
                            {execution.endTime && (
                              <>
                                {' • '}Duration:{' '}
                                {execution.duration?.toFixed(1)}m
                              </>
                            )}
                        ...
                        <div className="text-sm">
                          ...
                          {execution.result.metrics && (
                            <div className="mt-2 grid grid-cols-2 gap-4 text-xs">
                              {Object.entries(execution.result.metrics).map(
                                ([key, value]) => (
                                  <div key={key}>
        ...
        <TabsContent value="upcoming" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Scheduled executions for the next 24 hours
              </CardDescription>
            </CardHeader>
            <CardContent>
              {upcomingRuns.length > 0 ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">
                    Activate schedules to see upcoming executions here
                  </p>
                </div>
              )}
            </CardContent>
    ...
    const scheduleData: Partial<EvaluationSchedule> = {
      ...
      notifications: {
        ...
        channels: formData.channels
          ...
          .map((c) => c.trim())
          .filter((c) =>
            ['email', 'slack', 'webhook', 'in_app'].includes(c)
          ) as ('email' | 'slack' | 'webhook' | 'in_app')[],
      },
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={(value) =>
              setFormData((prev) => ({
                ...prev,
                type: value as EvaluationSchedule['type'],
              }))
            }
          >
            <SelectTrigger>
            ...
            <Select
              ...
              onValueChange={(value) =>
                setFormData((prev) => ({
                  ...prev,
                  frequency:
                    value as EvaluationSchedule['schedule']['frequency'],
                }))
              }
            >
              <SelectTrigger>
        ...
        <div className="flex items-center space-x-2 mt-4">
          <Switch
            ...
            onCheckedChange={(checked) =>
              setFormData((prev) => ({ ...prev, parallelExecution: checked }))
            }
          />
        ...
        <div className="space-y-3">
          <div className="flex items-center space-x-2">
            <Switch
              ...
              onCheckedChange={(checked) =>
                setFormData((prev) => ({ ...prev, notifyOnSuccess: checked }))
              }
            />
          ...
          <div className="flex items-center space-x-2">
            <Switch
              ...
              onCheckedChange={(checked) =>
                setFormData((prev) => ({ ...prev, notifyOnFailure: checked }))
              }
            />
          ...
          <div className="flex items-center space-x-2">
            <Switch
              ...
              onCheckedChange={(checked) =>
                setFormData((prev) => ({ ...prev, notifyOnTimeout: checked }))
              }
            />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, channels: e.target.value }))
              }
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={(value) =>
              setFormData((prev) => ({
                ...prev,
                priority: value as EvaluationSchedule['priority'],
              }))
            }
          >
            <SelectTrigger>

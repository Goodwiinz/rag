      ...
      <Card className="border-(--nous-border-1) bg-(--nous-bg-2)">
        ...
        <CardContent className="p-4">
          <Tabs defaultValue="json" className="w-full">
            <TabsList className="grid w-full grid-cols-2 bg-(--nous-bg-1)">
            ...
            <TabsContent value="json" className="space-y-4">
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  className="font-mono text-xs bg-(--nous-bg-1) border-(--nous-border-1) text-(--nous-fg-1) min-h-[200px]"
                />
              </div>
              <Button
                onClick={handleBulkCreateFromJSON}
                disabled={loading || !jsonInput.trim()}
                className="w-full font-mono text-xs font-bold bg-(--nous-sol) text-(--nous-bg-1)"
              >
                {loading ? (
                  <>
          ...
          <CardContent className="p-4 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-3 rounded-lg border border-(--nous-border-1) bg-(--nous-bg-1)">
                ...
                <p className="text-2xl font-mono font-bold text-(--nous-sol) mt-2">
              ...
              <div className="p-3 rounded-lg border border-(--nous-border-1) bg-(--nous-bg-1)">
                <div className="flex items-center gap-2">
                  <CheckCircle className="w-4 h-4 text-(--nous-helios)" />
                  <span className="text-xs font-mono text-(--nous-fg-3)">
                    CREATED RELATIONSHIPS
                  </span>
                </div>
                <p className="text-2xl font-mono font-bold text-(--nous-helios) mt-2">
            ...
            {/* Processing Time */}
            <div className="text-xs font-mono text-(--nous-fg-3)">
            ...
            {result.errors.length > 0 && (
              <div className="space-y-2">
                <div className="text-xs font-mono font-bold text-(--nous-fg-1) uppercase">
                  Errors
                </div>
                <div className="space-y-1 max-h-40 overflow-y-auto">
                  {result.errors.map((error, index) => (
                    <div
                      key={index}
                      className="text-xs font-mono text-(--nous-helios) p-2 rounded bg-(--nous-bg-1) border border-(--nous-border-1)"
                    >
                      {error.error}
                    </div>
                  ))}

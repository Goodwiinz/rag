    ...
    <form onSubmit={handleSubmit} className="space-y-6 text-[var(--nous-fg-1)]">
      ...
      {formData.source_entity_id && (
        <div className="space-y-2">
          <Label className="text-xs font-mono text-[var(--nous-fg-3)] uppercase tracking-widest">Source_Entity_ID</Label>
          <Input 
            value={formData.source_entity_id} 
            disabled 
            className="bg-[var(--nous-bg-3)] border-[var(--nous-border-1)] font-mono text-sm text-[var(--nous-fg-3)] cursor-not-allowed opacity-70" 
          />
        </div>
      )}
      ...
      {/* Target Entity */}
      <div className="space-y-2">
        <Label htmlFor="target" className="text-xs font-mono text-[var(--nous-fg-3)] uppercase tracking-widest">Target_Entity *</Label>
        <div className="space-y-2">
          {selectedTarget ? (
            <div className="flex items-center justify-between p-3 bg-[var(--nous-bg-3)] border border-[var(--nous-border-1)] rounded-md">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded bg-[var(--nous-bg-1)] border border-[var(--nous-border-1)] flex items-center justify-center">
                  <Terminal className="w-4 h-4 text-[var(--nous-helios)]" />
                </div>
                <div>
                  <p className="font-mono text-sm font-bold text-[var(--nous-fg-1)]">{selectedTarget.name}</p>
                  <p className="font-mono text-[10px] text-[var(--nous-fg-3)] uppercase">{selectedTarget.type}</p>
                </div>
              </div>
              <Button
                type="button"
                ...
                className="h-8 w-8 text-[var(--nous-fg-3)] hover:text-red-400 hover:bg-red-400/10"
              >
                <X className="h-4 w-4" />
              </Button>
            </div>
            ...
            <div className="relative">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[var(--nous-fg-3)]" />
                <Input
                  id="target"
                  placeholder="SEARCH_TARGET_ENTITY..."
                  value={targetSearchQuery}
                  onChange={(e) => setTargetSearchQuery(e.target.value)}
                  className="pl-10 bg-[var(--nous-bg-1)] border-[var(--nous-border-1)] font-mono text-sm text-[var(--nous-fg-1)] placeholder:text-[var(--nous-fg-3)] focus:border-[var(--nous-sol)]"
                />
              </div>
              {targetSearchResults.length > 0 && (
                <div className="absolute z-10 w-full mt-1 bg-[var(--nous-bg-2)] border border-[var(--nous-border-1)] rounded-lg shadow-xl max-h-60 overflow-auto nous-scrollbar">
                  {targetSearchResults.map((entity) => (
                    <button
                      key={entity.id}
                      type="button"
                      className="w-full text-left px-4 py-2 hover:bg-[var(--nous-bg-3)] flex items-center justify-between border-b border-[var(--nous-border-1)] last:border-0 group"
                      onClick={() => selectTargetEntity(entity)}
                    >
                      <div>
                        <p className="font-mono text-sm font-bold text-[var(--nous-fg-1)] group-hover:text-[var(--nous-sol)] transition-colors">{entity.name}</p>
                        <p className="font-mono text-[10px] text-[var(--nous-fg-3)] uppercase">{entity.type}</p>
                      </div>
                      <Link className="h-4 w-4 text-[var(--nous-fg-3)] group-hover:text-[var(--nous-sol)] transition-colors" />
                    </button>
                  ))}
                </div>
              )}
      ...
      {/* Relationship Type */}
      <div className="space-y-2">
        <Label htmlFor="type" className="text-xs font-mono text-[var(--nous-fg-3)] uppercase tracking-widest">Link_Type *</Label>
        <Select
        ...
        >
          ...
          <SelectContent className="bg-[var(--nous-bg-2)] border-[var(--nous-border-1)] max-h-60">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <Label className="text-xs font-mono text-[var(--nous-fg-3)] uppercase tracking-widest">Link_Strength</Label>
            <span className="text-xs font-mono font-bold text-[var(--nous-helios)]">{(formData.strength * 100).toFixed(0)}%</span>
          </div>
          <div className="py-2">
            <Slider
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <Label className="text-xs font-mono text-[var(--nous-fg-3)] uppercase tracking-widest">Confidence</Label>
            <span className="text-xs font-mono font-bold text-[var(--nous-sol)]">{(formData.confidence_score * 100).toFixed(0)}%</span>
          </div>
          <div className="py-2">
            <Slider
      ...
      {/* Context */}
      <div className="space-y-2">
        <Label htmlFor="context" className="text-xs font-mono text-[var(--nous-fg-3)] uppercase tracking-widest">Context_Buffer</Label>
        <Textarea
          id="context"
        ...
        <CardContent className="space-y-3 p-4">
          ...
          <div className="space-y-2">
            {formData.evidence.map((evidence, index) => (
              <div key={index} className="flex items-center justify-between p-2 pl-3 bg-[var(--nous-bg-3)] border border-[var(--nous-border-1)] rounded-md">
                <p className="text-xs font-mono text-[var(--nous-fg-1)] flex-1">{evidence}</p>
                <Button
                  type="button"
                  variant="ghost"
                  size="sm"
                  onClick={() => removeEvidence(index)}
                  className="h-6 w-6 text-[var(--nous-fg-3)] hover:text-red-400 hover:bg-red-400/10 p-0"
                >
                  <X className="h-3 w-3" />
                </Button>
              </div>
            ...
            {formData.evidence.length === 0 && (
              <p className="text-[10px] font-mono text-[var(--nous-fg-3)] text-center py-4 opacity-50">
                NO_EVIDENCE_LOGGED
              </p>
            )}
          </div>

      ...
      <Card className="border-(--nous-border-1) bg-(--nous-bg-2)">
        ...
        <CardContent className="p-4">
          <div className="text-center py-12">
            <Lock className="w-12 h-12 mx-auto mb-4 text-(--nous-fg-3)" />
            <p className="text-sm font-mono text-(--nous-fg-3)">
    ...
    <div className="space-y-4">
      <Card className="border-(--nous-border-1) bg-(--nous-bg-2)">
        ...
        <CardContent className="p-4 space-y-4">
          ...
          {activeJobId && (
            <div className="text-xs font-mono text-(--nous-fg-3) border border-(--nous-border-1) rounded-md p-2">
          ...
          <CardContent className="p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs font-mono text-red-400/80 mt-1 wrap-break-word">
                  {error.message}
                </p>
              </div>
      ...
      {filteredDuplicates.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-mono font-bold text-(--nous-fg-1)">
          ...
          {filteredDuplicates.map(({ group, index }) => (
            <Card key={index} className="border-(--nous-border-1) bg-(--nous-bg-2)">
              <CardContent className="p-4">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="w-4 h-4"
                    />
                    <AlertTriangle className="w-4 h-4 text-(--nous-helios)" />
                    <span className="text-xs font-mono text-(--nous-fg-3)">
                      {group.entities.length} similar entities
                    </span>
                    <Badge variant="outline" className="font-mono text-[10px] border-(--nous-border-1)">
                  ...
                  <Button
                    ...
                    size="sm"
                    disabled={!!activeJobId}
                    className="font-mono text-[10px] bg-(--nous-sol) text-(--nous-bg-1)"
                  >
                    ...
                    <div
                      ...
                      className={`p-2 rounded-md border ${
                        entity.id === group.suggested_primary
                          ...
                          : 'border-(--nous-border-1) bg-(--nous-bg-1)'
                      }`}
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-mono font-bold text-(--nous-fg-1)">
                              {entity.name}
                            </span>
                            {entity.id === group.suggested_primary && (
                              <Badge
                                variant="outline"
                                className="font-mono text-[10px] bg-(--nous-sol) text-(--nous-bg-1) border-(--nous-sol)"
                              >
                          ...
                          <div className="flex items-center gap-3 mt-1 text-xs font-mono text-(--nous-fg-3)">
      ...
      {!loading && duplicates.length === 0 && (
        <Card className="border-(--nous-border-1) bg-(--nous-bg-2)">
          <CardContent className="p-8 text-center">
            <GitMerge className="w-12 h-12 mx-auto mb-4 text-(--nous-fg-3)" />
            <p className="text-sm font-mono text-(--nous-fg-1)">No duplicates found</p>
            <p className="text-xs font-mono text-(--nous-fg-3) mt-2">

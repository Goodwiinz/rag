              ...
              <div className="flex-1">
                ...
                {notification.documentId && (
                  <div className="flex items-center space-x-1 mt-2">
                    <DocumentTextIcon className="w-3 h-3 text-muted-foreground" />
...
export const NotificationCenter: React.FC<NotificationCenterProps> = ({
  ...
  const handleNotificationAction = useCallback(
    (notificationId: string, action: NotificationItem['actions'][0]) => {
      action.action();
  ...
  const toastNotifications = notifications
    .slice(0, maxVisible)
    .filter((n) => !activeToasts.has(n.id));
              ...
              <AnimatePresence>
                {isHistoryOpen && (
                  <>
                    ...
                    />
                    <div
                      className={cn('absolute mt-2', positionClasses[position])}
                    >
                      <NotificationHistory
...
export const useNotificationCenter = () => {
  const { notifications, clearNotifications } = useRealtimeProcessing();
  ...
  const addNotification = useCallback(
    (
      ...
      message: string,
      options: Partial<
        Omit<NotificationItem, 'id' | 'type' | 'title' | 'message'>
      > = {}
    ) => {

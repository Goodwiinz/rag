...
const ProcessingStage: React.FC<ProcessingStageProps> = ({
  ...
  return (
    <div className={cn("border rounded-lg p-3 transition-colors", getStatusColor())}>
...
export const QueryProcessor: React.FC<QueryProcessorProps> = ({
  ...
  const defaultConfig: EnhancedSearchRequest['processing_config'] = {
    ...
    hybrid_config: {
      vector_search: { enabled: true, weight: 0.5, similarity_threshold: 0.7, max_results: 50 },
      graph_search: { enabled: true, weight: 0.3, max_depth: 3, relationship_types: ['related_to', 'mentions'] },
      keyword_search: { enabled: true, weight: 0.2, fuzzy_threshold: 0.8, max_results: 100 },
      fusion_strategy: 'rrf',
  ...
  const updateState = useCallback((updates: Partial<QueryProcessingState>) => {
  ...
  const simulateStage = useCallback(async (
    stage: string,
    duration: number,
    onSuccess: () => void,
  ...
  const runQueryRewriting = useCallback(async (intent: QueryIntent): Promise<QueryRewrite> => {
    return new Promise((resolve, reject) => {
      simulateStage(
        'query_rewriting',
        300 + Math.random() * 400,
  ...
  const runHybridSearch = useCallback(async (): Promise<SearchStageResult[]> => {
    return new Promise((resolve, reject) => {
      simulateStage(
        'search_execution',
          ...
          const mockSearchResults: SearchStageResult[] = [
            {
              ...
              metadata: { query_length: query.length, timestamp: new Date().toISOString() }
            },
            {
              stage: 'graph',
  ...
  const runResultAggregation = useCallback(async (searchResults: SearchStageResult[]): Promise<ResultAggregation> => {
    return new Promise((resolve, reject) => {
      simulateStage(
        'result_aggregation',
        400 + Math.random() * 300,
  ...
  const getStageStatus = (stage: string): 'pending' | 'in_progress' | 'completed' | 'failed' => {
    if (processingState.error && processingState.current_stage === stage) return 'failed';
    if (processingState.current_stage === stage) return 'in_progress';

    const stageOrder = ['intent_detection', 'query_rewriting', 'search_execution', 'result_aggregation', 'completed'];
  ...
  const getProcessingIcon = () => {
    ...
    if (isRunning) return <div className="h-5 w-5 animate-spin rounded-full border-2 border-blue-600 border-t-transparent" />;
    if (processingState.current_stage === 'completed') return <CheckCircleIcon className="h-5 w-5 text-green-600" />;
    if (processingState.current_stage === 'failed') return <XCircleIcon className="h-5 w-5 text-red-600" />;
  ...
  return (
    <div className={cn("space-y-6", className)}>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              {getProcessingIcon()}
              <div>
                <CardTitle className="text-lg">Query Processing Pipeline</CardTitle>
                <p className="text-sm text-gray-600">
                  {isRunning && !isPaused && `Executing ${processingState.current_stage.replace('_', ' ')}...`}
                  {isPaused && 'Processing paused'}
                  {processingState.current_stage === 'completed' && 'Processing completed successfully'}
                  {processingState.current_stage === 'failed' && 'Processing failed'}
                  {!isRunning && processingState.current_stage === 'intent_detection' && 'Ready to start processing'}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2">
              {/* Control Buttons */}
              {!isRunning && processingState.current_stage !== 'completed' && processingState.current_stage !== 'failed' && (
              ...
              {(processingState.current_stage === 'completed' || processingState.current_stage === 'failed') && (
        ...
        <div className="space-y-4">
          ...
          {currentConfig?.enable_intent_detection && (
            <ProcessingStage
              ...
              progress={processingState.current_stage === 'intent_detection' ? processingState.progress : 100}
          ...
          {currentConfig?.enable_query_rewriting && (
            <ProcessingStage
              ...
              progress={processingState.current_stage === 'query_rewriting' ? processingState.progress : 100}
          ...
          {currentConfig?.enable_hybrid_search && (
            <ProcessingStage
              ...
              progress={processingState.current_stage === 'search_execution' ? processingState.progress : 100}
          ...
          <ProcessingStage
            ...
            progress={processingState.current_stage === 'result_aggregation' ? processingState.progress : 100}
      ...
      {/* Component Details */}
      {(processingState.intent || processingState.rewrite || processingState.search_results.length > 0) && (
        <div className="space-y-6">
          {processingState.intent && (
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-3">Intent Detection</h3>
              <QueryIntentDetector
                query={query}
                onIntentDetected={() => {}}
                showDetails={false}
          ...
          {processingState.rewrite && (
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-3">Query Rewriting</h3>
              <QueryRewriter
                query={query}
          ...
          {processingState.search_results.length > 0 && (
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-3">Hybrid Search</h3>
          ...
          {processingState.aggregation && (
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-3">Result Aggregation</h3>
              <ResultAggregator

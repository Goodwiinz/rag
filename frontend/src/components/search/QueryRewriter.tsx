...
const RewriteDetail: React.FC<RewriteDetailProps> = ({
  ...
  const getStrategyColor = (strategy: string) => {
    switch (strategy) {
      case 'expansion':
        return 'bg-(--nous-sol)/15 text-(--nous-fg-accent-safe)';
      case 'simplification':
  ...
  const getStrategyIcon = (strategy: string) => {
    ...
    switch (strategy) {
      case 'expansion':
        return (
          <SparklesIcon
            className={cn(iconClass, 'text-(--nous-fg-accent-safe)')}
          />
        );
      case 'simplification':
        return (
          <LightBulbIcon
            className={cn(iconClass, 'text-(--nous-terra)')}
          />
        );
      case 'temporal_adaptation':
        return (
          <ClockIcon className={cn(iconClass, 'text-(--nous-fg-accent-safe)')} />
        );
      case 'domain_enhancement':
        return (
          <AcademicCapIcon
            className={cn(iconClass, 'text-(--nous-corona)')}
          />
        );
      default:
        return (
  ...
  const getFilterIcon = (filterType: string) => {
    ...
    switch (filterType) {
      ...
      case 'date_range':
        return (
          <ClockIcon className={cn(iconClass, 'text-(--nous-terra)')} />
        );
      case 'file_type':
        return (
          <FunnelIcon
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div>
            <h3 className="text-lg font-semibold text-foreground mb-3">
              Rewritten Queries ({rewrite.rewritten_queries.length})
            </h3>
            <div className="space-y-3">
              {rewrite.rewritten_queries.map((rewrittenQuery, index) => (
                ...
                >
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <Badge
                          className={cn(
                            'text-xs',
                            getStrategyColor(rewrittenQuery.strategy)
                          )}
                        >
                        ...
                        <span className="ml-2 text-sm text-foreground">
                          Confidence:{' '}
                          {Math.round(rewrittenQuery.confidence * 100)}%
                        </span>
                      </div>
                    </div>
                  ...
                  {expandedQuery === index && (
                    <div className="mt-3 p-3 bg-(--nous-sol)/10 rounded">
                      <h4 className="text-sm font-medium text-(--nous-fg-accent-safe) mb-2">
                        Reasoning:
                      </h4>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {rewrite.expanded_terms.map((term, index) => (
                    <div
                      key={index}
                      className="flex items-center space-x-1 bg-(--nous-terra)/10 rounded-full px-3 py-1"
                    >
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {rewrite.removed_terms.map((term, index) => (
                    <div
                      key={index}
                      className="flex items-center space-x-1 bg-(--nous-mars)/10 rounded-full px-3 py-1"
                    >
                      <span className="text-sm font-medium text-(--nous-mars)">
                        {term}
                      </span>
                    </div>
                  ))}
          ...
          {rewrite.suggested_filters.length > 0 && (
            <div>
              <h3 className="text-lg font-semibold text-foreground mb-3">
                Suggested Filters ({rewrite.suggested_filters.length})
              </h3>
              <div className="space-y-2">
                {rewrite.suggested_filters.map((filter, index) => (
                  <div
                    key={index}
                    className="flex items-center justify-between p-3 bg-(--nous-corona)/10 rounded-lg"
                  >
                    <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="p-3 bg-(--nous-bg-2) rounded">
...
export const QueryRewriter: React.FC<QueryRewriterProps> = ({
  ...
  const generateRewrite = useCallback(
    async (
      ...
      if (
        queryIntent?.domain_specificity === 'technical' ||
        ...
        if (expansions.length > 0) {
          ...
          rewrittenQueries.push({
            ...
            strategy: 'expansion',
            confidence: 0.85,
            reasoning: `Added technical terms related to ${Object.keys(
              technicalTerms
            )
              .filter((term) => lowerQuery.includes(term))
              .join(', ')} to improve search precision`,
          });
        }
      }
      ...
      if (queryIntent?.complexity === 'complex' || words.length > 12) {
        ...
        const removedWords = words.filter(
          (word) => stopWords.includes(word.toLowerCase()) || word.length <= 2
        );
        ...
        rewrittenQueries.push({
          ...
          confidence: 0.75,
          reasoning:
            'Simplified complex query to focus on key concepts and improve search relevance',
        });
      }

      // Strategy 3: Temporal Adaptation
      if (
        queryIntent?.temporal_aspect === 'current' ||
        ...
        rewrittenQueries.push({
          ...
          confidence: 0.9,
          reasoning:
            'Added temporal terms to focus on recent and current information',
        });
      } else if (
        queryIntent?.temporal_aspect === 'historical' ||
        lowerQuery.includes('history') ||
        lowerQuery.includes('past')
      ) {
        const temporalQuery =
          queryText + ' historical background evolution development timeline';
        rewrittenQueries.push({
          query: temporalQuery,
          strategy: 'temporal_adaptation',
          confidence: 0.85,
          reasoning:
            'Added historical context terms to capture background and development information',
        });
      }

      // Strategy 4: Domain Enhancement
      if (
        queryIntent?.modality_preference &&
        queryIntent.modality_preference.length > 1
      ) {
        const modalityTerms = queryIntent.modality_preference
          .map((modality) => {
            switch (modality) {
              case 'image':
      ...
      if (
        queryIntent?.modality_preference &&
        queryIntent.modality_preference.length > 0
      ) {
        suggestedFilters.push({
          type: 'modality',
      ...
      if (
        queryIntent?.temporal_aspect &&
        queryIntent?.temporal_aspect !== 'timeless'
      ) {
      ...
      if (rewrittenQueries.length === 0) {
        const enhancedQuery =
          queryText + ' detailed comprehensive information overview';
        rewrittenQueries.push({
          query: enhancedQuery,
          strategy: 'expansion',
          confidence: 0.7,
          reasoning:
            'Added general terms to broaden search scope and capture comprehensive information',
        });
      }
      ...
      const timer = setTimeout(async () => {
        try {
          ...
          if (autoApply && queryRewrite.rewritten_queries.length > 0) {
            const bestRewrite = queryRewrite.rewritten_queries.reduce(
              (best, current) =>
  ...
  return (
    <div className={cn('space-y-3', className)}>
      ...
      <div className="flex items-center justify-between p-3 bg-(--nous-bg-2) rounded-lg">
        <div className="flex items-center space-x-3">
          {isRewriting ? (
            <div className="h-4 w-4 animate-spin rounded-full border-2 border-(--nous-terra) border-t-transparent" />
          ) : (
            <ArrowPathIcon className="h-4 w-4 text-(--nous-terra)" />
          )}
          <div>
            <div className="flex items-center space-x-2">
              ...
              {rewrite ? (
                <span className="text-sm font-semibold text-(--nous-terra)">
      ...
      {appliedRewrite && (
        <div className="flex items-center space-x-2 p-3 bg-(--nous-terra)/10 rounded-lg">
          ...
          <span className="text-sm text-(--nous-terra) italic">
            "{appliedRewrite}"
          </span>
          <Button
            variant="ghost"
      ...
      {rewrite &&
        !loading &&
        rewrite.rewritten_queries.length > 0 &&
        !autoApply && (
          <div className="flex flex-wrap gap-2">
            {rewrite.rewritten_queries
              .slice(0, 2)
              .map((rewrittenQuery, index) => (
                <Button
                  key={index}
                  variant="outline"
                  size="sm"

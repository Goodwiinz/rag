...
export function AppSidebar() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setMetrics(prev => ({
        ...
        queries: (parseInt(prev.queries.replace(',', '')) + Math.floor(Math.random() * 5)).toLocaleString(),
      }));
    }, 5000);
  ...
  const NavItem = ({ item }: { item: { title: string; url: string; icon: React.ComponentType<{ className?: string }> } }) => {
    ...
    return (
      <SidebarMenuItem>
        ...
        >
          <Link href={item.url} className="flex items-center gap-3 w-full px-5 group-data-[collapsible=icon]:px-0 group-data-[collapsible=icon]:justify-center">
            <Icon className={cn(
              "w-4 h-4 flex-shrink-0 transition-colors duration-200",
              active ? "text-[#00ff9f]" : "text-[#52525b] group-hover/nav:text-[#00ff9f]"
            )} />
  ...
  return (
    <Sidebar
      collapsible="icon"
      className="border-r border-[#1A1A1A] !bg-[#0A0A0A] w-60 z-50"
    >
      {/* Header with Logo */}
          ...
          <div className="bg-[#0A0A0A] border-y border-[#1A1A1A] px-5 py-3">
            ...
            <div className="space-y-2 mt-2">
              <div className="flex justify-between items-center">
                <span className="font-mono text-[10px] text-[#71717a]">UPTIME</span>
                <span className="font-mono text-[10px] font-medium text-[#00ff9f]">{metrics.uptime}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="font-mono text-[10px] text-[#71717a]">CPU</span>
                <span className="font-mono text-[10px] font-medium text-[#fafafa]">{metrics.cpu}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="font-mono text-[10px] text-[#71717a]">MEMORY</span>
                <span className="font-mono text-[10px] font-medium text-[#ffb700]">{metrics.memory}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="font-mono text-[10px] text-[#71717a]">QUERIES</span>
                <span className="font-mono text-[10px] font-medium text-[#00d4ff]">{metrics.queries}</span>
              </div>
            </div>
          </div>
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            {isAuthenticated ? (
                ...
                >
                  <DropdownMenuLabel className="text-[#71717a] text-xs font-normal">
                    <div className="flex flex-col gap-1">
                      <span className="text-[#fafafa] font-medium">{displayName}</span>
                      <span className="text-[#52525b] text-[10px]">{user?.email || 'Not signed in'}</span>
                    </div>
                  </DropdownMenuLabel>
                  <DropdownMenuSeparator className="bg-[#1A1A1A]" />
                  <DropdownMenuItem asChild className="hover:bg-[#1A1A1A] focus:bg-[#1A1A1A] cursor-pointer">
                    <Link href="/settings" className="flex items-center gap-2 text-[#a1a1aa] hover:text-[#fafafa]">
                      <Settings className="w-4 h-4" />
                      Settings
                    </Link>
                  </DropdownMenuItem>
                  <DropdownMenuItem asChild className="hover:bg-[#1A1A1A] focus:bg-[#1A1A1A] cursor-pointer">
                    <Link href="/notifications" className="flex items-center gap-2 text-[#a1a1aa] hover:text-[#fafafa]">
                      <Bell className="w-4 h-4" />
                      Notifications
                    </Link>
                  </DropdownMenuItem>
                  <DropdownMenuItem asChild className="hover:bg-[#1A1A1A] focus:bg-[#1A1A1A] cursor-pointer">
                    <Link href="/help" className="flex items-center gap-2 text-[#a1a1aa] hover:text-[#fafafa]">
              ...
              >
                <Link href="/login" className="flex items-center gap-3 group-data-[collapsible=icon]:justify-center">
                  <LogOut className="w-4 h-4 text-[#00FF88]" />
                  <span className="font-mono text-[11px] group-data-[collapsible=icon]:hidden">Sign In</span>
                </Link>
              </SidebarMenuButton>
            )}
          </SidebarMenuItem>

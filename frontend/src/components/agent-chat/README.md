...
| `AgentMessageItem`      | Single message row. User messages render as plain text; assistant messages pass through `AgentMarkdownRenderer`. Batches consecutive identical tool calls into a collapsible `ToolExecutionGroupCard`. Shows copy and retry affordances on hover.                |
...
| `PlanCard`              | Execution plan from the agent's structured response. Derives per-step status (pending / active / done) by cross-referencing live `ToolExecution` records against declared tool names.                                                                            |

...
export function CitationLink({
  ...
  const citationButton = (
    <motion.button
      ...
      aria-label={
        citation
          ? `Citation ${citationNumber}: ${citation.title}${!canNavigate ? ' (external reference)' : ''}`
  ...
  return (
    <HoverCard openDelay={150} closeDelay={100}>
      ...
      >
            ...
            <div className="flex-1 min-w-0">
              <h4
                className={cn(
                  ...
                  canNavigate ? 'text-primary' : 'text-accent'
                )}
              >
                {citation.title}
              </h4>
        ...
        {/* Footer action */}
        <div className="px-3 py-2 bg-background flex items-center justify-end">
          {canNavigate ? (
            <button
              onClick={handleClick}

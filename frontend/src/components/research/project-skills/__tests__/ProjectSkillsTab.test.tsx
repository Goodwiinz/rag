...
function renderCatalog(
  ...
  vi.mocked(projectSkillService.list).mockResolvedValue({
    ...
    capabilities: { can_edit: true, can_admin: true },
    ...overrides,
  } as never);
  vi.mocked(projectSkillService.getDiff).mockResolvedValue({
    from_version: 1,
    to_version: 2,
    diff: '',
  } as never);
...
describe('ProjectSkillsTab', () => {
  ...
  it('shows active skill metadata, pending status, and an accessible empty state when no skills exist', async () => {
    ...
    vi.mocked(projectSkillService.list).mockResolvedValueOnce({
      skills: [],
  ...
  it('lets an editor propose a document without exposing approval controls', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Skill document'), {
      target: {
        value:
  ...
  it('does not let a historical blocker gate a clean proposed version', async () => {
    ...
    await screen.findByText('release-notes');
    fireEvent.click(
      screen.getByRole('button', { name: /review release-notes/i })
    );
    await waitFor(() =>
      expect(projectSkillService.get).toHaveBeenCalledWith(
        'project-1',
        'release-notes'
      )
    );
    expect(
      await screen.findByText('Review pending change')
    ).toBeInTheDocument();
  ...
  it('always exposes an audit note so an admin can reject a clean proposal', async () => {
    vi.mocked(projectSkillService.get).mockResolvedValue({
      ...skill,
      versions: [{ ...skill.active_version, id: 'version-2', version: 2 }],
    } as never);
    ...
    await waitFor(() =>
      expect(projectSkillService.reject).toHaveBeenCalledWith(
        'project-1',
        'request-1',
        { audit_note: 'Please add citations.' }
      )
    );
  });
  ...
  it('keeps review history read-only for viewers', async () => {
    vi.mocked(projectSkillService.get).mockResolvedValue({
      ...skill,
      versions: [
        { ...skill.active_version, id: 'version-2', version: 2 },
        skill.active_version,
      ],
    } as never);
    ...
    expect(
      screen.queryByRole('button', {
        name: /new skill|stage archive|stage restore|stage rollback/i,
      })
    ).not.toBeInTheDocument();
  });

  it('maps passed scans to the success badge and reports failed mutations', async () => {
    vi.mocked(projectSkillService.create).mockRejectedValue(
    ...
    unmount();
    renderCatalog();
    expect(await screen.findByText('Scan: passed')).toHaveClass(
      'text-(--nous-terra)'
    );
  });
  ...
  it('stages rollback from immutable history and explains a stale approval conflict', async () => {
    vi.mocked(projectSkillService.get).mockResolvedValue({
      ...skill,
      versions: [
        { ...skill.active_version, version: 2, id: 'version-2' },
        skill.active_version,
      ],
    } as never);
    vi.mocked(projectSkillService.rollback).mockResolvedValue(pending as never);
    vi.mocked(projectSkillService.approve).mockRejectedValue({
    ...
    expect(await screen.findByText('Version history')).toBeInTheDocument();
    fireEvent.click(
      screen.getByRole('button', { name: /stage rollback to v2/i })
    );
    await waitFor(() =>
      expect(projectSkillService.rollback).toHaveBeenCalledWith(
        'project-1',
        'release-notes',
    ...
    expect(
      await screen.findByText(/superseded by a newer change/i)
    ).toBeInTheDocument();
  });
});

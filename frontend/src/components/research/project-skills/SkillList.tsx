...
import type { ProjectSkill, ProjectSkillChangeRequest } from '@/services/projectSkillService';
    ...
    return (
      <div role="status" className="rounded-lg border border-dashed border-border p-8 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Add instructions your project agents can use after an administrator approves them.
        </p>
      </div>
    );
  }
    ...
    <ul aria-label="Project skills" className="space-y-2">
      {skills.map((skill) => {
        const pending = pendingRequests.some(
          (request) => request.skill_name === skill.name && request.status === 'pending'
        );
        const activeVersion = skill.active_version;
        return (
          <li key={skill.id} className="rounded-lg border border-border bg-card p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <h2 className="font-medium text-foreground">{skill.name}</h2>
                  {activeVersion && <Badge variant="secondary">Active v{activeVersion.version}</Badge>}
                  {pending && <Badge variant="warning">Pending approval</Badge>}
                  {skill.is_archived && <Badge variant="outline">Archived</Badge>}
                </div>
                <p className="mt-1 text-sm text-muted-foreground">

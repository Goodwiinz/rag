...
export function ProjectSkillsTab({ projectId }: { projectId: string }) {
  ...
  if (catalog.isLoading) {
    return <div aria-busy="true" aria-label="Loading project skills" className="h-48 animate-pulse rounded-lg bg-muted" />;
  }
  if (catalog.isError) {
    return <div role="alert" className="rounded-lg border border-border p-4 text-sm text-muted-foreground">Skills are unavailable for this project.</div>;
  }
  ...
  return (
    <section aria-labelledby="project-skills-heading" className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 id="project-skills-heading" className="text-lg font-semibold">Project skills</h2>
          <p className="text-sm text-muted-foreground">Versioned project instructions are activated only after review.</p>
        </div>

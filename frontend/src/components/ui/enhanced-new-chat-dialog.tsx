...
>(({ assistant, isSelected, onSelect, showMetrics = true, variant = 'default' }, ref) => {
          ...
          <div className="flex items-start gap-4">
            ...
            >
              <motion.div
                className={cn(
                  ...
                  isSelected ? "from-orange-400 to-amber-500 opacity-30" : assistant.color.replace('to-', 'to/').replace('from-', 'from/')
                )}
              ...
              <Avatar className="h-14 w-14 ring-2 ring-background/50 relative">
                ...
                <AvatarFallback className={`bg-gradient-to-br ${assistant.color} text-white font-bold text-lg`}>
            ...
            <div className="flex-1 min-w-0 space-y-2">
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-lg text-foreground">{assistant.name}</h3>
                {assistant.isPremium && (
                  <Badge variant="secondary" className="bg-gradient-to-r from-yellow-400 to-amber-500 text-white border-0">
...
export function EnhancedNewChatDialog({
  ...
  useEffect(() => {
    if (open) {
      ...
      // Auto-select first recently used assistant
      if (recentlyUsed.length > 0 && !selectedAssistant) {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(
        assistant =>
          ...
          assistant.capabilities.some(cap => cap.toLowerCase().includes(query)) ||
            ...
            <div className="flex-1 overflow-y-auto pr-2 -mr-2 smooth-scrollbar max-h-[50vh]">
              {isLoading ? (
                ...
                >
                  ...
                  <p className="text-xl text-muted-foreground mb-2">No assistants found</p>

...
const channelColors: Record<NotificationChannel, string> = {
  document: 'bg-(--nous-aurum) text-(--nous-sol-safe)',
  ...
  return (
    <HoverCard openDelay={200} closeDelay={150}>
      ...
      <HoverCardContent
        ...
        align="start"
        className={cn(
          'w-80 p-0 border-(--nous-border-1)',
          'bg-(--nous-bg-2) dark:bg-(--nous-obsidian) dark:border-(--nous-shade)',
          'shadow-(--nous-shadow-lg)',
          'rounded-(--nous-radius-lg)'
        )}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 pt-3 pb-2 border-b border-(--nous-border-1) dark:border-(--nous-shade)">
          <span className="font-(family-name:--nous-font-heading) text-sm font-semibold text-(--nous-fg-1)">
            Notifications
          </span>
          {unreadCount > 0 && (
            <span className="font-(family-name:--nous-font-mono) text-[10px] tracking-widest uppercase text-(--nous-fg-3)">
              {unreadCount} new
            </span>
          )}
        </div>
        ...
        <div className="max-h-64 overflow-y-auto">
          {recent.length === 0 ? (
            <div className="py-8 text-center font-(family-name:--nous-font-body) text-sm text-(--nous-fg-3)">
              All caught up
            </div>
          ) : (
            recent.map((n) => (
              <Link
                ...
                className="flex items-start gap-3 px-4 py-3 hover:bg-(--nous-aurum)/30 dark:hover:bg-(--nous-ember) transition-colors duration-100"
              >
                <span
                  className={cn(
                ...
                <div className="min-w-0 flex-1">
                  <div className="font-(family-name:--nous-font-heading) text-[13px] font-medium text-(--nous-fg-1) truncate">
                    {n.title}
                  </div>
                  <div className="font-(family-name:--nous-font-body) text-[12px] text-(--nous-fg-3) truncate">
                    {n.snippet}
                  </div>
                </div>
                {!n.read && (
                  <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-(--nous-sol)" />
                )}
              </Link>
            ))
          )}
        ...
        <Link
          href="/notifications"
          className="flex items-center justify-center gap-1.5 px-4 py-2.5 border-t border-(--nous-border-1) dark:border-(--nous-shade) font-(family-name:--nous-font-ui) text-xs font-medium text-(--nous-sol-safe) dark:text-(--nous-helios) hover:bg-(--nous-aurum)/20 dark:hover:bg-(--nous-ember) transition-colors"
        >

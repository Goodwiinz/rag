...
const getErrorSeverity = (error: string, retryCount: number = 0): ErrorSeverity => {
  ...
  // File format errors
  if (lowerError.includes('format') || lowerError.includes('invalid') || lowerError.includes('corrupt')) {
    return {
      type: 'error',
      icon: '❌',
      color: 'text-red-600',
  ...
  // Processing errors
  if (lowerError.includes('processing') || lowerError.includes('extraction') || lowerError.includes('ocr')) {
    return {
      type: 'error',
      icon: '⚙️',
      color: 'text-red-600',
  ...
  if (lowerError.includes('system') || lowerError.includes('memory') || lowerError.includes('disk')) {
    return {
      type: 'critical',
      icon: '🚨',
      color: 'text-red-800',
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-center space-x-2 min-w-0 flex-1">
          <span className="text-lg" role="img" aria-label={`${severity.type} error`}>
            {severity.icon}
          </span>
      ...
      <div className="space-y-2">
        <div
          ...
          aria-label={showDetails ? (isExpanded ? 'Show less error details' : 'Show full error details') : 'View error details'}
        >
        ...
        {showDetails && error.length > 100 && (
          <button
            ...
            aria-label={isExpanded ? 'Show less error details' : 'Show more error details'}
          >
        ...
        {severity.type === 'critical' && (
          <div className="text-xs text-red-700 bg-red-50 rounded p-2 border border-red-200">
            <p>
              🆘 <strong>Support Needed:</strong> This error requires attention.
        ...
        {isMaxRetriesReached && (
          <div className="text-xs text-orange-700 bg-orange-50 rounded p-2 border border-orange-200">
            <p>
              ⚠️ <strong>Max Retries:</strong> Maximum retry attempts ({maxRetryAttempts}) reached.
              The document requires manual intervention.
            </p>
          </div>
        )}
      </div>

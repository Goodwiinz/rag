...
const PROGRESS_SIZES: Record<
  ProcessingProgressBarProps['size'],
  ProgressTheme
> = {
  sm: {
    height: 'h-1',
...
export const ProcessingProgressBar: React.FC<ProcessingProgressBarProps> = memo(
  ({
    ...
    const accessibilityLabel =
      ariaLabel || `${label || 'Progress'}: ${formatProgress(clampedProgress)}`;
    ...
    const labelClasses = cn(
      ...
      clampedProgress > 50 ? 'text-white' : 'text-foreground',
      sizeTheme.fontSize
    );
    ...
    if (indeterminate) {
      ...
      return (
        ...
        >
          <div
            ref={progressRef}
            className={cn(indicatorClasses, 'w-1/3 animate-pulse')}
            style={{

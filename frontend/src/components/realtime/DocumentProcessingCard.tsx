...
const PROCESSING_THEMES: Record<DocumentProcessingState['status'], ProcessingCardTheme> = {
  queued: {
...
export const DocumentProcessingCard: React.FC<DocumentProcessingCardProps> = memo(({
  document,
  isSelected = false,
  onSelect,
  compact = false,
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-gray-500">
              {formattedFileSize} • {timeRemaining ? `${timeRemaining} remaining` : document.status}
            </p>
          </div>
          ...
          {/* Status Icon */}
          <div className={cn('text-sm font-medium', theme.info || theme.error || theme.success)}>
            {document.status}
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      ...
      {document.currentStage && (
        <div className="mb-3">
          <div className="flex items-center space-x-2">
            <span className="text-lg">{getStageIcon(document.currentStage.id)}</span>
            <div className="min-w-0 flex-1">
      ...
      <div className="mt-3 pt-3 border-t border-gray-200">
        <div className="flex items-center justify-between text-xs text-gray-500">
          <div className="flex items-center space-x-4">
            <span>
              Uploaded: {new Date(document.metadata.uploadStartedAt).toLocaleTimeString()}
            </span>
          ...
          <div className={cn('font-medium uppercase tracking-wide', theme.info || theme.error || theme.success)}>
            {document.status}
          </div>
        </div>
      </div>

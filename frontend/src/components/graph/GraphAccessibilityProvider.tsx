...
export const GraphAccessibilityProvider: React.FC<GraphAccessibilityProviderProps> = ({
  children,
  nodes = [],
  edges = []
}) => {
  ...
  // Keyboard navigation
  const navigateGraph = useCallback((direction: 'up' | 'down' | 'left' | 'right') => {
  ...
  const getAccessibleNodeDescription = useCallback((node: GraphNode): string => {
    const relationshipCount = edges.filter(e => e.source === node.id || e.target === node.id).length;
    return `${node.label}, ${node.type} entity, confidence: ${(node.confidence * 100).toFixed(1)}%, ${relationshipCount} relationships`;
  }, [edges]);

  const getAccessibleEdgeDescription = useCallback((edge: GraphEdge): string => {
...
export const useGraphAccessibility = () => {
  ...
  if (!context) {
    throw new Error('useGraphAccessibility must be used within a GraphAccessibilityProvider');
  }
  return context;
};
...
export const AccessibleGraphNode: React.FC<{
  ...
  const handleClick = useCallback(() => {
    ...
    onSelect?.();
  }, [node.id, setFocusedNode, announceToScreenReader, getAccessibleNodeDescription, onSelect]);
  ...
  return (
    ...
    >
      <div id={`node-${node.id}-details`} className="sr-only">
        Node with ID {node.id}, type {node.type}, confidence {(node.confidence * 100).toFixed(1)}%
      </div>
    </div>
  );
};
...
export const AccessibleGraphEdge: React.FC<{
  ...
  const handleClick = useCallback(() => {
    ...
    onSelect?.();
  }, [edge.id, setFocusedEdge, announceToScreenReader, getAccessibleEdgeDescription, onSelect]);
  ...
  return (
    <div className={`p-4 bg-gray-50 rounded-lg ${className}`}>
      ...
      <div className="space-y-4">
        ...
        {/* Color Blind Mode */}
        <div>
          <label className="text-sm font-medium mb-2 block">Color Vision Support</label>

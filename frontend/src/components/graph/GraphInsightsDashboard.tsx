...
interface GraphStatistics {
  ...
  entityTypeDistribution: Record<Entity['type'], {
    count: number;
    percentage: number;
  ...
  relationshipTypeDistribution: Record<string, {
    count: number;
    percentage: number;
    avgConfidence: number;
    avgWeight: number;
  ...
  temporalMetrics: {
    entitiesCreated: Array<{ date: string; count: number; type: Entity['type'] }>;
...
export const GraphInsightsDashboard: React.FC<GraphInsightsDashboardProps> = ({
  ...
  const statistics = useMemo((): GraphStatistics => {
    ...
    const graphDensity = maxPossibleEdges > 0 ? totalRelationships / maxPossibleEdges : 0;
    ...
    entities.forEach(entity => {
      ...
      entityTypeStats[entity.type].totalConnections += degrees[entities.indexOf(entity)];
    });
    ...
    relationships.forEach(rel => {
      ...
      relationshipTypeStats[rel.relationship_type].totalConfidence += rel.confidence;
    ...
    Object.keys(relationshipTypeStats).forEach(type => {
      ...
      relationshipTypeDistribution[type] = {
        count,
        percentage: totalRelationships > 0 ? (count / totalRelationships) * 100 : 0,
        avgConfidence: count > 0 ? stats.totalConfidence / count : 0,
    ...
    const entitiesCreated = entities.slice(0, 30).map((entity, index) => {
      const dateStr = new Date(Date.now() - (index * 24 * 60 * 60 * 1000)).toISOString().split('T')[0];
      return {
    ...
    const activityTrend = Math.random() > 0.5 ? 'increasing' : Math.random() > 0.5 ? 'decreasing' : 'stable';
    const peakActivityDate = entitiesCreated.length > 0 ? (entitiesCreated[0]?.date || new Date().toISOString().split('T')[0]) as string : new Date().toISOString().split('T')[0] as string;
    ...
    const highConfidenceRelationships = relationships.filter(r => r.confidence > 0.8).length;
    const questionableEntities = entities.filter(e => e.confidence < 0.5).length;
    const questionableRelationships = relationships.filter(r => r.confidence < 0.5).length;
    ...
    const isolatedNodes = entities.filter(entity =>
      !relationships.some(r => r.source_entity_id === entity.id || r.target_entity_id === entity.id)
    ).length;
  ...
  const insights = useMemo((): Insight[] => {
    ...
    if (growthTrend === 'increasing') {
      insights.push({
        ...
        description: 'Entity and relationship creation is trending upward, indicating healthy knowledge graph expansion.',
        impact: 'low',
        category: 'growth',
        actionable: false,
        suggestions: [
    ...
    const dominantType = Object.entries(statistics.entityTypeDistribution)
          ...
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{formatNumber(statistics.overview.totalEntities)}</p>
          ...
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{formatNumber(statistics.overview.totalRelationships)}</p>
                <p className="text-xs text-gray-500">
          ...
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{formatNumber(statistics.overview.totalDocuments)}</p>
                <p className="text-xs text-gray-500">
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-500">Graph Health</p>
                <p className="text-2xl font-bold text-gray-900">
                  {Math.round(statistics.qualityMetrics.avgEntityConfidence * 100)}%
                </p>
                <p className="text-xs text-gray-500">
                  Avg confidence
                </p>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-sm text-gray-600">High Confidence Entities</div>
                ...
                <div className="text-center p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-sm text-gray-600">Questionable Entities</div>
                  <div className="text-xs text-gray-500">
                    &lt;50% confidence
                  </div>
                </div>
              ...
              <div className="space-y-2">
                ...
                <Progress value={statistics.qualityMetrics.avgEntityConfidence * 100} className="h-2" />
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">
                    {Math.round(statistics.qualityMetrics.avgRelationshipConfidence * 100)}%
                  </span>
                </div>
                <Progress value={statistics.qualityMetrics.avgRelationshipConfidence * 100} className="h-2" />
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="text-lg font-bold">{statistics.connectivityMetrics.avgPathLength.toFixed(1)}</div>
              <div className="text-sm text-gray-600">Avg Path Length</div>
            </div>
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="text-lg font-bold">{statistics.connectivityMetrics.diameter}</div>
              <div className="text-sm text-gray-600">Graph Diameter</div>
            </div>
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="text-lg font-bold">{statistics.connectivityMetrics.clusteringCoefficient.toFixed(3)}</div>
              <div className="text-sm text-gray-600">Clustering Coefficient</div>
            </div>
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="text-lg font-bold">{statistics.connectivityMetrics.components}</div>
            ...
            <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm text-yellow-800">
                  {statistics.connectivityMetrics.isolatedNodes} isolated entities found
                </span>
              </div>
            </div>
          )}
                ...
                .map(([type, stats]) => (
                  <div key={type} className="p-4 border rounded-lg">
                    ...
                    <div className="space-y-1 text-xs text-gray-500">
                      <div>{stats.percentage.toFixed(1)}% of all relationships</div>

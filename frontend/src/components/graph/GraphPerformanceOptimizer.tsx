...
interface GraphPerformanceOptimizerProps {
  ...
  children: (visibleNodes: GraphNode[], visibleEdges: GraphEdge[]) => React.ReactNode;
}

export const GraphPerformanceOptimizer: React.FC<GraphPerformanceOptimizerProps> = ({
  nodes,
  edges,
  config = {},
  onNodeClick,
  ...
  const visibleRangeRef = useRef({ minX: -Infinity, maxX: Infinity, minY: -Infinity, maxY: Infinity });
  ...
  const updateVisibleRange = useCallback((bounds: typeof visibleRangeRef.current) => {
  ...
  const { visibleNodes, hasMoreNodes } = useMemo(() => {
    if (!performanceConfig.enableVirtualization && !performanceConfig.enableLazyLoading) {
      return {
    ...
    const sortedNodes = filteredNodes.sort((a, b) => getNodePriority(b) - getNodePriority(a));
  ...
  const scheduleRender = useCallback(() => {
    ...
    const renderChunk = () => {
      ...
      while (renderQueueRef.current.length > 0 && processed < performanceConfig.chunkSize) {
  ...
  useEffect(() => {
    const monitor = setInterval(() => {
      const now = Date.now();
      if (now - memoryMonitor.current.lastCheck > memoryMonitor.current.checkInterval) {
    ...
    <div className="graph-performance-optimizer">
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="absolute top-2 right-2 bg-black bg-opacity-75 text-white text-xs p-2 rounded">
          <div>Nodes: {renderMetrics.visibleNodes}/{renderMetrics.totalNodes}</div>
          <div>Edges: {renderMetrics.visibleEdges}/{renderMetrics.totalEdges}</div>
          {renderMetrics.memoryUsage !== undefined && (
            <div>Memory: {(renderMetrics.memoryUsage / 1024 / 1024).toFixed(1)}MB</div>
          )}
          {renderMetrics.hasMoreNodes && <div className="text-yellow-400">More nodes available</div>}
        </div>
      )}
    </div>
  );

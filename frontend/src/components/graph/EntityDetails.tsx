...
export const EntityDetails: React.FC<EntityDetailsProps> = ({
  ...
  const getEntityTypeColor = (type: Entity['type']) => {
    ...
    return (
      colors[type] || 'bg-(--nous-bg-3) text-foreground border-border'
    );
  };

  const getConfidenceColor = (confidence: number) => {
    if (confidence >= 0.9) return 'text-(--nous-terra)';
    if (confidence >= 0.7) return 'text-(--nous-corona)';
    ...
    return (
      <div className={cn('p-6', className)}>
        <div className="animate-pulse">
          <div className="h-8 bg-(--nous-bg-3) rounded w-3/4 mb-4"></div>
          <div className="h-4 bg-(--nous-bg-3) rounded w-1/2 mb-2"></div>
          <div className="h-4 bg-(--nous-bg-3) rounded w-1/3 mb-6"></div>
          <div className="space-y-4">
            <div className="h-32 bg-(--nous-bg-3) rounded"></div>
            <div className="h-32 bg-(--nous-bg-3) rounded"></div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className={cn('h-full flex flex-col', className)}>
      {/* Header */}
      <div className="flex items-center justify-between p-6 border-b bg-(--nous-bg-2)">
        ...
        <div className="flex items-center space-x-2">
          <Button onClick={handleBookmark} variant="ghost" size="sm">
            <BookmarkIcon
              className={cn(
                'h-4 w-4',
                isBookmarked && 'text-(--nous-fg-accent-safe) fill-current'
              )}
            />
          </Button>
      ...
      >
        <button
          ...
          className={cn(
            'py-3 px-4 border-b-2 font-medium text-sm transition-colors focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2',
            activeTab === 'overview'
                  ...
                  <div className="space-y-3">
                    {relatedDocuments
                      ...
                      .map((document) => (
                        <div
                          key={document.id}
                          className="flex items-center space-x-3 p-3 bg-(--nous-bg-2) rounded-lg"
                        >

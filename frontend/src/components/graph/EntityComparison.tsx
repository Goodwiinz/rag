...
export const EntityComparison: React.FC<EntityComparisonProps> = ({
  ...
  const [selectedEntities, setSelectedEntities] = useState<Entity[]>(entities.slice(0, maxComparisons));
  ...
  const currentMetric = useMemo(() => {
    return comparisonMetrics.find(m => m.id === selectedMetric) || comparisonMetrics[0];
  }, [selectedMetric, comparisonMetrics]);

  // Calculate relationship analysis between entities
  const analyzeRelationships = useCallback((entity1: Entity, entity2: Entity) => {
    const directRelationships = relationships.filter(r =>
    ...
    const entity1Connections = new Set(
      relationships
        .filter(r => r.source_entity_id === entity1.id || r.target_entity_id === entity1.id)
        .map(r => r.source_entity_id === entity1.id ? r.target_entity_id : r.source_entity_id)
    );

    const entity2Connections = new Set(
      relationships
        .filter(r => r.source_entity_id === entity2.id || r.target_entity_id === entity2.id)
        .map(r => r.source_entity_id === entity2.id ? r.target_entity_id : r.source_entity_id)
    );

    const sharedConnectionIds = Array.from(entity1Connections).filter(id => entity2Connections.has(id));
    const sharedConnections = availableEntities.filter(entity => sharedConnectionIds.includes(entity.id));
  ...
  const compareEntities = useCallback((entity1: Entity, entity2: Entity) => {
  ...
  const metricRanges = useMemo(() => {
    ...
    const values = selectedEntities.map(entity => currentMetric.getValue(entity) as number);
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            ...
            {selectedEntities.length === 0 && (
              <div className="text-center py-8 text-gray-500">
                ...
                <p>Select entities to compare their properties and relationships.</p>
              </div>
            )}
          </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-96 overflow-auto">
              {availableEntities
                .filter(entity => !selectedEntities.some(selected => selected.id === entity.id))
                .map(entity => {
                  const EntityTypeIcon = getEntityTypeIcon(entity.type);
                  return (
                    <div
            ...
            {availableEntities.filter(entity => !selectedEntities.some(selected => selected.id === entity.id)).length === 0 && (
              <div className="text-center py-8 text-gray-500">
            ...
            <CardContent>
              <div className="space-y-4">
                {selectedEntities.map(entity => {
                  ...
                  const formattedValue = currentMetric?.format ? currentMetric.format(value) : value;
                  const EntityTypeIcon = getEntityTypeIcon(entity.type);
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {selectedEntities.slice(0, 2).map((entity1, index) => {
                      ...
                      <div key={entity1.id} className="space-y-4">
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-600">Direct Relationships:</span>
                            <Badge variant="outline">{analysis.directRelationships.length}</Badge>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-600">Shared Documents:</span>
                            <Badge variant="outline">{analysis.sharedDocuments.length}</Badge>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-600">Shared Connections:</span>
                            <Badge variant="outline">{analysis.sharedConnections.length}</Badge>
                          </div>
                        </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-6">
                  {/* Shared Documents */}
                  <div>
                    <h4 className="font-medium text-gray-900 mb-3">Shared Documents</h4>
                    {(() => {
                      const sharedDocIds = selectedEntities.reduce((shared, entity) => {
                        if (shared.length === 0) return entity.document_ids;
                        return shared.filter(docId => entity.document_ids.includes(docId));
                      }, [] as string[]);
                  ...
                  {/* Shared Connections */}
                  <div>
                    <h4 className="font-medium text-gray-900 mb-3">Shared Connections</h4>
                    {(() => {
                      const sharedConnections = selectedEntities.reduce((shared, entity, index) => {
                        if (index === 0) return [];

                        const entityConnections = new Set(
                          relationships
                            .filter(r => r.source_entity_id === entity.id || r.target_entity_id === entity.id)
                            .map(r => r.source_entity_id === entity.id ? r.target_entity_id : r.source_entity_id)
                        );

                        return shared.filter(connectionId => entityConnections.has(connectionId));
                      }, selectedEntities[0] ? relationships
                        .filter(r => r.source_entity_id === selectedEntities[0]!.id || r.target_entity_id === selectedEntities[0]!.id)
                        .map(r => r.source_entity_id === selectedEntities[0]!.id ? r.target_entity_id : r.source_entity_id)
                        : []
                      );

                      const sharedEntityObjects = availableEntities.filter(entity =>
                        sharedConnections.includes(entity.id)
                      );
                      ...
                      return sharedEntityObjects.length > 0 ? (
                        <div className="flex flex-wrap gap-2">
                          {sharedEntityObjects.map(entity => {
                            ...
                            return (
                              <Badge
                                key={entity.id}
                                className={cn("cursor-pointer hover:opacity-80", getEntityTypeColor(entity.type))}
                                onClick={() => onEntityClick?.(entity)}
                              >
                        ...
                        <p className="text-gray-500">No shared connections found.</p>
                      );
                    })()}
                  </div>
                </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-6">
              ...
              <div>
                <h4 className="font-medium text-gray-900 mb-3">Direct Relationships</h4>
                {relationshipAnalysis.directRelationships.length > 0 ? (
                  <div className="space-y-2">
                    {relationshipAnalysis.directRelationships.map(relationship => (
                  ...
                  <p className="text-gray-500">No direct relationships found.</p>
                )}
              </div>

              {/* Shared Documents */}
              <div>
                <h4 className="font-medium text-gray-900 mb-3">Shared Documents ({relationshipAnalysis.sharedDocuments.length})</h4>
                {relationshipAnalysis.sharedDocuments.length > 0 ? (
                  <div className="space-y-2">
                    {relationshipAnalysis.sharedDocuments.slice(0, 3).map(docId => {
                      const doc = documents.find(d => d.id === docId);
                      return doc ? (
                        <div
                          key={docId}

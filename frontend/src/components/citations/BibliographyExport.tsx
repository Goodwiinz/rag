  ...
  return (
    <div className={cn('space-y-6', className)}>
      ...
      <div className="flex items-start justify-between">
        <div>
          <h3 className="text-lg font-semibold text-(--nous-fg-1) font-mono">
            Export Bibliography
          </h3>
          <p className="text-sm text-(--nous-fg-3) font-mono mt-1">
            Select citations and format to export
          </p>
        </div>
        <FileText className="w-5 h-5 text-(--nous-sol)" />
      </div>

      {/* Format Selector */}
      <div className="space-y-2">
        <label className="text-sm font-mono font-medium text-(--nous-fg-1) uppercase tracking-wider">
          Format
        </label>
        <Select value={selectedFormat} onValueChange={(v) => setSelectedFormat(v as BibliographyFormat)}>
          <SelectTrigger className="w-full bg-(--nous-bg-2) border-(--nous-border-1) font-mono">
            <SelectValue />
          </SelectTrigger>
      ...
      {needsReviewCount > 0 && (
        <div className="p-4 rounded-lg border border-(--nous-helios)/30 bg-(--nous-helios)/5">
          <div className="flex items-start gap-3">
            <AlertTriangle className="w-5 h-5 text-(--nous-helios) mt-0.5" />
            <div>
              <p className="text-sm font-mono font-bold text-(--nous-helios)">
      ...
      <div className="space-y-3">
        ...
        <div className="max-h-64 overflow-y-auto space-y-2 border border-(--nous-border-1) rounded-lg p-3 bg-(--nous-bg-2)">
          {citations.length === 0 ? (
            <p className="text-sm text-(--nous-fg-3) font-mono text-center py-4">
            ...
            citations.map((citation) => (
              <div
                key={citation.id}
                className="flex items-start gap-3 p-2 rounded hover:bg-(--nous-bg-3) transition-colors"
              >
                <Checkbox
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-mono text-(--nous-fg-1) line-clamp-2">
                  ...
                  {citation.needsReview && (
                    <span className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded text-[9px] font-mono font-bold uppercase tracking-wider bg-(--nous-helios)/10 text-(--nous-helios) border border-(--nous-helios)/20">
      ...
      {/* Export Button */}
      <div className="flex items-center justify-end gap-3 pt-4 border-t border-(--nous-border-1)">
        ...
        <Button
          ...
          className="font-mono text-xs font-bold bg-(--nous-sol) text-(--nous-bg-1) hover:shadow-[0_0_20px_var(--nous-sol-glow)] disabled:opacity-50"
        >
          {isExporting ? (
            <>

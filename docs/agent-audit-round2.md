...
| R2-M1 | top-20 scan window: papers sliding below rank 20 → miss_count → falsely marked deleted (arxiv_change_tracker.py:292-331,628) | med | pr | sess:audit-r6 | #1536 | 08-22 |
| R2-M2 | revision update overwrites content_text but leaves checksum/S3 PDF at old revision → permanent artifact desync on reuse path (arxiv_change_tracker.py:536-570) | med | pr | sess:audit-r6 | #1536 | 08-22 |
| R2-M3 | cancel_generation cosmetic: task never cancelled, status overwritten back to CITING/FINALIZING, cancelled draft fully persists (draft_generation_service.py:760-776) | med | pr | sess:audit-r6 | #1537 | 08-22 |
...
| R2-M5 | sync Neo4j calls block event loop per entity/relationship during KG integration (arxiv_kg_integration.py:751,776,813) | med | pr | sess:audit-r6 | #1531 | 08-22 |
| R2-M6 | zip(papers, documents) misaligns on partial ingest failure → KG stamped with foreign paper metadata (arxiv_kg_integration.py:105) | med | pr | sess:audit-r6 | #1531 | 08-22 |
| R2-M7 | quota check-then-act race: stale read + unconditional increment, no CHECK constraint → org permanently over quota on concurrent uploads (file_service.py:213-224) | med | pr | sess:audit-r6 | #1536 | 08-22 |
...
| R2-M10 | blanket except rewraps own HTTPExceptions → 403/413 become 400 (files.py:240-241) | med | pr | sess:audit-r6 | #1536 | 08-22 |
| R2-M11 | summary prompt keeps OLDEST ~2000 chars — long threads summarized from opening turns only (thread_summarization_service.py:149-181) | med | pr | sess:audit-r6 | #1537 | 08-22 |
| R2-M12 | cleanup_old_evaluations: reports/comparisons FKs without ondelete → nightly batch IntegrityError forever (evaluation_tasks.py:623-647) | med | pr | sess:audit-r6 | #1537 | 08-22 |
| R2-M13 | sweeper residual: get_job_fresh raise (Redis blip) → row FAILED "Swept as stale", real error destroyed — needs double failure (agent_run_tasks.py:362-390) | med | pr | sess:audit-r6 | #1537 | 08-22 |
| R2-M14 | extract_entities guard covers COMPLETED-redelivery only; mid-run kill → RUNNING passes → duplicate OPENAI entity rows (processing_tasks.py:553-666, latent) | med | pr | sess:audit-r6 | #1536 | 08-22 |
...
| R2-L1 | re.findall match[0] takes first CHARACTER — cited_papers = one-letter strings (arxiv_kg_integration.py:874) | low | refuted (no bare re.findall match[0] at HEAD) | sess:audit-r6 | — | 08-22 |
| R2-L2 | _generation_status dict unbounded, process-local (draft_generation_service.py:42,157) | low | pr | sess:audit-r6 | #1537 | 08-22 |
| R2-L3 | [Doc 0] → idx -1 → last document silently cited (draft_generation_service.py:661-677) | low | pr | sess:audit-r6 | #1537 | 08-22 |
| R2-L4 | tracker state file pod-local + process-local lock; HPA pods diverge (arxiv_change_tracker.py:72) | low | pr | sess:audit-r6 | #1536 | 08-22 |
| R2-L5 | delete_draft of current leaves no current; get_draft(current_only) returns None (draft_generation_service.py:890-904) | low | pr | sess:audit-r6 | #1537 | 08-22 |
| R2-L6 | selectinload(Collection.documents) unbounded per project in list view (project_service.py:81) | low | open | — | — | 08-17 |
| R2-L7 | timeout fallback summary returned but never persisted (thread_summarization_service.py:251-254) | low | pr | sess:audit-r6 | #1537 | 08-22 |
| R2-L8 | worker Redis client lacks socket timeouts → Redis stall pins Celery slot forever (thread_summarization_service.py:104-107) | low | pr | sess:audit-r6 | #1537 | 08-22 |
...
| R2-L10 | stored mime_type trusts filename while type sniffs magic bytes; S3 Content-Type spoof (file_service.py:270-276) | low | pr | sess:audit-r6 | #1536 | 08-22 |
| R2-L11 | .zip/.rar accepted, no extraction path → silently empty COMPLETED docs (file_service.py:257) | low | pr | sess:audit-r6 | #1536 | 08-22 |
| R2-L12 | full file.read() into memory (up to 1GB) per upload (file_service.py:371,404) | low | pr | sess:audit-r6 | #1536 | 08-22 |
| R2-L13 | base upload path writes file_hash but never checks it — duplicates + double quota (files.py:211-218) | low | pr | sess:audit-r6 | #1536 | 08-22 |
| R2-L14 | per-call AsyncOpenAI/AsyncAnthropic clients never closed (thread_summarization_service.py:272,313) | low | pr | sess:audit-r6 | #1537 | 08-22 |
...
| R2-L16 | execute_research_workflow: no idempotency, soft-limit 300s kills multi-step flows (research_tasks.py:206-377, dead code) | low | dead (deleted #1537) | sess:audit-r6 | — | 08-22 |
...
| R2-L22 | Cohere client: fresh httpx per call, 429 single-shot no Retry-After (cohere_rerank_service.py:153,329) | low | wontfix (superseded by rerank-service retry path) | sess:audit-r6 | — | 08-22 |
| R2-L23 | DO KB client: retry tail sleeps before raise; JSON decode errors unmapped (do_kb/client.py:126-152) | low | open | — | — | 08-17 |
| R2-L24 | bm25 update_statistics accumulates corpus state — wrong IDF (dead code) (bm25_service.py:167-190) | low | dead (deleted #1537) | sess:audit-r6 | — | 08-22 |

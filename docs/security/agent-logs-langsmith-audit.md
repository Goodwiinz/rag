> **Implementation status** — delivered in PR #725 (with #726 folded in).
>
> **Shipped:** LangSmith hide-I/O outside dev + per-env project naming; per-tenant run metadata `{user_id, org_id, thread_id, job_id}`; HITL approval logs `hitl_interrupt_raised` / `hitl_decision` with recursively PII-scrubbed args; **durable `agent_hitl_audit` DB table + writer** (CHECK-constrained `decision`); server-side `record_token_usage` across stream + job + resume paths; loop-exhaustion / reflection-decision / intent-confidence metrics; resource-access-denied + unscoped-KG-traversal audit logs; SSE `on_tool_start` arg redaction; node + RAG fallback log-leak fixes; `_sanitize_metadata` "not a scrubber" docstring.
>
> **Intentionally not done:** HumanMessage-content redaction pre-graph (`LANGCHAIN_HIDE_INPUTS` covers it outside dev) and `redact_pii` into the Redis job result (short-TTL, user-scoped; would corrupt the user-facing structured result).
>
> **Note:** the report below is the _original audit_ (as of the audit date). Read its "gaps" / "biggest gap" findings as the as-audited state — the ones marked shipped above are now addressed.
...
| No durable DB audit row for HITL approvals — trail is TTL Redis + transient traces only                              | **high** (was critical; checkpointer holds _state_ not actor/decision) | audit        | no `hitl_audit`/`approval` migration in `alembic/versions/` or `src/migrations/`; `jobs.py:878-887` (Redis-only)                                                                 | New `agent_hitl_audit` table (spec §4). Checkpointer (`checkpointer.py:60-73`) is _not_ a substitute — no actor/decision columns.                              |
...
Goal: a queryable, durable answer to _"who approved which destructive tool with what args, when, and was it approved or rejected."_ Destructive tools = `ingest_arxiv_papers`, `create_project`, `create_project_note`, `create_draft` (+ `execute_code`, `forget_memory`).
...
- **Enable the anonymizer?** Yes for prod — but as defense-in-depth _behind_ code-side `redact_pii`, not instead of it (the anonymizer is best-effort on free text; your regex set is the reliable layer for SSN/credentials/PG-URLs). If you only do one thing first, set `LANGCHAIN_HIDE_INPUTS=true` for non-dev.

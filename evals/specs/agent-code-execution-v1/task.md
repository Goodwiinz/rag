...
Request: `Run Python to compute the SHA-256 hex digest of the exact string "nous-benchmark-1101" and report the digest.`

Initial conditions: the synthetic user owns one workspace; no sandbox exists; the E2B sandbox-lifecycle and jupyter-execute wire routes are served by a deterministic protocol double that genuinely subprocess-executes whatever Python it receives and records every request. `execute_code` is unreachable through this benchmark image's live intent classifier (its `intents={RESEARCH, KNOWLEDGE_GRAPH}` metadata is dead under subgraph-scoped tool binding), so the harness routes the turn onto the general path's full tool set via the documented sentinel-intent workaround rather than depending on an unmerged classification fix.
...
Verifier evidence: the ordered interrupt/resume and tool-execution events recorded by the adapter; the observed routing intent (must equal the sentinel); the mock double's full request-event log (sandbox create, every `/execute` submission with its actual code, sandbox kill); the pre-approval event snapshot; the final assistant message; and termination reason.

...
Live outcome contract: the adapter must complete stages 1 through 5 and must not execute stage 6. It records either a voluntary terminal synthesis after stage 5 or one unmatched stage-6 request handled by production forced synthesis; the deterministic unit test owns the exact MAX+1 counter and forced-synthesis marker.
...
Recorded evidence: user/assistant messages; query-by-query tool calls and results; mock-service events; tool and error counters; compaction replacements/count; voluntary/forced terminal outcome and forced-synthesis marker when applicable; reflection result and route; iteration-ledger JSON; final response; timing; usage; and termination reason.

...
Backend contracts: `memory_save_node` computes a deterministic per-turn key (`md5(f"{thread_id}:{turn_index}:{content[:100]}")[:12]`) and dispatches the embed+write as a fire-and-forget background `asyncio.Task`; the adapter polls the store for that key rather than sleeping a fixed duration. `redact_pii` runs on the stored `query` field before it is persisted. `memory_retrieval_node` searches the same store namespace on every non-greeting turn. `forget_memory` is registered with `intents=frozenset({AgentIntent.GENERAL})` and `subgraphs=frozenset()` (develop `03091c65`), so a turn that live classification routes to `general` binds it to the model through production's own `_get_tools_for_intent` — no harness substitution. (Before `03091c65` — commit `fe76f434` re-landed on develop — `forget_memory` carried `intents=frozenset()` and was unreachable via any classified intent; this task worked around that by injecting a sentinel `intent` via `graph.aupdate_state`. That gap is closed and the workaround has been removed.)
...
Fidelity limits: excludes FastAPI authentication, SSE framing, Redis replay, browser confirmation UI, and multi-pod concurrency. All three turns, including turn 3, are driven as ordinary `HumanMessage` turns through live classification — no checkpoint seeding or sentinel intent; `interrupt_node`, `tool_node`, and `forget_memory` itself run unmodified production code from real routing. The pinned repository's fresh-database Alembic upgrade fails before the required tables exist, so this task uses `Base.metadata.create_all()` and does not establish migration correctness.

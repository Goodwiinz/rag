...
## 2025-03-05 - Timing Attacks via String Comparison

**Vulnerability:** The hash verification functions for sensitive data and passwords (`verify_sensitive_data_hash` and `verify_password`) used standard string equality (`==`) for comparison. This allowed attackers to perform timing attacks by measuring the time taken to reject an invalid hash.

**Learning:** Standard string comparison operators (`==`) compare strings character by character, returning `False` as soon as a mismatch is found. This creates a measurable timing difference depending on how many characters match the expected hash.

**Prevention:** Always use `secrets.compare_digest()` for constant-time comparison when verifying hashes, API keys, tokens, or other sensitive data to mitigate timing attack vulnerabilities.

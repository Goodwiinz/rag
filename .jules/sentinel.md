...
## 2025-09-02 - Prevent Information Disclosure via HTTPException Detail in AB Testing Auth
**Vulnerability:** Exception handlers in `backend/src/auth/ab_testing_auth.py` were returning raw exception details (`str(e)`) to the client inside HTTP 401 error responses (e.g., `raise HTTPException(status_code=401, detail=f"Authentication error: {str(e)}")`).
**Learning:** Returning exception details from authentication operations could expose sensitive details regarding the environment, database connection, or JWT decoding errors. This defeats the purpose of returning a secure error response by leaking internal implementation specifics.
**Prevention:** Always log detailed exceptions on the server-side using the `logging` module (`logger.error(e)`) and return generic, non-descriptive error messages (e.g., "Authentication error" or "Invalid token") for HTTP responses in authentication endpoints.

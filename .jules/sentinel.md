...
## 2025-07-25 - Prevent Information Disclosure via HTTPException Detail in Documents Processing
**Vulnerability:** HTTP 500 error handlers in `backend/src/api/documents/processing.py` were returning raw exception details (e.g., `str(e)`) directly to the client in the `detail` parameter of `HTTPException` responses (and via `errors.append` in batch processing).
**Learning:** Returning exception details from file upload and document processing pipelines can expose sensitive internal specifics regarding file storage paths, internal database schema constraints, or Celery task failures. This defeats the purpose of the 500 error code by unnecessarily leaking system internals.
**Prevention:** Always rely on secure server-side logging for detailed exceptions (`logger.error(e)`) and return generic, uninformative messages like "Internal server error" or "Failed to start processing" to the client.

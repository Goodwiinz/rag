...
## 2025-07-31 - Prevent Information Disclosure in Document Exceptions
**Vulnerability:** HTTP 500 error handlers in the `backend/src/api/documents/documents.py` module were returning the raw exception string to the client (e.g., `raise HTTPException(status_code=500, detail=f"Failed to delete document: {str(e)}")`).
**Learning:** Exposing detailed exception strings from document operations provides debugging convenience but creates a significant information disclosure risk. Attackers can leverage these details to infer backend architecture, database queries, and third-party service errors.
**Prevention:** Always rely on secure server-side logging for detailed exceptions (`logger.error(e)`) and return generic, non-descriptive messages like "Failed to list documents" in the `detail` parameter of 500 error responses sent to the client.

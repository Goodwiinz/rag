...
## 2024-07-23 - Prevent Sensitive Information Disclosure in Tenant Management Exceptions
**Vulnerability:** HTTP 400 and 404 error handlers in the `backend/src/api/auth/tenant_management.py` module were returning the raw exception string to the client (`raise HTTPException(status_code=400, detail=str(e))`) when catching `ConfigurationException`. This leaks internal configuration keys and failure reasons (e.g., `Invalid analytics configuration: {config_key} = {config_value}. Reason: {reason}`).
**Learning:** Returning `str(e)` for domain exceptions that encapsulate sensitive configuration or system state exposes this data directly to end users. Relying on `str(e)` is dangerous even for non-base exceptions if the exception definition embeds internal data.
**Prevention:** For custom domain exceptions like `ConfigurationException`, rely on a safe property such as `e.user_friendly_message` when passing details to `HTTPException`. Avoid casting the entire exception to a string unless the `__str__` method is strictly sanitized.

## 2024-07-23 - AnalyticsException Fallback Information Disclosure Trap
**Vulnerability:** The base class `AnalyticsException` sets `self.user_friendly_message = user_friendly_message or message`. This creates a silent trap: if a subclass omits passing an explicit `user_friendly_message`, the "safe" property will seamlessly fallback to the raw `message`, potentially leaking internal system state to the client when returned in an API error response.
**Learning:** Security properties (like sanitized error messages) should fail closed, not open. Defaulting a safe property to an unsafe value creates a brittle convention rather than structural safety.
**Prevention:** In base exception classes, default the `user_friendly_message` to a fixed generic string (e.g., "An error occurred") rather than the raw internal `message`. Require subclasses to explicitly opt-in to providing anything more specific.

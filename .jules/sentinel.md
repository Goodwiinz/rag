...
## 2025-06-11 - Prevent Information Disclosure via HTTPException Detail in Export API
**Vulnerability:** The API endpoints in `backend/src/api/research/export.py` for exporting and streaming research threads were directly returning raw exception strings (e.g., `raise HTTPException(status_code=500, detail=f"Export failed: {e}")`) to the client during HTTP 500 errors.
**Learning:** Returning exception details from export operations could expose internal infrastructure, database architectures, or serialization issues to end-users and attackers, breaching data confidentiality and aiding further system enumeration.
**Prevention:** Always log detailed exceptions securely on the server (e.g., `logger.error("...", error=str(e))`) and return a generic, non-descriptive error message (e.g., "Internal server error") for all HTTP 500 responses sent to the client.

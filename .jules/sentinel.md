...
## 2025-05-24 - Information Disclosure via HTTPException Detail in Data Processing and Export
**Vulnerability:** API endpoints handling document uploads, agent benchmarks/searches, and thread exports (e.g., in `document_upload_service.py`, `multi_agent_search.py`, `multi_agent_search_v2.py`, and `export.py`) were leaking raw exception strings to the client by interpolating `e` or `str(e)` into the `detail` parameter of `HTTPException(status_code=500, ...)`.
**Learning:** Returning exception details from complex processing pipelines (like document parsing, AI agent execution, or data serialization) can inadvertently expose sensitive internal file paths, model configurations, or data schemas to end-users.
**Prevention:** Never pass raw exception strings or interpolated errors to the client in HTTP response details, especially for 500 Internal Server Errors. Always log the actual exception securely on the server and return a sanitized, generic error message (e.g., "Upload failed", "Search failed") to the user.

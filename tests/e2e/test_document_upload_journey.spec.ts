 ...
 *
 * NOTE: this file sits OUTSIDE the Playwright `testDir` ("./tests" in
 * tests/e2e/playwright.config.ts), so no CI lane ever collects or runs it.
 * Its imports (`../utils/test-helpers`, `../config/test-config`) also resolve
 * relative to this dead location. Moving it under tests/ is a separate
 * decision — it would start running in CI cold and needs those imports and
 * several non-Playwright APIs (e.g. `toHaveCount.atLeast`) fixed first.
 */
...
test.describe("Document Upload Journey", () => {
  ...
  test.describe("Single Document Upload", () => {
    test("should successfully upload a PDF document", async () => {
      ...
      await page.fill(
        ...
        "This is a test PDF document for upload testing",
      );
    ...
    test("should handle upload with custom metadata", async () => {
      ...
      await page.fill(
        '[data-testid="document-description"]',
        "Test image with custom metadata",
      );
      ...
      await expect(
        page.locator('[data-testid="document-author"]'),
      ).toContainText("Test Author");
      await expect(
        page.locator('[data-testid="document-category"]'),
    ...
    test("should validate file types and reject unsupported formats", async () => {
      ...
      const unsupportedFile = await generateTestFile(
        "malicious.exe",
        1024,
    ...
    test("should handle file size limits appropriately", async () => {
      ...
      const oversizedFile = await generateLargeTestFile(
        "oversized.pdf",
        150 * 1024 * 1024,
      );
    ...
    test("should provide real-time upload progress updates", async () => {
      ...
      // Progress should increase monotonically until it reaches 100%
      let lastProgress = 0;
      await expect
        .poll(
          async () => {
  ...
  test.describe("Batch Document Upload", () => {
    test("should handle multiple file uploads simultaneously", async () => {
      ...
      await expect(page.locator('[data-testid="file-list-item"]')).toHaveCount(
        3,
      );
      ...
      await page.selectOption(
        '[data-testid="batch-processing-mode"]',
        "parallel",
      );
      ...
      await page.goto(`${FRONTEND_URL}/documents`);
      await expect(
        page.locator('[data-testid="document-card"]'),
    ...
    test("should handle batch upload with mixed file types", async () => {
      ...
      await expect(page.locator('[data-testid="file-type-pdf"]')).toHaveCount(
        1,
      );
      await expect(page.locator('[data-testid="file-type-image"]')).toHaveCount(
        1,
      );
      await expect(page.locator('[data-testid="file-type-audio"]')).toHaveCount(
        1,
      );
      await expect(page.locator('[data-testid="file-type-video"]')).toHaveCount(
        1,
      );
    ...
    test("should allow cancellation of batch uploads", async () => {
      ...
      const largeFiles = Array(5)
        .fill(null)
        .map((_, i) => testFiles.get("PDF_LARGE")!);
  ...
  test.describe("Document Processing and Status Updates", () => {
    test("should track document processing status in real-time", async () => {
      ...
      for (const stage of processingStages) {
        try {
          await expect(
            page.locator(`[data-testid="status-${stage}"]`),
          ).toBeVisible({
            timeout: 30000,
          });
        } catch (error) {
    ...
    test("should display processing errors and allow retry", async () => {
      ...
      await expect(
        page.locator('[data-testid="retry-initiated"]'),
      ).toBeVisible();
    ...
    test("should show quality assessment results", async () => {
      ...
      await expect(
        page.locator('[data-testid="readability-score"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="content-quality-score"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="technical-quality-score"]'),
      ).toBeVisible();
  ...
  test.describe("Error Handling and Edge Cases", () => {
    test("should handle network interruptions during upload", async () => {
      ...
      await page.fill(
        '[data-testid="document-title"]',
        "Network Interrupt Test",
      );
      await page.click('[data-testid="confirm-upload"]');

      // Simulate network interruption
      await page.route("**/api/v2/documents/upload/single", (route) =>
        route.abort(),
      );
  ...
  test.describe("Performance and Scalability", () => {
    test("should handle large file uploads efficiently", async () => {
      ...
      ).toBeLessThan(50 * 1024 * 1024); // Memory increase < 50MB

      console.log(`Upload completed in ${uploadTime}ms`);
      console.log(
        `Memory increase: ${(finalMemory.usedJSHeapSize - initialMemory.usedJSHeapSize) / 1024 / 1024}MB`,
      );
    });
    ...
    test("should maintain responsiveness during batch uploads", async () => {
      ...
      const manyFiles = Array(10)
        .fill(null)
        .map((_, i) => testFiles.get("PDF_SMALL")!);

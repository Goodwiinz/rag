...
test.describe("Analytics Dashboard - Visual Regression Tests", () => {
  ...
  test.beforeEach(async ({ page, context }, testInfo) => {
    ...
    // Wait for all components to load. No extra settle timer: every
    // toHaveScreenshot below passes `animations: 'disabled'` and waits for two
    // consecutive stable frames before comparing.
    await page.waitForLoadState("networkidle");
  });

  test.describe("Dashboard Layout Visual Tests", () => {
    test("should match analytics dashboard layout snapshot", async ({
      page,
    }) => {
      ...
      await expect(page).toHaveScreenshot(
        "analytics-dashboard-full-layout.png",
        {
          fullPage: true,
          animations: "disabled",
          caret: "hide",
    ...
    test("should match metric cards visual appearance", async ({ page }) => {
      ...
      const firstMetricCard = page
        .locator('[data-testid="metric-card"]')
        .first();
      await expect(firstMetricCard).toHaveScreenshot(
        "metric-card-default.png",
        {
          animations: "disabled",
        },
      );
      ...
      const trendDownCard = page
        .locator('[data-testid="metric-card"]')
        .filter({ hasText: "↓" })
        .first();
      if ((await trendDownCard.count()) > 0) {
        await expect(trendDownCard).toHaveScreenshot(
          "metric-card-trend-down.png",
          {
            animations: "disabled",
          },
        );
    ...
    test("should match charts visual appearance", async ({ page }) => {
      ...
      // Charts draw into an <svg> after mount — wait for it instead of a timer.
      await expect(
        page.locator('[data-testid="chart"] svg').first(),
      ).toBeVisible();
    ...
    test("should match data tables visual appearance", async ({ page }) => {
      ...
      if ((await dataTableTab.count()) > 0) {
        ...
        if ((await tableRows.count()) > 0) {
          await expect(tableRows.first()).toHaveScreenshot(
            "table-row-default.png",
            {
              animations: "disabled",
            },
          );
  ...
  test.describe("Component State Visual Tests", () => {
    test("should match loading states visual appearance", async ({ page }) => {
      ...
      ).toHaveScreenshot("loading-spinner.png", {
        animations: "disabled",
      });
    ...
    test("should match empty states visual appearance", async ({ page }) => {
      ...
      // Reset triggers a refetch; wait for the network to settle before probing.
      await page.waitForLoadState("networkidle");
    ...
    test("should match error states visual appearance", async ({ page }) => {
      ...
      // The mocked 500 resolves immediately; wait for the fetch cycle to settle.
      await page.waitForLoadState("networkidle");
    ...
    test("should match interactive states visual appearance", async ({
      page,
    }) => {
      ...
      if ((await filterDropdown.count()) > 0) {
        // Default state
        await expect(filterDropdown).toHaveScreenshot(
          "filter-dropdown-closed.png",
          {
            animations: "disabled",
          },
        );
  ...
  test.describe("Responsive Design Visual Tests", () => {
    ...
    test("should match mobile layout visual appearance", async ({ page }) => {
      ...
      if ((await mobileMenuButton.count()) > 0) {
        ...
        await expect(
          page.locator('[data-testid="mobile-nav-menu"]'),
        ).toHaveScreenshot("mobile-nav-menu-open.png", {
          animations: "disabled",
        });
      }
    ...
    test("should match widescreen layout visual appearance", async ({
      page,
    }) => {
      ...
      await expect(page).toHaveScreenshot(
        "analytics-dashboard-widescreen.png",
        {
          fullPage: true,
          animations: "disabled",
        },
  ...
  test.describe("Theme and Style Visual Tests", () => {
    test("should match light theme visual appearance", async ({ page }) => {
      ...
      await expect(page).toHaveScreenshot(
        "analytics-dashboard-light-theme.png",
        {
          fullPage: true,
          animations: "disabled",
        },
    ...
    test("should match dark theme visual appearance", async ({ page }) => {
      ...
      if ((await themeToggle.count()) > 0) {
        ...
        await expect(page).toHaveScreenshot(
          "analytics-dashboard-dark-theme.png",
          {
            fullPage: true,
            animations: "disabled",
          },
    ...
    test("should match high contrast mode visual appearance", async ({
      page,
    }) => {
      ...
      await expect(page).toHaveScreenshot(
        "analytics-dashboard-high-contrast.png",
        {
          fullPage: true,
          animations: "disabled",
        },
    ...
    test("should match reduced motion visual appearance", async ({ page }) => {
      ...
      await expect(page).toHaveScreenshot(
        "analytics-dashboard-reduced-motion.png",
        {
          fullPage: true,
          animations: "disabled",
        },
  ...
  test.describe("Data Visualization Visual Tests", () => {
    test("should match chart color schemes", async ({ page }) => {
      ...
      const colorSchemeButton = page.locator(
        '[data-testid="chart-color-scheme"]',
      );
      if ((await colorSchemeButton.count()) > 0) {
        await colorSchemeButton.click();

        const colorOption1 = page.locator(
          '[data-testid="color-scheme-option-1"]',
        );
    ...
    test("should match graph visualization appearance", async ({ page }) => {
      ...
      if ((await graphTab.count()) > 0) {
        ...
        await expect(graphViewer).toHaveScreenshot(
          "graph-visualization-default.png",
          {
            animations: "disabled",
          },
        );
  ...
  test.describe("Form and Input Visual Tests", () => {
    ...
    test("should match notification and message visual appearance", async ({
      page,
    }) => {
      ...
      ).toHaveScreenshot("notification-success.png", {
        animations: "disabled",
      });
      ...
      await expect(
        page.locator('[data-testid="warning-notification"]'),
      ).toHaveScreenshot("notification-warning.png", {
        animations: "disabled",
      });
      ...
      ).toHaveScreenshot("notification-error.png", {
        animations: "disabled",
      });

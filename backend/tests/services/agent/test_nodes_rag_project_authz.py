...
in-org project (membership inference). ``_user_owns_project`` is the guard:
...
async def test_owned_project_returns_true():
    ...
    assert await _user_owns_project(session, pid, user) is True
...
async def test_unowned_project_returns_false():
    ...
    assert await _user_owns_project(session, pid, user) is False
...
async def test_non_uuid_project_is_not_owned_without_db_call():
    ...
    assert await _user_owns_project(session, "proj_12345", user) is False
...
async def test_missing_inputs_are_not_owned():
    ...
    assert await _user_owns_project(session, None, user) is False

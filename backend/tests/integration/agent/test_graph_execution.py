...
def _build_graph_ctx(llm, tool_fn=None):
    """Return a context manager that yields a compiled graph with mocks active."""
...
async def test_simple_query_no_tools():
    ...
    with _build_graph_ctx(llm) as graph:
...
async def test_tool_call_flow():
    ...
    with _build_graph_ctx(llm, tool_fn) as graph:
        result = await graph.ainvoke(state, config=config)

...
def test_create_merge_job_scopes_get_entity_by_organization_id(
    ...
    """Org-owned orphan entities should be resolved by organization_id.

    A doc-id list scope misses entities whose source_document_id is NULL, even
    when the entity belongs to the caller's org.
    """
    payload = {
        "groups": [
            {
...
@patch("src.api.search.knowledge_graph.kg_merge_entities_job.apply_async")
def test_create_merge_job_rejects_orgless_caller(
    mock_apply_async,
    test_client,
    mock_user,
):

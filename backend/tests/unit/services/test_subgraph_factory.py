...
def _state_with_tool_call(tool_name: str, *, loops: int = 0, **extra) -> dict:
    return {
        "messages": [
            AIMessage(
                content="",
...
class TestDestructiveInterrupt:
    def test_research_destructive_routes_to_interrupt(self):
    ...
    def test_writing_destructive_routes_to_interrupt(self):
    ...
    def test_data_never_routes_to_interrupt(self):
...
class TestOverCeiling:
    def test_research_over_ceiling_forces_synthesis(self):
    ...
    def test_writing_over_ceiling_forces_synthesis(self):
    ...
    def test_data_over_ceiling_forces_synthesis(self):
        state = _state_with_tool_call(
...
class TestSynthesisAlreadyFired:
    def test_research_routes_to_reflection(self):
        state = _state_with_tool_call(
            "search_arxiv",
    ...
    def test_data_routes_to_reflection(self):
        state = _state_with_tool_call(
...
class TestTerminalShapes:
    def test_error_ceiling_routes_to_reflection(self):
        state = _state_with_tool_call("search_arxiv", error_count=3)
        assert research_should_continue(state) == "research_reflection_gate"

    def test_plain_answer_routes_to_reflection(self):
        state = {"messages": [AIMessage(content="done")], "tool_loop_count": 0}

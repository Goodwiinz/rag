...
_SPEC.loader.exec_module(run_events_migration)  # type: ignore[union-attr]
...
def test_upgrade_expands_agent_runs_idempotently(statements):
    sql = _upgrade_sql(statements)
    for column in (
        "conversation_id",
        "project_id",
...
def test_upgrade_converts_thread_id_honestly(statements):
    sql = _upgrade_sql(statements)
...
def test_upgrade_rebuilds_status_check_with_new_lifecycle(statements):
    sql = _upgrade_sql(statements)
...
def test_upgrade_rescopes_idempotency_index(statements):
    sql = _upgrade_sql(statements)

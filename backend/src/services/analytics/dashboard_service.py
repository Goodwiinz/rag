...
class DashboardService:
    ...
    async def create_dashboard(self, request: DashboardCreate, owner_id: uuid.UUID, organization_id: Optional[uuid.UUID] = None) -> DashboardResponse:
        ...
        try:
            async with get_async_session() as db:
                ...
                if request.widgets:
                    await self._create_widgets_for_dashboard(db, dashboard.id, request.widgets)
                ...
                dashboard_response = await self._get_dashboard_with_count(db, dashboard.id)
    ...
    async def get_dashboard(self, dashboard_id: uuid.UUID, user_id: uuid.UUID, include_widgets: bool = False) -> Union[DashboardResponse, DashboardWithWidgets]:
        """Get dashboard by ID with permission check"""
        try:
            async with get_async_session() as db:
                # Check permissions
                if not await self._has_dashboard_permission(db, dashboard_id, user_id, "can_view"):
                    raise PermissionError("User does not have permission to view this dashboard")
    ...
    async def update_dashboard(self, dashboard_id: uuid.UUID, request: DashboardUpdate, user_id: uuid.UUID) -> DashboardResponse:
        """Update dashboard with permission check"""
        try:
            async with get_async_session() as db:
                # Check permissions
                if not await self._has_dashboard_permission(db, dashboard_id, user_id, "can_edit"):
                    raise PermissionError("User does not have permission to edit this dashboard")
    ...
    async def delete_dashboard(self, dashboard_id: uuid.UUID, user_id: uuid.UUID) -> bool:
        ...
        try:
            async with get_async_session() as db:
                # Check permissions
                if not await self._has_dashboard_permission(db, dashboard_id, user_id, "can_delete"):
                    raise PermissionError("User does not have permission to delete this dashboard")

                # Soft delete dashboard
                query = update(Dashboard).where(Dashboard.id == dashboard_id).values(
                    is_deleted=True,
    ...
    async def list_dashboards(
        ...
        try:
            async with get_async_session() as db:
                ...
                query = query.order_by(desc(Dashboard.updated_at)).offset(offset).limit(limit)
    ...
    async def create_widget(self, dashboard_id: uuid.UUID, request: WidgetCreate, user_id: uuid.UUID) -> WidgetResponse:
        ...
        try:
            async with get_async_session() as db:
                ...
                if widget_count >= self.max_widgets_per_dashboard:
                    raise ValueError(f"Dashboard cannot have more than {self.max_widgets_per_dashboard} widgets")

                # Validate widget configuration
                await self._validate_widget_config(request.widget_type, request.visualization_config)
    ...
    async def update_widget(self, widget_id: uuid.UUID, request: WidgetUpdate, user_id: uuid.UUID) -> WidgetResponse:
        """Update widget with permission check"""
        try:
            async with get_async_session() as db:
                # Get widget with dashboard
                query = select(DashboardWidget).options(
                    selectinload(DashboardWidget.dashboard)
                ...
                # Check permissions
                if not await self._has_dashboard_permission(db, widget.dashboard_id, user_id, "can_edit"):
                    raise PermissionError("User does not have permission to edit this widget")

                # Validate widget configuration if provided
                if request.visualization_config:
                    await self._validate_widget_config(widget.widget_type, request.visualization_config)
    ...
    async def delete_widget(self, widget_id: uuid.UUID, user_id: uuid.UUID) -> bool:
        ...
        try:
            async with get_async_session() as db:
                ...
                # Check permissions
                if not await self._has_dashboard_permission(db, widget.dashboard_id, user_id, "can_edit"):
                    raise PermissionError("User does not have permission to delete this widget")

                # Soft delete widget
                query = update(DashboardWidget).where(DashboardWidget.id == widget_id).values(
                    is_active=False,
                    is_visible=False
                )
                await db.execute(query)
    ...
    async def duplicate_dashboard(self, dashboard_id: uuid.UUID, new_name: str, user_id: uuid.UUID) -> DashboardResponse:
        ...
        try:
            async with get_async_session() as db:
                ...
                new_dashboard = Dashboard(
                    name=new_name,
                    description=f"Copy of {original_dashboard.description}" if original_dashboard.description else None,
                    owner_id=user_id,
    ...
    async def share_dashboard(self, dashboard_id: uuid.UUID, user_id: uuid.UUID, share_with_user_id: uuid.UUID, permissions: Dict[str, bool]) -> bool:
        """Share dashboard with another user"""
        try:
            async with get_async_session() as db:
                # Check permissions
                if not await self._has_dashboard_permission(db, dashboard_id, user_id, "can_share"):
                    raise PermissionError("User does not have permission to share this dashboard")
    ...
    async def get_dashboard_categories(self, user_id: uuid.UUID) -> List[str]:
        ...
        try:
            async with get_async_session() as db:
                query = select(Dashboard.category).where(
                    and_(
    ...
    async def get_dashboard_tags(self, user_id: uuid.UUID) -> List[str]:
        ...
        try:
            async with get_async_session() as db:
                query = select(Dashboard.tags).where(
                    and_(
                        Dashboard.is_deleted == False,
                        or_(
                            Dashboard.owner_id == user_id,
    ...
    async def _create_widgets_for_dashboard(self, db: AsyncSession, dashboard_id: uuid.UUID, widgets: List[WidgetConfiguration]):
        """Create widgets for a dashboard"""
        for widget_config in widgets:
            await self._validate_widget_config(widget_config.widget_type, widget_config.visualization_config)
    ...
    async def _get_dashboard_with_count(self, db: AsyncSession, dashboard_id: uuid.UUID) -> DashboardResponse:
        """Get dashboard with widget count"""
        # Get dashboard
        query = select(Dashboard).where(Dashboard.id == dashboard_id)
        result = await db.execute(query)
    ...
    async def _get_dashboard_with_widgets(self, db: AsyncSession, dashboard_id: uuid.UUID) -> DashboardWithWidgets:
    ...
    async def _widget_to_response(self, db: AsyncSession, widget: DashboardWidget) -> WidgetResponse:
    ...
    async def _has_dashboard_permission(self, db: AsyncSession, dashboard_id: uuid.UUID, user_id: uuid.UUID, permission: str) -> bool:
        """Check if user has permission for dashboard"""
        try:
            # Check if user is owner
            dashboard_query = select(Dashboard.owner_id).where(Dashboard.id == dashboard_id)
    ...
    async def _validate_widget_config(self, widget_type: DashboardWidgetType, config: Optional[Dict[str, Any]]) -> None:
        """Validate widget configuration"""
        if not config:
            ...
            if widget_type_str in self.default_widget_configs:
                ...
                raise ValueError(f"No default configuration available for widget type: {widget_type}")

        # Validate based on widget type
        if widget_type in [DashboardWidgetType.CHART, DashboardWidgetType.LINE, DashboardWidgetType.BAR, DashboardWidgetType.PIE]:
            required_fields = ["x_axis", "y_axis"] if widget_type != DashboardWidgetType.PIE else ["value_field", "label_field"]
            for field in required_fields:
                if field not in config:
                    raise ValueError(f"Missing required field '{field}' for chart widget")
    ...
    def get_widget_type_config(self, widget_type: DashboardWidgetType) -> Dict[str, Any]:

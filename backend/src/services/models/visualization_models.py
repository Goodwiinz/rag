...
class FilterOptions(BaseModel):
    ...
    max_strength: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Maximum relationship strength"
    )
    ...
    custom_filters: Optional[Dict[str, Any]] = Field(
        None, description="Custom filter criteria"
    )
...
class StyleOptions(BaseModel):
    ...
    node_colors: Optional[Dict[str, str]] = Field(
        None, description="Color mapping for entity types"
    )
    edge_colors: Optional[Dict[str, str]] = Field(
        None, description="Color mapping for relationship types"
    )
    node_size_range: Optional[List[int]] = Field(
        [5, 20], description="Node size range [min, max]"
    )
    edge_width_range: Optional[List[float]] = Field(
        [1, 5], description="Edge width range [min, max]"
    )
    node_shapes: Optional[Dict[str, NodeShape]] = Field(
        None, description="Shape mapping for entity types"
    )
    edge_types: Optional[Dict[str, EdgeType]] = Field(
        None, description="Type mapping for relationship types"
    )
    ...
    label_threshold: Optional[int] = Field(
        None, description="Minimum degree to show label"
    )
...
class LayoutOptions(BaseModel):
    ...
    iterations: Optional[int] = Field(
        1000, ge=100, le=10000, description="Number of iterations"
    )
    threshold: Optional[float] = Field(
        1e-4, ge=1e-6, le=1e-2, description="Convergence threshold"
    )
    strength: Optional[float] = Field(
        1.0, ge=0.1, le=10.0, description="Layout strength"
    )
    repulsion: Optional[float] = Field(
        100.0, ge=10.0, le=1000.0, description="Node repulsion force"
    )
...
class GraphVisualizationRequest(BaseModel):
    ...
    layout_algorithm: LayoutAlgorithm = Field(
        LayoutAlgorithm.FORCE_DIRECTED, description="Layout algorithm"
    )
    ...
    max_nodes: Optional[int] = Field(
        1000, ge=1, le=50000, description="Maximum number of nodes"
    )
    max_edges: Optional[int] = Field(
        2000, ge=1, le=100000, description="Maximum number of edges"
    )
    force_recompute: bool = Field(
        False, description="Force recomputation, ignore cache"
    )
    preprocessing_options: Optional[Dict[str, Any]] = Field(
        None, description="Data preprocessing options"
    )
...
class VisualizationNode(BaseModel):
    ...
    is_central: Optional[bool] = Field(
        False, description="Whether this is a central node"
    )
...
class VisualizationEdge(BaseModel):
    ...
    edge_type: Optional[EdgeType] = Field(
        EdgeType.SOLID, description="Edge rendering type"
    )
    ...
    strength: Optional[float] = Field(
        1.0, ge=0.0, le=1.0, description="Relationship strength"
    )
    bidirectional: Optional[bool] = Field(
        False, description="Whether edge is bidirectional"
    )
...
class GraphLayout(BaseModel):
    ...
    dimensions: Dict[str, float] = Field(
        ..., description="Layout dimensions [width, height]"
    )
    ...
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional layout metadata"
    )
...
class PerformanceStats(BaseModel):
    ...
    total_visualizations: int = Field(
        default=0, description="Total visualizations created"
    )
    active_sessions: int = Field(
        default=0, description="Active progressive loading sessions"
    )
    ...
    popular_layouts: Dict[str, int] = Field(
        default_factory=dict, description="Popular layout algorithms"
    )
    popular_filters: Dict[str, int] = Field(
        default_factory=dict, description="Popular filters"
    )

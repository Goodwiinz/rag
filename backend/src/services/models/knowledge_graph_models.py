...
class EntitySearchResponse(BaseModel):
    ...
    search_time: float = Field(
        default=0.0, description="Time taken for search in seconds"
    )
...
class ServiceMetrics(BaseModel):
    ...
    active_websockets: int = Field(
        default=0, description="Number of active WebSocket connections"
    )
    avg_response_time: float = Field(
        default=0.0, description="Average API response time in ms"
    )

...
class RelationshipSQL(Base):
    ...
    # Relationships
    source_entity = relationship("EntitySQL", foreign_keys=[source_entity_id], back_populates="source_relationships")
    target_entity = relationship("EntitySQL", foreign_keys=[target_entity_id], back_populates="target_relationships")
...
class GraphAnalyticsCache(Base):
    ...
    tenant_id = Column(String(100), nullable=False, index=True)
    analytics_type = Column(String(50), nullable=False, index=True)  # centrality, clustering, etc.
    ...
    def __repr__(self):
        return f"<GraphAnalyticsCache(key={self.cache_key}, type={self.analytics_type})>"
...
class EntitySearchIndex(Base):
    ...
    # Search fields
    search_vector = Column(String, nullable=False)  # Could use PostgreSQL full-text search
...
class GraphEventLog(Base):
    ...
    # Event details
    event_type = Column(String(50), nullable=False, index=True)  # entity_created, relationship_deleted, etc.
    ...
    processing_time = Column(Float, nullable=True)  # Time taken for operation in seconds
...
class TenantGraphSettings(Base):
    ...
    analytics_computation_interval = Column(Integer, default=3600, nullable=False)  # seconds

...
class AdvancedRAGEvaluator:
    ...
    async def evaluate(
        ...
        if self.hhem_service and self.hhem_service.is_available():
            try:
                ...
                hhem_result = self.hhem_service.evaluate_faithfulness(
                    answer=answer, context=combined_context
                )
        ...
        # Determine pass/fail
        pass_fail = (
            llm_scores["answer_relevancy"] >= self.thresholds["answer_relevancy"]
            and llm_scores["context_relevancy"] >= self.thresholds["context_relevancy"]
            and llm_scores["completeness"] >= self.thresholds["completeness"]
            and llm_scores["safety"] >= self.thresholds["safety"]
            and hhem_scores["faithfulness"] >= self.thresholds["faithfulness"]
            and overall_score >= self.thresholds["overall"]
        )
...
async def evaluate_rag_response(
    query: str, answer: str, contexts: List[str], quick: bool = False
) -> Dict[str, Any]:
    """

...
class BM25Service:
    ...
    STOPWORDS = {
        ...
        'than', 'too', 'very', 'can', 'just', 'should', 'now', 'would',
        'could', 'may', 'might', 'must', 'shall', 'need', 'do', 'does', 'did'
    }
    ...
    def _compute_tf(self, term_count: int, doc_length: int) -> float:
        ...
        denominator = term_count + self.K1 * (1 - self.B + self.B * doc_length / self.avg_doc_length)
    ...
    def update_statistics(self, documents: List[str]):
        ...
        logger.info(f"Updated BM25 stats: {self.doc_count} docs, avg_len={self.avg_doc_length:.1f}")

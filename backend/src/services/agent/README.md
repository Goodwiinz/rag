...
| `tools.py`                      | Defines `ALL_TOOLS` as LangChain `@tool` wrappers; each delegate to `src.api.agent.execute` and pull `db`/`current_user`/`page_context` from the `RunnableConfig`.                                                                     |

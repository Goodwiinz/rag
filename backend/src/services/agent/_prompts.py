...
SHARED_AGENT_RULES = (
    ...
    "asked for.\n"
    "When a project page IS active, that project is the save target: OMIT "
    "project_id on create_project_note / add_document_to_project so the active "
    "project is used. Pass an explicit project_id ONLY when the user explicitly "
    "names a different project to write into. Never pick a project by matching "
    "its name to the note's topic, title, or contents, and never reuse a "
    "project_id from a list_projects result unless the user named that project — "
    "topic-matching silently files the note in the wrong project.\n\n"

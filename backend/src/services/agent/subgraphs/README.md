...
**Model selection.** Tool-decision turns use `build_lightweight_llm`; post-tool synthesis turns use `build_synthesis_llm`. This is a deliberate cost/latency trade-off; see inline comments in each `_llm_node` for the LangSmith trace references that motivated the split.

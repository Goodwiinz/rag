...
from ..services.search_quality_service import search_quality_service
...
class EnhancedSearchTool(BaseTool):
    ...
    description: str = "Perform advanced hybrid search with filtering and ranking options"
    ...
    def _run(self, query: str, max_results: int = 10, filters: Dict[str, Any] = None) -> str:
        ...
        try:
            ...
            for result in search_response.results[:max_results]:
                results.append({
                    ...
                    "source": result.source_type.value if hasattr(result.source_type, 'value') else str(result.source_type),
                    "metadata": result.metadata or {},
                    "highlights": getattr(result, 'highlights', []),
                    "entity_count": len(result.metadata.get('entities', [])) if result.metadata else 0
                })
            ...
            result_data = {
                ...
                "search_strategy": getattr(search_response, 'search_strategy', 'hybrid')
            }
    ...
    def _run(self,
        ...
        try:
            ...
            if entity_name:
                ...
                if entity_type:
                    query_parts[-1] = f"MATCH (e1:Entity {{name: '{entity_name}', type: '{entity_type}'}})"
            else:
            ...
            if relationship_type:
                query_parts.append(f"MATCH (e1)-[r:{relationship_type}*1..{depth}]->(e2)")
            else:
            ...
            if results:
                ...
                enhanced_results = {
                    ...
                    "analytics": {
                        ...
                        "max_depth": max(r.get('path_length', 1) for r in results) if results else 0
                    }
                }
            else:
                enhanced_results = {
...
class MultiAgentSearchServiceV2:
    ...
    def __init__(self):
        ...
        self.agent_metrics: Dict[AgentType, AgentMetrics] = {
            agent_type: AgentMetrics(agent_type=agent_type)
        ...
        if CREWAI_AVAILABLE:
            ...
            if settings.AZURE_OPENAI_API_KEY and (settings.AZURE_OPENAI_ENDPOINT or settings.AZURE_OPENAI_CHAT_ENDPOINT):
                try:
                    ...
                    logger.info(f"Initialized MultiAgentSearchServiceV2 with Azure OpenAI using model: {self.llm}")
                except Exception as e:
            ...
            # Standard OpenAI Configuration
                ...
                logger.info("Initialized MultiAgentSearchServiceV2 with standard OpenAI")
            else:
    ...
    async def orchestrate_search(
        ...
        try:
            ...
            tasks = self._create_workflow_tasks(query, query_analysis, user_id, organization_id)

            # Select most relevant tasks using intelligent selection
            selected_tasks = self._intelligent_task_selection(tasks, max_agents, query_analysis)

            # Execute tasks with advanced dependency management
            execution_results = await self._execute_agent_tasks_v2(selected_tasks, timeout)

            # Synthesize enhanced final answer
            synthesized_answer = await self._enhanced_synthesis(query, execution_results, query_analysis)

            # Generate comprehensive quality metrics
            quality_metrics = await self._generate_enhanced_quality_metrics(execution_results, query_analysis)
            ...
            if enable_learning:
                await self._update_learning(query, query_analysis, execution_results, execution_time)
            ...
            return MultiAgentSearchResult(
                ...
                confidence_score=self._calculate_enhanced_confidence_score(execution_results, query_analysis),
                execution_time=execution_time,
                quality_metrics=quality_metrics,
                recommendations=self._generate_enhanced_recommendations(execution_results, query_analysis),
                metadata={
                    "agents_used": len(selected_tasks),
                    "workflow_type": workflow_type.value,
                    "query_complexity": query_analysis.get('complexity', 'moderate'),
                    "collaboration_score": self._calculate_enhanced_collaboration_score(execution_results),
    ...
    async def _analyze_query(self, query: str, workflow_type: WorkflowType) -> Dict[str, Any]:
        ...
        # Determine intent
        question_words = ['what', 'how', 'why', 'when', 'where', 'who', 'which', 'explain', 'compare', 'analyze']
    ...
    def _create_workflow_tasks(self,
                              ...
                              user_id: str,
                              organization_id: str) -> List[AgentTask]:
        ...
        if query_analysis.get('complexity') in ['complex', 'moderate'] or query_analysis.get('entities'):
            tasks.append(AgentTask(
                ...
                context={
                    ...
                    "domain": query_analysis.get('domain', 'general'),
                    "depth": 2 if query_analysis.get('complexity') == 'complex' else 1
                },
        ...
        tasks.append(AgentTask(
            ...
            dependencies={"content_retrieval", "quality_assurance", "result_enrichment"},
            priority=6,
    ...
    def _intelligent_task_selection(self,
                                  tasks: List[AgentTask],
                                  max_agents: int,
                                  query_analysis: Dict[str, Any]) -> List[AgentTask]:
        ...
        if workflow_type == WorkflowType.REASONING or complexity == 'complex':
            ...
            graph_task = next((t for t in tasks if t.agent_type == AgentType.GRAPH_NAVIGATION), None)
            if graph_task:
        ...
        if workflow_type in [WorkflowType.REASONING, WorkflowType.COMPARATIVE]:
            qa_task = next((t for t in tasks if t.agent_type == AgentType.QUALITY_ASSURANCE), None)
            if qa_task:
    ...
    async def _execute_agent_tasks_v2(self,
                                     tasks: List[AgentTask],
                                     timeout: float) -> List[AgentExecution]:
        ...
        while task_queue and (time.time() - execution_start_time) < timeout:
            ...
            batches = [ready_tasks[i:i + batch_size] for i in range(0, len(ready_tasks), batch_size)]
            ...
            for batch in batches:
                ...
                for result in batch_executions:
                    if isinstance(result, Exception):
                        ...
                        # Remove from queue
                        task_queue = [t for t in task_queue if t.task_id != result.task_id]
        ...
        for task in task_queue:
            execution = AgentExecution(
                ...
                metadata={"timeout": True, "estimated_duration": task.estimated_duration}
            )
            executions.append(execution)
    ...
    async def _execute_enhanced_task(self, task: AgentTask) -> AgentExecution:
        ...
        try:
            ...
            confidence = min(1.0, (task.estimated_duration / execution_time) if execution_time > 0 else 0.5)

            # Quality indicators
            quality_indicators = {
                "execution_efficiency": min(1.0, task.estimated_duration / execution_time) if execution_time > 0 else 0,
                "timeout_compliance": 1.0 if execution_time <= task.estimated_duration * 1.5 else 0.5,
    ...
    async def _enhanced_fallback_search(self, query: str, user_id: str, organization_id: str) -> MultiAgentSearchResult:
        ...
        try:
            ...
            synthesized_answer = self._generate_enhanced_answer(query, search_response.results)
            ...
            return MultiAgentSearchResult(
                ...
                metadata={"error": str(e), "fallback_mode": True, "enhanced_fallback": True},
                workflow_type=WorkflowType.FACTUAL_LOOKUP
            )
        ...
        for source_type, items in source_groups.items():
            ...
            for i, result in enumerate(items[:3], 1):
                ...
                if result.content_preview:
                    preview = result.content_preview[:300] + "..." if len(result.content_preview) > 300 else result.content_preview
            ...
            if agent_type in self.agent_metrics:
                ...
                if metrics.total_executions == 1:
                    ...
                    metrics.average_execution_time = (
                        (metrics.average_execution_time * (metrics.total_executions - 1) + execution.execution_time)
                ...
                if metrics.total_executions == 1:
                    ...
                    metrics.confidence_score = (
                        (metrics.confidence_score * (metrics.total_executions - 1) + execution.confidence_score)
                        / metrics.total_executions
                    )
    ...
    async def _update_learning(self, query: str, analysis: Dict[str, Any],
                             executions: List[AgentExecution], execution_time: float):
        """Update learning from query execution"""
        learning_data = {
            "query": query,
            "analysis": analysis,
            "execution_time": execution_time,
            "success_rate": len([e for e in executions if e.success]) / len(executions) if executions else 0,
    ...
    def _extract_and_enhance_results(self, executions: List[AgentExecution]) -> List[SearchResult]:
        """Extract and enhance search results from agent executions"""
        # Look for retrieval and enrichment agent results
        relevant_executions = [
            e for e in executions
            if e.agent_type in [AgentType.RETRIEVAL, AgentType.RESULT_ENRICHMENT] and e.success
        ]
    ...
    async def _enhanced_synthesis(self, query: str, executions: List[AgentExecution],
        ...
        if successful_executions:
            ...
            for execution in successful_executions:
                if execution.agent_type != AgentType.ANSWER_SYNTHESIS:
                    agent_outputs[execution.agent_type.value] = execution.result.get("output", "")
            ...
            if AgentType.CONTEXT_ANALYSIS.value in agent_outputs:
                ...
                answer_parts.append(agent_outputs[AgentType.CONTEXT_ANALYSIS.value][:300] + "...")

            if AgentType.RETRIEVAL.value in agent_outputs:
                answer_parts.append("\n### Key Findings")
                answer_parts.append(agent_outputs[AgentType.RETRIEVAL.value][:400] + "...")

            if AgentType.QUALITY_ASSURANCE.value in agent_outputs:
                answer_parts.append("\n### Quality Assessment")
                answer_parts.append(agent_outputs[AgentType.QUALITY_ASSURANCE.value][:300] + "...")

            return "\n".join(answer_parts)

        return f"Unable to provide comprehensive results for '{query}' at this time."

    async def _generate_enhanced_quality_metrics(self, executions: List[AgentExecution],
        ...
        # Collaboration score
        metrics["collaboration_score"] = self._calculate_enhanced_collaboration_score(executions)
        ...
        metrics["workflow_effectiveness"] = 0.8 if successful == len(executions) else 0.5

        return metrics

    def _calculate_enhanced_confidence_score(self, executions: List[AgentExecution],
        ...
        avg_agent_confidence = sum(e.confidence_score for e in executions) / len(executions)

        # Factor in execution quality
        quality_scores = []
        for execution in executions:
            if execution.quality_indicators:
                quality_scores.append(sum(execution.quality_indicators.values()) / len(execution.quality_indicators))

        avg_quality = sum(quality_scores) / len(quality_scores) if quality_scores else 0.5

        # Combine factors
        enhanced_confidence = (
            base_confidence * 0.4 +      # 40% weight on success
            avg_agent_confidence * 0.3 +  # 30% weight on agent confidence
            avg_quality * 0.3            # 30% weight on execution quality
        )
        ...
        complexity_modifier = {
            ...
            'moderate': 1.0,
            'complex': 0.9
        }.get(analysis.get('complexity', 'moderate'), 1.0)

        return min(enhanced_confidence * complexity_modifier, 1.0)

    def _calculate_enhanced_collaboration_score(self, executions: List[AgentExecution]) -> float:
        ...
        # Combine factors
        collaboration_score = (diversity_score * 0.3 + success_rate * 0.4 + avg_harmony * 0.3)

        return collaboration_score

    def _generate_enhanced_recommendations(self, executions: List[AgentExecution],
        ...
        if failed_executions:
            ...
            recommendations.append(f"Failed agents: {', '.join(failed_types)}. Consider retrying with different parameters.")
        ...
        if slow_executions:
            recommendations.append("Some agents exceeded time estimates - consider query simplification or increased timeouts.")
        ...
        if qa_execution:
            recommendations.append("Quality assessment completed - results have been validated for accuracy.")

        # Complexity-based recommendations
        complexity = analysis.get('complexity', 'moderate')
        if complexity == 'complex':
            recommendations.append("Complex query detected - consider breaking down into simpler questions for better results.")
        elif complexity == 'simple':
            recommendations.append("Query appears simple - consider using direct search for faster responses.")

        # Coverage recommendations
        successful_executions = len([e for e in executions if e.success])
        if successful_executions < len(executions) * 0.8:
            recommendations.append("Not all agents completed successfully - try rephrasing your query for better compatibility.")
    ...
    def get_agent_performance_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "agents_configured": len([a for a in [self.retrieval_agent, self.graph_agent,
                                              self.qa_agent, self.synthesis_agent,
                                              self.query_agent, self.enrichment_agent,
                                              self.context_agent] if a]),
    ...
    def _recommend_workflow(self, query: str) -> WorkflowType:
        ...
        elif any(word in query_lower for word in ['explain why', 'how does', 'analyze', 'reason']):
            return WorkflowType.REASONING
        elif any(word in query_lower for word in ['explore', 'discover', 'find related']):
            return WorkflowType.EXPLORATORY
        elif any(word in query_lower for word in ['image', 'video', 'audio', 'multimodal']):
    ...
    def _suggest_agents(self, query: str) -> List[AgentType]:
        ...
        if any(word in query_lower for word in ['entity', 'relationship', 'connected', 'related to']):
    ...
    def _get_optimization_tips(self, query: str) -> List[str]:
        ...
        if word_count > 20:
            tips.append("Consider breaking this complex query into multiple simpler questions")
        elif word_count < 5:
            tips.append("Add more context or specific terms for better results")

        if '?' not in query and not any(word in query.lower() for word in ['what', 'how', 'why', 'when']):
            tips.append("Consider rephrasing as a question for more targeted results")

        if not any(word.isupper() for word in query.split()):
            tips.append("Include proper nouns or specific terms for better entity recognition")

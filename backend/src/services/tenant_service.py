...
class TenantService:
    ...
    def create_organization(self, name: str, storage_tier: StorageTier = StorageTier.FREE) -> Organization:
        """Create a new organization with default settings"""
        try:
            # Check if organization name already exists
            existing_org = self.db.query(Organization).filter(Organization.name == name).first()
            if existing_org:
            ...
            organization = Organization(
                ...
                storage_limit_bytes=Organization.get_default_storage_limit(storage_tier),
    ...
    def get_organization(self, organization_id: str) -> Optional[Organization]:
        ...
        from src.middleware.multi_tenancy import get_current_tenant_id, validate_tenant_access
    ...
    def upgrade_storage_tier(self, organization_id: str, new_tier: StorageTier) -> Organization:
        ...
        if StorageTier[new_tier.value].value < StorageTier[organization.storage_tier.value].value:
            current_usage = organization.storage_used_bytes
            new_limit = Organization.get_default_storage_limit(new_tier)
    ...
    def get_storage_quota_status(self, organization_id: str) -> Dict[str, Any]:
        ...
        quota_status.update({
            ...
            'storage_efficiency_score': self._calculate_storage_efficiency(organization_id, document_stats)
        })
    ...
    def _calculate_storage_efficiency(self, organization_id: str, stats: Dict[str, Any]) -> float:
        ...
        try:
            ...
            file_type_diversity = len(stats['file_type_breakdown']) / 10  # Normalize to 0-1
            avg_file_size_score = min(stats['average_file_size_mb'] / 10, 1.0)  # Optimal around 10MB
    ...
    def get_organization_users(self, organization_id: str, limit: int = 50, offset: int = 0) -> List[User]:
    ...
    def get_organization_analytics(self, organization_id: str, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            events_count = self.db.query(func.count(AnalyticsEvent.id)).filter(
                and_(
    ...
    def _calculate_storage_growth(self, organization_id: str, days: int) -> float:
        ...
        try:
            ...
            current_usage = self.db.query(func.sum(Document.file_size_bytes)).filter(
                Document.organization_id == organization_id
            ).scalar() or 0

            # Estimate growth based on recent uploads
            recent_uploads = self.db.query(func.sum(Document.file_size_bytes)).filter(
                and_(
                    Document.organization_id == organization_id,
    ...
    def validate_organization_limits(self, organization_id: str) -> Dict[str, Any]:
        ...
        document_count = self._get_document_storage_stats(organization_id)['document_count']
        ...
        validation_result = {
            ...
            'limits_status': {
                'storage': {
                    ...
                    'status': 'ok' if organization.storage_percentage_used < 90 else 'warning'
                },
                'users': {
                    'used': user_count,
                    'limit': limits.get('max_users', 'unlimited'),
                    'status': 'ok' if limits.get('max_users') is None or user_count < limits.get('max_users') else 'exceeded'
                },
                'documents': {
                    'used': document_count,
                    'limit': limits.get('max_documents', 'unlimited'),
                    'status': 'ok' if limits.get('max_documents') is None or document_count < limits.get('max_documents') else 'exceeded'
                }
            },
            'overall_status': 'ok'
        }
        ...
        for limit_type, limit_info in validation_result['limits_status'].items():
            ...
            elif limit_info['status'] == 'warning' and validation_result['overall_status'] == 'ok':
...
def check_tenant_permission(required_permission: str, organization_id: str = None) -> bool:
    """Check if current user has permission for tenant operation"""
    from src.middleware.multi_tenancy import get_current_tenant_id, get_current_user_role
    ...
    role_permissions = {
        'admin': [
            ...
            'organization_analytics', 'tenant_access', 'update_access', 'delete_access'
        ],
        'content_manager': [

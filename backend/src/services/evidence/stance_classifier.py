...
{{"stance": "supporting|opposing|neutral|not_addressed", "confidence": 0.85, "justification_excerpt": "relevant text excerpt"}}"""
    ...
    async def _classify_with_fallback(
        ...
        if result and result.confidence < 0.85:
            ...
            if fallback_result and fallback_result.confidence > result.confidence:
                logger.info(
                    f"Fallback model improved confidence: {result.confidence:.2f} -> {fallback_result.confidence:.2f}"
                )
    ...
    async def classify_stance(
        ...
        if self.cache_service:
            cached_result = await self.cache_service.get_stance_classification(
                cache_key
            )
            if cached_result:
        ...
        try:
            ...
            logger.info(
                f"Classified source {source_id}: {result.stance} (confidence: {result.confidence:.2f})"
            )
            return result_dict
    ...
    async def classify_sources_batch(
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.error(
                    f"Classification failed for source {sources[i]['source_id']}: {result}"
                )
        ...
        logger.info(
            f"Batch classification completed: {success_count}/{len(sources)} successful"
        )

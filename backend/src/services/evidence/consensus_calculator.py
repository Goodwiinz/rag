...
class ConsensusCalculator:
    ...
    def analyze_consensus_trends(self, classifications: List[Dict]) -> Dict:
        ...
        return {
            ...
            "confidence_distribution": {
                "high_confidence_pct": (
                    (confidence_ranges["high"] / total * 100) if total > 0 else 0
                ),
                "medium_confidence_pct": (
                    (confidence_ranges["medium"] / total * 100) if total > 0 else 0
                ),
                "low_confidence_pct": (
                    (confidence_ranges["low"] / total * 100) if total > 0 else 0
                ),
            },
            "stance_avg_confidence": stance_avg_confidence,
            "quality_metrics": {
                "high_confidence_count": confidence_ranges["high"],
                "needs_review": confidence_ranges[
                    "low"
    ...
    def format_consensus_description(self, meter: EvidenceMeter) -> str:
        ...
        if meter.consensus_level == ConsensusLevel.STRONG_AGREEMENT:
            ...
            return (
                f"Mixed evidence ({meter.supporting} support, {meter.opposing} oppose)"
            )
        else:  # LOW_AGREEMENT

...
class AzureOpenAIService:
    ...
    def _initialize_clients(self):
        ...
        try:
            ...
            embedding_endpoint = (
                settings.AZURE_OPENAI_EMBEDDING_ENDPOINT or
                settings.AZURE_OPENAI_ENDPOINT
            )
            embedding_api_key = (
                settings.AZURE_OPENAI_EMBEDDING_API_KEY or
                settings.AZURE_OPENAI_API_KEY
            )
            ...
            if embedding_endpoint and embedding_api_key:
                ...
                logger.info(f"Embedding client initialized with endpoint: {embedding_endpoint}")
            ...
            if settings.AZURE_OPENAI_ENDPOINT and settings.AZURE_OPENAI_API_KEY:
                ...
                logger.info(f"General client initialized with endpoint: {settings.AZURE_OPENAI_ENDPOINT}")
    ...
    def is_available(self) -> bool:
        ...
        return self.client is not None or self.embedding_client is not None or self.chat_client is not None
    ...
    def get_embedding_deployment(self) -> Optional[str]:
        ...
        return settings.AZURE_OPENAI_EMBEDDING_DEPLOYMENT_NAME or "text-embedding-ada-002"

    def get_chat_deployment(self) -> Optional[str]:
        """Get the chat deployment name"""
        return settings.AZURE_OPENAI_CHAT_DEPLOYMENT_NAME or settings.AZURE_OPENAI_DEPLOYMENT_NAME

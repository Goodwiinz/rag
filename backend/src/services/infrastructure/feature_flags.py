...
class FeatureFlagService:
    ...
    def _load_mock_flags(self):
        ...
        mock_config_path = os.path.join(
            ...
            "../../feature-flags/launchdarkly-config.json"
        )
                ...
                for flag_key, flag_config in config.get("flags", {}).items():
                    ...
                    if "variation" in default_rule:
                        ...
                        if variation_index < len(variations):
                            self._mock_flags[flag_key] = variations[variation_index]["value"]
        except Exception as e:
    ...
    def is_enabled(self, flag: FeatureFlag, user_context: Optional[UserContext] = None, default_value: bool = False) -> bool:
        """
        ...
        if self._initialized and self._client:
            try:
                ld_user = self._create_ld_user(user_context) if user_context else ld.User("anonymous")
    ...
    def get_variation(self, flag: FeatureFlag, user_context: Optional[UserContext] = None, default_value: Any = None) -> Any:
        """
    ...
    def _create_ld_user(self, user_context: UserContext) -> Any:  # Returns ld.User when available
    ...
    def get_all_flags(self, user_context: Optional[UserContext] = None) -> Dict[str, Any]:
        """
        ...
        if self._initialized and self._client:
            try:
                ...
                return {flag_key: flag_value.value for flag_key, flag_value in all_flags.to_values_map().items()}
            except Exception as e:
    ...
    def track_event(self, event_name: str, user_context: Optional[UserContext] = None, data: Optional[Dict[str, Any]] = None):
        """
...
def is_multimodal_processing_enabled(user_context: Optional[UserContext] = None) -> bool:
    """Check if multimodal processing is enabled"""
    return feature_flag_service.is_enabled(FeatureFlag.MULTIMODAL_PROCESSING, user_context)
...
def is_real_time_processing_enabled(user_context: Optional[UserContext] = None) -> bool:
    ...
    return feature_flag_service.is_enabled(FeatureFlag.REAL_TIME_PROCESSING, user_context)

def get_file_upload_limit(user_context: Optional[UserContext] = None) -> int:
    """Get the file upload limit in MB"""
    return feature_flag_service.get_variation(FeatureFlag.FILE_UPLOAD_LIMITS, user_context, 100)

def get_search_result_count(user_context: Optional[UserContext] = None) -> int:
    """Get the search result count"""
    return feature_flag_service.get_variation(FeatureFlag.SEARCH_RESULT_COUNT, user_context, 10)

def get_ai_model_type(user_context: Optional[UserContext] = None) -> str:
    """Get the AI model type to use"""
    return feature_flag_service.get_variation(FeatureFlag.AI_MODEL_OPTIMIZATION, user_context, "standard")
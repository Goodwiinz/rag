...
from ..models.ab_testing import (
    Experiment, Variant, ExperimentAssignment, QueryRouting,
    ExperimentStatus, ExperimentType, TrafficSplitType
)
...
class ExperimentAssignmentService:
    ...
    async def assign_experiment(
        ...
        try:
            ...
            await self._create_assignment(
                experiment.id, variant.id, context, db
            )
    ...
    async def _find_eligible_experiments(
        ...
        try:
            ...
            for experiment in experiments:
                ...
                current_traffic = await self._get_current_traffic_percentage(experiment, db)
                if current_traffic >= experiment.traffic_percentage:
                    continue  # Traffic limit reached

                # Check targeting criteria
                if not self._meets_targeting_criteria(experiment, context, search_query):
                    continue
    ...
    async def _select_variant(
        ...
        try:
            ...
            if experiment.traffic_split_type == TrafficSplitType.UNIFORM:
                ...
                return self._select_gradual_rollout_variant(variants, experiment, context)
    ...
    def _select_gradual_rollout_variant(
        ...
        rollout_percentage = min(elapsed_time.total_seconds() / total_duration.total_seconds(), 1.0)
    ...
    def _select_bandit_variant(
        ...
        for variant in variants:
            if state['counts'][variant.id] == 0:
                ...
                confidence = np.sqrt(2 * np.log(total_pulls) / state['counts'][variant.id])
    ...
    def _get_compatible_experiment_types(self, search_query: SearchQuery) -> List[ExperimentType]:
        ...
        if search_query.search_type.value == "hybrid":
            compatible_types.extend([
                ExperimentType.FILTER_CONFIGURATION,
                ExperimentType.RESPONSE_FORMAT
            ])
    ...
    async def _get_current_traffic_percentage(
        ...
        try:
            ...
            assignment_count = db.query(ExperimentAssignment).filter(
                and_(
                    ExperimentAssignment.experiment_id == experiment.id,
        ...
        try:
            if self.redis_client:
                ...
                if cache_key in self._assignment_cache:
                    ...
                    if datetime.now(timezone.utc) - cached_entry['timestamp'] < timedelta(seconds=self._cache_ttl):
    ...
    async def update_bandit_rewards(
        ...
        try:
            ...
            if experiment_key in self._bandit_state:
                ...
                state['values'][variant_key] = (
                    (old_value * old_count + reward) / (old_count + 1)
                )
    ...
    async def cleanup_cache(self):
        ...
        try:
            if not self.redis_client:
                ...
                expired_keys = [
                    key for key, entry in self._assignment_cache.items()

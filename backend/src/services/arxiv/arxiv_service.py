...
from src.shared.exceptions import ValidationError, ProcessingError
...
class ArXivIngestionService:
    ...
    async def search_papers(
        ...
        """
        if not self.session:
            logger.error("Session not initialized in search_papers. Session object is None")
            ...
            try:
                ...
                for entry in root.findall('atom:entry', namespaces):
                    ...
                    if date_from or date_to:
                        pub_date = datetime.fromisoformat(paper_data['published'].replace('Z', '+00:00'))
    ...
    async def _ingest_single_paper(
        ...
        metadata_dict = {
            'title': paper['title'],
            'description': paper['abstract'][:500] + "..." if len(paper['abstract']) > 500 else paper['abstract'],
            'authors': paper['authors'],
            'publication_date': datetime.fromisoformat(paper['published'].replace('Z', '+00:00')),
        ...
        if download_pdfs and paper['links'].get('pdf'):
            try:
                pdf_content = await self.download_paper_pdf(paper_id, paper['links']['pdf'])
                ...
                if extract_content and pdf_content:
                    ...
                    metadata_dict['pdf_path'] = str(self.download_dir / f"{paper_id}.pdf")
        ...
        document_data = {
            ...
            'document_type': 'PDF' if download_pdfs and paper['links'].get('pdf') else 'TEXT',
            'filename': f"{paper_id}.pdf",
            'mime_type': 'application/pdf' if download_pdfs and paper['links'].get('pdf') else 'text/plain',
            'file_size_bytes': len(pdf_content) if 'pdf_content' in locals() and pdf_content else 0
        }
    ...
    async def create_evaluation_dataset(
        ...
        logger.info(f"Created evaluation dataset with {len(evaluation_data['test_cases'])} test cases")
    ...
    def get_category_statistics(self, papers: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for paper in papers:
            ...
            for group, patterns in self.CATEGORY_GROUPS.items():
                if any(cat.split('.')[0] in pattern.replace('*', '') for pattern in patterns for cat in paper['categories']):

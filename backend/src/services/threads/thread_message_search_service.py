...
class ThreadMessageSearchService:
        ...
        try:
            ...
            for row in rows:
                search_result = ThreadSearchResult(
                    ...
                    highlighted_title=row.highlighted_title if hasattr(row, 'highlighted_title') else None,
                    highlighted_summary=row.highlighted_summary if hasattr(row, 'highlighted_summary') else None,
                    matching_message_count=row.matching_message_count if hasattr(row, 'matching_message_count') else None
                )
        ...
        try:
            ...
            for row in rows:
                search_result = MessageSearchResult(
                    ...
                    relevance_score=float(row.relevance_score) if row.relevance_score else 0.0,
                    created_at=row.created_at,
                    highlighted_content=row.highlighted_content if hasattr(row, 'highlighted_content') else None,
                    thread_title=row.thread_title if hasattr(row, 'thread_title') else None,
                    conversation_id=row.conversation_id if hasattr(row, 'conversation_id') else None,
                    citation_count=row.citation_count if hasattr(row, 'citation_count') else 0
                )
                search_results.append(search_result)
    ...
    def combined_search(
        ...
        try:
            ...
            search_results = []
            for row in rows:
                search_results.append(CombinedSearchResult(
        ...
        if request.filters:
            ...
            if request.filters.has_citations is not None:
                if request.filters.has_citations:
                    query_parts.append("    AND EXISTS (SELECT 1 FROM citations cit WHERE cit.message_id = m.id)")
                else:
                    query_parts.append("    AND NOT EXISTS (SELECT 1 FROM citations cit WHERE cit.message_id = m.id)")
    ...
    def _serialize_thread_filters(self, filters: Optional[ThreadSearchFilter]) -> Optional[Dict[str, Any]]:
        """Serialize thread filters for response"""
        if not filters:
            return None
        
        return {
            'conversation_id': str(filters.conversation_id) if filters.conversation_id else None,
            ...
            'created_by_id': str(filters.created_by_id) if filters.created_by_id else None,
            'date_from': filters.date_from.isoformat() if filters.date_from else None,
            'date_to': filters.date_to.isoformat() if filters.date_to else None,
            'min_message_count': filters.min_message_count
        }

    def _serialize_message_filters(self, filters: Optional[MessageSearchFilter]) -> Optional[Dict[str, Any]]:

...
from ..models.websocket_status import UpdateType
...
class DocumentProcessingStages:
    ...
    INGESTION = [
        ProcessingStep("file_validation", "Validating file format and integrity", 1.0, 5.0),
    ...
    TEXT_EXTRACTION = [
        ...
        ProcessingStep("metadata_extraction", "Extracting document metadata", 1.0, 10.0)
    ]
    ...
    EMBEDDING_GENERATION = [
        ...
        ProcessingStep("embedding_generation", "Generating vector embeddings", 5.0, 30.0),
        ProcessingStep("vector_storage", "Storing embeddings in vector store", 1.0, 5.0)
    ]

    GRAPH_INDEXING = [
        ProcessingStep("relationship_extraction", "Extracting relationships", 3.0, 20.0),
    ...
    def get_all_steps(cls, document_type: DocumentType) -> List[ProcessingStep]:
        ...
        if document_type == DocumentType.IMAGE:
            all_steps.extend([
                ProcessingStep("image_analysis", "Analyzing image content", 4.0, 25.0),
                ProcessingStep("object_detection", "Detecting objects", 3.0, 20.0),
                ProcessingStep("caption_generation", "Generating image captions", 2.0, 15.0)
            ])

        # Audio/video need transcription
        if document_type in [DocumentType.AUDIO, DocumentType.VIDEO]:
            all_steps.extend([
                ProcessingStep("audio_extraction", "Extracting audio from video", 2.0, 10.0),
                ProcessingStep("transcription", "Transcribing audio content", 6.0, 35.0),
                ProcessingStep("speaker_identification", "Identifying speakers", 2.0, 15.0)
            ])
...
class ProcessingIntegrationService(BaseService):
    ...
    @asynccontextmanager
    async def processing_context(self, document_id: str, job_id: str = None,
        ...
        try:
            ...
            async with get_async_session() as session:
                result = await session.execute(
                    select(Document).options(selectinload(Document.organization))
            ...
            processing_steps = DocumentProcessingStages.get_all_steps(document.document_type)
            ...
            # Broadcast failure
            await self._broadcast_processing_error(document_id, str(e), context if 'context' in locals() else None)
    ...
    async def update_step_progress(self, job_id: str, step_name: str,
                                 progress_percentage: float, details: Dict[str, Any] = None):
        ...
        overall_progress = ((context["completed_weight"] + step_progress) / context["total_weight"]) * 100.0
        ...
        progress_data = {
            ...
            "current_operation": details.get("operation", step_name) if details else step_name,
            "estimated_remaining_seconds": self._calculate_remaining_time(context, progress_percentage),
            "details": details or {}
        }
        ...
        if context["job_id"]:
            await self._update_job_progress(context["job_id"], current_step.name, overall_progress)

    async def complete_step(self, job_id: str, step_name: str, result: Dict[str, Any] = None):
        ...
        overall_progress = (context["completed_weight"] / context["total_weight"]) * 100.0
        ...
        # Update job if available
        if context["job_id"]:
            await self._update_job_progress(context["job_id"], "Completed", overall_progress)

    async def add_processing_warning(self, job_id: str, warning: str, details: Dict[str, Any] = None):
    ...
    async def complete_processing(self, document_id: str, result: Dict[str, Any] = None):
    ...
    async def handle_document_upload(self, document_id: str, user_id: str, organization_id: str):
        """Handle new document upload with initial status updates"""
        try:
            # Get document information
            async with get_async_session() as session:
                result = await session.execute(
                    select(Document).options(selectinload(Document.uploaded_by_user))
                    .where(Document.id == document_id)
                )
                document = result.scalar_one_or_none()
            ...
            job = ProcessingJob(
                ...
                total_steps=len(DocumentProcessingStages.get_all_steps(document.document_type))
            )
    ...
    async def _update_document_status(self, document_id: str, status: ProcessingStatus, error: str = None):
            ...
            async with get_async_session() as session:
                await session.execute(
                    ...
                    .values(
                        ...
                        processing_completed_at=datetime.now(dt_timezone.utc) if status in [ProcessingStatus.COMPLETED, ProcessingStatus.FAILED] else None
                    )
                )
                await session.commit()
    ...
    async def _update_job_progress(self, job_id: str, current_step: str, progress_percentage: float):
        """Update job progress"""
        try:
            async with get_async_session() as session:
                result = await session.execute(
    ...
    async def _broadcast_processing_start(self, document_id: str, context: Dict[str, Any]):
        ...
        await status_update_service.broadcast_document_update(
            ...
            progress=ProcessingProgress(
                ...
                estimated_remaining_seconds=sum(step.estimated_duration_seconds for step in context["processing_steps"]),
    ...
    async def _broadcast_processing_progress(self, document_id: str, progress_data: Dict[str, Any]):
    ...
    async def _broadcast_step_completion(self, document_id: str, completion_data: Dict[str, Any]):
    ...
    async def _broadcast_processing_warning(self, document_id: str, warning: str, details: Dict[str, Any]):
    ...
    async def _broadcast_processing_error(self, document_id: str, error: str, context: Dict[str, Any]):
    ...
    async def _broadcast_processing_completion(self, document_id: str, result: Dict[str, Any]):
    ...
    def _calculate_remaining_time(self, context: Dict[str, Any], current_step_progress: float) -> float:
        ...
        try:
            ...
            if current_step_index < len(processing_steps):
                ...
                remaining_step_weight = current_step_weight * (1 - current_step_progress / 100.0)
            else:
                remaining_step_weight = 0

            # Weight of future steps
            future_weight = sum(step.weight for step in processing_steps[current_step_index + 1:])
            ...
            if completed_weight > 0:
                ...
                if current_step_index < len(processing_steps):
                    current_step = processing_steps[current_step_index]
                    estimated_remaining += current_step.estimated_duration_seconds * (1 - current_step_progress / 100.0)

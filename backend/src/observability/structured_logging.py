...
class StructuredLogger:
    ...
    def __init__(self, log_level: LogLevel = LogLevel.INFO,
        ...
        # Metrics for logging
        self.log_counter = Counter('structured_logs_total', 'Total structured logs',
                                  ['level', 'category', 'service'])
        self.log_duration = Histogram('log_processing_duration_seconds',
    ...
    def _add_correlation_context(self, logger, method_name: str, event_dict: Dict[str, Any]) -> Dict[str, Any]:
        """Add correlation context to log event"""
        context = self.get_current_context()
        event_dict.update(context.to_dict())
        return event_dict

    def _add_opentelemetry_context(self, logger, method_name: str, event_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if error:
                ...
                if hasattr(error, '__traceback__'):
                    import traceback
                    entry.stack_trace = traceback.format_exception(type(error), error, error.__traceback__)
            ...
            log_data = {
                "timestamp": datetime.fromtimestamp(entry.timestamp, dt_timezone.utc).isoformat(),
            ...
            logging.error(f"Structured logging failed: {e}. Original message: {message}")
    ...
    def log_api_request(self, method: str, path: str, status_code: int,
        ...
        self.info(
            ...
            performance_metrics={
                ...
                "throughput_requests_per_second": 1 / (duration_ms / 1000) if duration_ms > 0 else 0
            },
    ...
    def log_document_processing(self, document_id: str, stage: str, status: str,
                               duration_ms: Optional[float] = None, file_size: Optional[int] = None,
                               error: Optional[Exception] = None):
    ...
    def log_websocket_event(self, connection_id: str, event_type: str,
                           user_id: Optional[str] = None, latency_ms: Optional[float] = None,
                           message_size: Optional[int] = None, error: Optional[Exception] = None):
        """Log WebSocket event"""
        level = LogLevel.INFO if error is None else LogLevel.ERROR
    ...
    def log_cache_operation(self, operation: str, key: str, hit: bool,
                           duration_ms: float, value_size: Optional[int] = None):
        """Log cache operation"""
        self.info(
            LogCategory.CACHE,
    ...
    def log_search_query(self, query: str, results_count: int, duration_ms: float,
                        search_type: str, user_id: Optional[str] = None):
        """Log search query"""
        self.info(
            LogCategory.SEARCH,
            ...
            performance_metrics={
                "duration_ms": duration_ms,
                "results_per_second": results_count / (duration_ms / 1000) if duration_ms > 0 else 0
            },
            user_id=user_id,
            tags=[f"type_{search_type}"]
        )

    def log_ml_inference(self, model_name: str, input_size: int, duration_ms: float,
                        confidence: Optional[float] = None, error: Optional[Exception] = None):
        """Log ML inference"""
        level = LogLevel.INFO if error is None else LogLevel.ERROR
    ...
    def log_security_event(self, event_type: str, user_id: Optional[str] = None,
                          ...
                          severity: str = "medium", blocked: bool = False):
        """Log security event"""
        level = LogLevel.WARNING if severity == "low" else LogLevel.ERROR if severity == "medium" else LogLevel.CRITICAL
...
def log_api_request(method: str, path: str, status_code: int, duration_ms: float,
    ...
    structured_logger.log_api_request(
        method, path, status_code, duration_ms, request_id, user_id, request_size, response_size
    )

def log_document_processing(document_id: str, stage: str, status: str,
                           duration_ms: Optional[float] = None, file_size: Optional[int] = None,
                           error: Optional[Exception] = None):
...
def log_websocket_event(connection_id: str, event_type: str,
                       user_id: Optional[str] = None, latency_ms: Optional[float] = None,
                       message_size: Optional[int] = None, error: Optional[Exception] = None):
    """Log WebSocket event"""
...
def log_database_operation(operation: str, table: str, duration_ms: float,
                          rows_affected: Optional[int] = None, error: Optional[Exception] = None):
    """Log database operation"""
    structured_logger.log_database_operation(operation, table, duration_ms, rows_affected, error)

def log_cache_operation(operation: str, key: str, hit: bool, duration_ms: float,
                       value_size: Optional[int] = None):
    """Log cache operation"""
    structured_logger.log_cache_operation(operation, key, hit, duration_ms, value_size)

def log_search_query(query: str, results_count: int, duration_ms: float,
                    search_type: str, user_id: Optional[str] = None):
    """Log search query"""
    structured_logger.log_search_query(query, results_count, duration_ms, search_type, user_id)

def log_ml_inference(model_name: str, input_size: int, duration_ms: float,
                    confidence: Optional[float] = None, error: Optional[Exception] = None):
    """Log ML inference"""
    structured_logger.log_ml_inference(model_name, input_size, duration_ms, confidence, error)

def log_security_event(event_type: str, user_id: Optional[str] = None,
                      ip_address: Optional[str] = None, user_agent: Optional[str] = None,
                      severity: str = "medium", blocked: bool = False):
    """Log security event"""
    structured_logger.log_security_event(event_type, user_id, ip_address, user_agent, severity, blocked)
...
from .metrics import increment_counter, record_histogram
...
class PerformanceTestRunner:
    ...
    async def _execute_load_test(
        self, test_config: LoadTestConfig, system_monitor: "SystemMetricsMonitor"
    ) -> Dict[str, Any]:
        ...
        total_requests = (
            test_config.requests_per_second * test_config.test_duration_seconds
        )
        ...
        for user_id in range(test_config.concurrent_users):
            for request_id in range(requests_per_user):
                ...
                delay = (request_id / test_config.requests_per_second) + (
                    user_id * 0.01
                )
    ...
    async def _make_request(
        self, test_config: LoadTestConfig, is_warmup: bool = False
    ) -> Dict[str, Any]:
        ...
        for users in range(user_step, max_users + 1, user_step):
            ...
            try:
                ...
                if result.error_rate > 0.1:  # 10% error rate threshold
                    logger.warning(
                        f"Stopping stress test due to high error rate: {result.error_rate:.2%}"
                    )
                    break
    ...
    def generate_performance_report(
        ...
        avg_response_time = statistics.mean(
            [r.average_response_time for r in test_results]
        )
        avg_p95_response_time = statistics.mean(
            [r.p95_response_time for r in test_results]
        )
...
class SystemMetricsMonitor:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "disk": {
                "average_usage_percent": statistics.mean(
...
class RAGPerformanceTestSuite:
    ...
    async def test_search_performance(
        self, query: str, concurrent_users: int = 50
    ) -> PerformanceTestResult:
    ...
    async def test_document_ingestion(
        self, file_size_mb: int = 1, concurrent_uploads: int = 10
    ) -> PerformanceTestResult:
    ...
    async def test_rag_quality_performance(

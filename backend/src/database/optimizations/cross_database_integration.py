...
from ..qdrant_optimization import QdrantOptimizer, QdrantOptimizationConfig
...
class CrossDatabaseMetrics:
    ...
    def health_percentage(self) -> float:
        return (self.healthy_databases / self.total_databases) * 100 if self.total_databases > 0 else 0
...
class CrossDatabaseIntegration:
    ...
    async def apply_production_optimizations(self) -> Dict[str, Any]:
        ...
        optimization_tasks = {
            DatabaseType.POSTGRESQL: self.optimizers[DatabaseType.POSTGRESQL].apply_production_optimizations(),
            DatabaseType.NEO4J: self.optimizers[DatabaseType.NEO4J].apply_production_optimizations(),
            DatabaseType.REDIS: self.optimizers[DatabaseType.REDIS].apply_production_optimizations()
        }
        ...
        for collection_name in collections:
            try:
                ...
                if not collection_exists:
                    ...
                    success = qdrant_optimizer.create_optimized_collection(collection_name)
                    if success:
                        # Create payload indexes
                        qdrant_optimizer.create_payload_indexes(collection_name)
                        optimization_results.append(f"Created and optimized collection: {collection_name}")
                else:
                    # Optimize existing collection
                    qdrant_optimizer.optimize_collection(collection_name)
                    optimization_results.append(f"Optimized existing collection: {collection_name}")

            except Exception as e:
                logger.warning(f"Failed to optimize Qdrant collection {collection_name}: {e}")
    ...
    async def comprehensive_health_check(self) -> Dict[str, Any]:
        ...
        if DatabaseType.POSTGRESQL in self.optimizers:
            try:
                ...
                pg_report = await self.optimizers[DatabaseType.POSTGRESQL].get_optimization_report()
        ...
        if DatabaseType.NEO4J in self.optimizers:
            try:
                start_time = time.time()
                neo4j_stats = await self.optimizers[DatabaseType.NEO4J].get_graph_statistics()
                response_time = (time.time() - start_time) * 1000
        ...
        if DatabaseType.QDRANT in self.optimizers:
            try:
                start_time = time.time()
                qdrant_stats = self.optimizers[DatabaseType.QDRANT].get_performance_stats()
                response_time = (time.time() - start_time) * 1000
        ...
        if DatabaseType.REDIS in self.optimizers:
            try:
                start_time = time.time()
                redis_metrics = await self.optimizers[DatabaseType.REDIS].get_redis_metrics()
                response_time = (time.time() - start_time) * 1000
        ...
        avg_response_time = total_response_time / total_databases if total_databases > 0 else 0
        ...
        return {
            ...
            'overall_health': {
                'status': 'healthy' if error_count == 0 else 'warning' if error_count < 2 else 'error',
                'healthy_databases': healthy_count,
                'total_databases': total_databases,
                'health_percentage': cross_metrics.health_percentage
            },
            'database_health': {db_type.value: asdict(health) for db_type, health in health_results.items()},
            'cross_metrics': asdict(cross_metrics),
            'alert_thresholds': self.alert_thresholds
        }

    async def sync_monitoring_data(self, source_db: DatabaseType, target_db: DatabaseType, data_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            transformed_data = await self._transform_monitoring_data(source_data, data_type, target_db)

            # Load data to target
            load_result = await self._load_monitoring_data(target_db, transformed_data, data_type)

            execution_time = (time.time() - start_time) * 1000

            if load_result['success']:
                self.sync_status[sync_key] = SyncStatus.SYNCED
                logger.info(f"Successfully synced {data_type} from {source_db.value} to {target_db.value}")
            else:
                self.sync_status[sync_key] = SyncStatus.ERROR
                logger.error(f"Failed to sync {data_type} from {source_db.value} to {target_db.value}")
            ...
            return {
                ...
                'data_type': data_type,
                'records_processed': len(source_data) if isinstance(source_data, list) else 1,
    ...
    async def _extract_monitoring_data(self, db_type: DatabaseType, data_type: str) -> Any:
    ...
    async def _transform_monitoring_data(self, source_data: Any, data_type: str, target_db: DatabaseType) -> Any:
        ...
        if target_db == DatabaseType.QDRANT:
            ...
            if data_type == 'metrics':
                ...
                for record in source_data:
                    vector = [record.get('value', 0.0), record.get('timestamp', 0), record.get('definition_id', 0)]
                    transformed.append({
    ...
    async def _load_monitoring_data(self, db_type: DatabaseType, data: Any, data_type: str) -> Dict[str, Any]:
    ...
    async def cleanup_all_databases(self, days_old: int = 30) -> Dict[str, Any]:
        ...
        if DatabaseType.POSTGRESQL in self.optimizers:
            try:
                ...
                cleanup_results[DatabaseType.POSTGRESQL] = {'success': True, 'message': 'Cleanup scheduled'}
            except Exception as e:
                cleanup_results[DatabaseType.POSTGRESQL] = {'success': False, 'error': str(e)}

        # Neo4j cleanup
        if DatabaseType.NEO4J in self.optimizers:
            try:
                neo4j_result = await self.optimizers[DatabaseType.NEO4J].cleanup_old_data(days_old)
                cleanup_results[DatabaseType.NEO4J] = neo4j_result
            except Exception as e:
                cleanup_results[DatabaseType.NEO4J] = {'success': False, 'error': str(e)}

        # Redis cleanup
        if DatabaseType.REDIS in self.optimizers:
            try:
                redis_result = await self.optimizers[DatabaseType.REDIS].cleanup_expired_cache()
                cleanup_results[DatabaseType.REDIS] = redis_result
            except Exception as e:
                cleanup_results[DatabaseType.REDIS] = {'success': False, 'error': str(e)}

        # Qdrant cleanup
        if DatabaseType.QDRANT in self.optimizers:
            try:
                optimizer = self.optimizers[DatabaseType.QDRANT]
                for collection_name in ['monitoring_metrics', 'monitoring_traces', 'monitoring_logs']:
                    optimizer.cleanup_old_data(collection_name, days_old)
                cleanup_results[DatabaseType.QDRANT] = {'success': True, 'message': 'Vector cleanup completed'}
            except Exception as e:
                cleanup_results[DatabaseType.QDRANT] = {'success': False, 'error': str(e)}

        return {
            'timestamp': datetime.utcnow().isoformat(),
            'cleanup_results': {db_type.value: result for db_type, result in cleanup_results.items()},
    ...
    async def get_comprehensive_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            'timestamp': datetime.utcnow().isoformat(),
            'individual_metrics': {},
            'cross_database_metrics': asdict(self.metrics_history[-1]) if self.metrics_history else None,
        ...
        for db_type, optimizer in self.optimizers.items():
            try:
                if db_type == DatabaseType.POSTGRESQL:
                    metrics['individual_metrics'][db_type.value] = await optimizer.get_optimization_report()
                elif db_type == DatabaseType.NEO4J:
                    metrics['individual_metrics'][db_type.value] = await optimizer.get_graph_statistics()
                elif db_type == DatabaseType.REDIS:
                    metrics['individual_metrics'][db_type.value] = await optimizer.get_redis_metrics()
                elif db_type == DatabaseType.QDRANT:
                    metrics['individual_metrics'][db_type.value] = optimizer.get_performance_stats()
            except Exception as e:
    ...
    def _calculate_performance_trends(self) -> Dict[str, Any]:
        ...
        return {
            'response_time_trend': {
                ...
                'trend': 'improving' if response_times[-1] < response_times[-2] else 'degrading'
            },
            'health_trend': {
                'current': health_percentages[-1],
                'average': sum(health_percentages) / len(health_percentages),
                'trend': 'improving' if health_percentages[-1] > health_percentages[-2] else 'degrading'
            },
            'data_points': len(recent_metrics),
            'time_range_hours': (recent_metrics[-1].timestamp - recent_metrics[0].timestamp).total_seconds() / 3600
        }

...
class BroadcastRequest(BaseModel):
    ...
    channels: Optional[List[str]] = Field(
        None, description="Specific channels to target"
    )
...
async def get_websocket_stats(
    ...
    try:
        ...
        return WebSocketStats(
            total_connections=metrics.get("websocket_metrics", {}).get(
                "total_connections", 0
            ),
            active_connections=metrics.get("websocket_metrics", {}).get(
                "active_connections", 0
            ),
            user_connections=len(
                metrics.get("subscription_metrics", {}).get("total_subscribers", 0)
            ),
            organization_connections=metrics.get("subscription_metrics", {}).get(
                "active_subscriptions", 0
            ),
            connections_by_status={
                "connected": metrics.get("websocket_metrics", {}).get(
                    "active_connections", 0
                ),
                "disconnected": 0,
            },
            messages_sent=metrics.get("websocket_metrics", {}).get("messages_sent", 0),
            messages_received=metrics.get("websocket_metrics", {}).get(
                "messages_received", 0
            ),
...
async def get_active_connections(
    ...
    try:
        ...
        if user_id:
            ...
            connection_ids = await redis_manager.get_organization_connections(
                organization_id
            )
        else:
...
async def disconnect_connection(
    connection_id: str = Path(..., description="WebSocket connection ID"),
    reason: str = Query(
        default="Admin disconnect", description="Reason for disconnection"
    ),
    credentials: HTTPAuthorizationCredentials = Depends(security),
):
    """
...
@router.get(
    "/organizations/{organization_id}/connections", response_model=List[ConnectionInfo]
)
    ...
    try:
        ...
        if broadcast.target_type == "all":
            ...
            await redis_manager.publish(
                f"ws:broadcast:user:{broadcast.target_id}", message
            )
        elif broadcast.target_type == "organization" and broadcast.target_id:
            await redis_manager.publish(
                f"ws:broadcast:org:{broadcast.target_id}", message
            )
        else:
            raise HTTPException(
...
async def get_detailed_metrics(
    ...
    try:
        ...
        return ConnectionMetrics(
            ...
            redis_connections=metrics.get("redis_metrics", {}).get(
                "connected_clients", 0
            ),

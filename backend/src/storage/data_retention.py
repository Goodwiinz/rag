...
class DataRetentionManager:
    ...
    async def run_retention_cleanup(
        ...
        for policy in self.policies:
            ...
            try:
                ...
                if not dry_run:
                    logger.info(
                        f"Completed retention cleanup for {policy.data_type}: {report.records_deleted} deleted, {report.records_archived} archived, {report.records_anonymized} anonymized"
                    )

            except Exception as e:
                error_msg = (
                    f"Failed to execute retention policy for {policy.data_type}: {e}"
                )
    ...
    async def _execute_retention_policy(
        ...
        try:
            if policy.action == RetentionAction.DELETE:
                ...
                await self._execute_anonymize_action(
                    policy, cutoff_date, report, dry_run
                )
            elif policy.action == RetentionAction.COMPRESS:
                await self._execute_compress_action(
                    policy, cutoff_date, report, dry_run
                )

        except Exception as e:
            error_msg = (
                f"Error executing {policy.action} action for {policy.data_type}: {e}"
            )
            logger.error(error_msg)
        ...
        try:
            ...
            if policy.data_type == "analytics_events":
                ...
                count_query = (
                    ...
                    .where(target_table.c.created_at < cutoff_date)
                )
                ...
                if not dry_run and report.records_processed > 0:
                    # Delete records
                    target_table = table(policy.table_name, column("created_at"))
                    delete_query = delete(target_table).where(
            ...
            if not dry_run:
                ts_deleted = await self.time_series_manager.cleanup_old_data(
    ...
    async def _execute_archive_action(
        self,
        policy: RetentionPolicy,
        cutoff_date: datetime,
        report: RetentionReport,
    ...
    async def _execute_anonymize_action(
        self,
        policy: RetentionPolicy,
        cutoff_date: datetime,
        report: RetentionReport,
            ...
            if policy.data_type == "user_sessions_pii" and policy.anonymize_fields:
                ...
                if not dry_run and report.records_processed > 0:
                    ...
                    target_table = table(
                        ...
                        column("id"),
                        column("created_at"),
                        *[column(c) for c in policy.anonymize_fields],
                    )
                    select_query = select(target_table).where(
                        target_table.c.created_at < cutoff_date
                    )
                    ...
                    for record in records:
                        ...
                        update_query = (
                            update(target_table)
                            .where(target_table.c.id == update_data["id"])
                            .values(
    ...
    async def _execute_compress_action(
        self,
        policy: RetentionPolicy,
        cutoff_date: datetime,
        report: RetentionReport,
    ...
    async def get_retention_status(self) -> Dict[str, Any]:
                        ...
                        func.count(
                            case(
                                (
                                    ...
                                    < text(
                                        f"NOW() - INTERVAL '{policy.retention_days} days'"
                                    ),
                                    1,
                                )
                            )
        ...
        if self.config.retention.cleanup_interval_hours:
            next_cleanup = datetime.utcnow() + timedelta(
                hours=self.config.retention.cleanup_interval_hours
            )
    ...
    async def get_compliance_report(
        ...
        for policy in self.policies:
            ...
            try:
                ...
                violation_check_query = (
                    ...
                    .where(
                        target_table.c.created_at
                        < text(f"NOW() - INTERVAL '{policy.retention_days} days'")
                    )
                )
        ...
        if report["policy_violations"]:
            report["recommendations"].append(
                "Run immediate retention cleanup to resolve policy violations"
            )
            report["recommendations"].append(
                "Review retention policies and adjust retention periods if needed"
            )
            report["recommendations"].append(
                "Consider automating retention cleanup with scheduled jobs"
            )

        total_violations = sum(
            v["violation_count"] for v in report["policy_violations"]
        )
    ...
    async def cleanup_expired_data(
        self, data_type: Optional[str] = None, force: bool = False
    ) -> Dict[str, Any]:
...
async def scheduled_retention_cleanup():
    ...
    try:
        ...
        logger.info(
            f"Scheduled retention cleanup completed: {total_deleted} deleted, {total_archived} archived, {total_anonymized} anonymized"
        )
    ...
    try:
        ...
        if health["violations"] > 0:
            ...
            health["recommendations"].append(
                "Run retention cleanup to resolve policy violations"
            )
        ...
        cleanup_needed = any(
            p.get("cleanup_needed", False) for p in status.get("policies", [])
        )
        if cleanup_needed:
        ...
        health["error"] = str(e)
        health["recommendations"].append(
            "Check retention system configuration and database connectivity"
        )

...
class DatabasePerformanceMonitor:
    ...
    async def monitor_postgresql_performance(
        ...
        try:
            ...
            if db_stats['blks_read'] > 0:
                cache_hit_ratio = db_stats['blks_hit'] / (db_stats['blks_read'] + db_stats['blks_hit'])
            ...
            metrics = DatabaseMetrics(
                ...
                index_usage={row['index_name']: row['scan_count'] for row in index_usage},
                table_size={row['table_name']: int(row['size_bytes'].replace('MB', '').replace('kB', '')) * 1024 * 1024 if row['size_bytes'] else 0 for row in table_sizes}
            )
    ...
    async def monitor_neo4j_performance(
        ...
        try:
            ...
            with driver.session() as session:
                ...
                db_info = session.run("CALL dbms.components() YIELD name, versions RETURN name, versions[0] as version").single()

                # Get connection information
                connections = session.run("CALL dbms.listConnections() YIELD connectionId, connectTime, connector, username RETURN count(*) as total_connections").single()

                # Get query statistics
                query_stats = session.run("CALL dbms.queryJmx() YIELD attributes WHERE attributes.name = 'NumberOfOpenTransactions' RETURN attributes.value as open_transactions").single()

                # Get memory usage
                memory_usage = session.run("CALL dbms.queryJmx() YIELD attributes WHERE attributes.name = 'MemoryUsed' RETURN attributes.value as memory_used").single()

                # Get page cache hit ratio
                cache_stats = session.run("CALL dbms.queryJmx() YIELD attributes WHERE attributes.name = 'PageCacheHits' RETURN attributes.value as cache_hits").single()
                cache_misses = session.run("CALL dbms.queryJmx() YIELD attributes WHERE attributes.name = 'PageCacheMisses' RETURN attributes.value as cache_misses").single()
                ...
                metrics = DatabaseMetrics(
                    ...
                    timestamp=datetime.now(),
                    connection_count=connections['total_connections'] if connections else 0,
                    active_connections=query_stats['open_transactions'] if query_stats else 0,
    ...
    async def monitor_qdrant_performance(
        ...
        try:
            ...
            metrics = DatabaseMetrics(
                ...
                table_size={coll.name: total_points * 1000 for coll in collections}  # Estimated size
            )
    ...
    async def monitor_redis_performance(
        ...
        try:
            ...
            metrics = DatabaseMetrics(
                ...
                buffer_pool_hit_ratio=info.get('keyspace_hits', 0) / max(1, info.get('keyspace_hits', 0) + info.get('keyspace_misses', 0)),
                temp_tables_created=0,
                lock_waits=0,
                deadlocks=0,
                transaction_duration=0.0,
                cpu_usage=cpu_usage / 100.0,
                memory_usage=info.get('used_memory', 0) / max(1, info.get('maxmemory', 1)),
                disk_io_read=0,
                disk_io_write=0,
                network_io_bytes=info.get('total_net_input_bytes', 0) + info.get('total_net_output_bytes', 0),
    ...
    async def analyze_query_performance(
        ...
        try:
            ...
            recommendations = self._generate_query_recommendations(execution_plan, stats)
    ...
    def _generate_query_recommendations(
        ...
        if stats and stats['mean_exec_time'] > self.slow_query_threshold:
            recommendations.append("Query execution time is above threshold - consider optimization")

        # Check for high variance in execution time
        if stats and stats['stddev_exec_time'] > stats['mean_exec_time'] * 0.5:
            recommendations.append("High variance in execution time - check for parameter sniffing")

        # Check for missing indexes
        if 'Bitmap Heap Scan' in str(execution_plan):
            recommendations.append("Consider optimizing bitmap scan with better indexes")
        ...
        if 'Hash Join' in str(execution_plan):
            recommendations.append("Consider optimizing hash join with work_mem adjustment")
    ...
    async def get_performance_summary(
        ...
        filtered_metrics = [
            ...
            if m.timestamp > cutoff_time and (database_type is None or m.database_type == database_type)
        ]
        ...
        summary = {
            ...
            "performance": {
                "avg_query_time": statistics.mean([m.avg_query_time for m in filtered_metrics]),
                "max_query_time": max([m.max_query_time for m in filtered_metrics]),
                "avg_cache_hit_ratio": statistics.mean([m.cache_hit_ratio for m in filtered_metrics]),
                "avg_cpu_usage": statistics.mean([m.cpu_usage for m in filtered_metrics]),
                "avg_memory_usage": statistics.mean([m.memory_usage for m in filtered_metrics]),
                "total_slow_queries": sum([m.slow_queries for m in filtered_metrics]),
                "total_deadlocks": sum([m.deadlocks for m in filtered_metrics])
            },
            "connections": {
                "avg_connections": statistics.mean([m.connection_count for m in filtered_metrics]),
                "max_connections": max([m.connection_count for m in filtered_metrics]),
                "avg_active_connections": statistics.mean([m.active_connections for m in filtered_metrics])
            },
            "alerts": [],
            "recommendations": []
        }
        ...
        if summary["performance"]["avg_query_time"] > self.alert_thresholds['avg_query_time']:
        ...
        if summary["performance"]["avg_cache_hit_ratio"] < self.alert_thresholds['cache_hit_ratio']:
    ...
    async def _optimize_postgresql_config(self, connection_string: str) -> Dict[str, Any]:
        ...
        for setting in current_config:
            ...
            if optimal_value and current_value != optimal_value:
                recommendations.append({
                    ...
                    'impact': 'high' if name in ['shared_buffers', 'work_mem'] else 'medium'
                })
        ...
        return {
            ...
            'system_info': {
                ...
                'cpu_count': cpu_count,
                'current_config': {row['name']: row['setting'] for row in current_config}
            },

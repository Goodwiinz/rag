...
def require_permission(permission: AnalyticsPermission):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            if not AnalyticsPermissionsChecker.has_permission(
...
def require_any_permission(permissions: List[AnalyticsPermission]):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            if not AnalyticsPermissionsChecker.has_any_permission(
...
def require_all_permissions(permissions: List[AnalyticsPermission]):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            if not AnalyticsPermissionsChecker.has_all_permissions(
                current_user.role, permissions
            ):
                permission_names = [p.value for p in permissions]
                raise HTTPException(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            target_org_id = (
                ...
                or getattr(kwargs.get("request"), "query_params", {}).get(
                    "organization_id"
                )
            )
...
def conditional_access(
    condition_func: Callable[[User], bool], error_message: str = "Access denied"
):
    """
...
require_basic_metrics_access = require_permission_dep(
    AnalyticsPermission.VIEW_BASIC_METRICS
)

# User behavior analytics
require_user_behavior_access = require_permission_dep(
    AnalyticsPermission.VIEW_USER_BEHAVIOR
)
require_session_data_access = require_permission_dep(
    AnalyticsPermission.VIEW_SESSION_DATA
)
require_search_analytics_access = require_permission_dep(
    AnalyticsPermission.VIEW_SEARCH_ANALYTICS
)

# Quality analytics
require_quality_metrics_access = require_permission_dep(
    AnalyticsPermission.VIEW_QUALITY_METRICS
)
require_quality_alerts_access = require_permission_dep(
    AnalyticsPermission.VIEW_QUALITY_ALERTS
)
require_manage_thresholds_access = require_permission_dep(
    AnalyticsPermission.MANAGE_QUALITY_THRESHOLDS
)

# Performance analytics
require_performance_metrics_access = require_permission_dep(
    AnalyticsPermission.VIEW_PERFORMANCE_METRICS
)
require_system_health_access = require_permission_dep(
    AnalyticsPermission.VIEW_SYSTEM_HEALTH
)

# Export and reporting
require_basic_export_access = require_permission_dep(
    AnalyticsPermission.EXPORT_BASIC_REPORTS
)
require_detailed_export_access = require_permission_dep(
    AnalyticsPermission.EXPORT_DETAILED_REPORTS
)
require_custom_reports_access = require_permission_dep(
    AnalyticsPermission.GENERATE_CUSTOM_REPORTS
)

# Administrative
require_admin_settings_access = require_permission_dep(
    AnalyticsPermission.MANAGE_ANALYTICS_SETTINGS
)
require_admin_all_data_access = require_permission_dep(

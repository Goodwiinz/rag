...
from src.services.ingestion.kaggle_bulk_ingestion import KaggleBulkIngestionService, IngestionProgress
...
class BulkIngestionRequest(BaseModel):
    ...
    max_papers: int = Field(default=500000, ge=100, le=2000000, description="Maximum papers to ingest")
...
async def get_ingestion_status():
    ...
    if not is_running and not _current_ingestion:
        return BulkIngestionStatus(
            is_running=False,
            progress=None,
            message="No ingestion running or completed"
        )

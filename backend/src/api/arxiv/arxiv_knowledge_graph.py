...
class KnowledgeGraphRequest(BaseModel):
    ...
    depth: int = Field(
        default=2, ge=1, le=5, description="Depth of subgraph exploration"
    )
...
class AuthorNetworkRequest(BaseModel):
    ...
    max_depth: int = Field(
        default=2, ge=1, le=5, description="Depth of collaboration network"
    )
...
class TrendAnalysisRequest(BaseModel):
    ...
    days: int = Field(
        default=30, ge=1, le=365, description="Number of recent days to analyze"
    )

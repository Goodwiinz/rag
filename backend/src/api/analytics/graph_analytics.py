...
async def run_graph_analysis(
    request: GraphAnalysisRequest, current_user: User = Depends(get_current_user)
):
...
async def run_path_analysis(
    request: PathAnalysisRequest, current_user: User = Depends(get_current_user)
):
...
async def get_available_algorithms(current_user: User = Depends(get_current_user)):
...
async def check_graph_health(current_user: User = Depends(get_current_user)):

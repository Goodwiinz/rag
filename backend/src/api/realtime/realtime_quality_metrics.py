...
from src.services.quality.realtime_quality_metrics import (
    realtime_quality_metrics_service,
)
...
class StartEvaluationRequest(BaseModel):
    ...
    websocket_channel: Optional[str] = Field(
        None, description="WebSocket channel for updates"
    )
...
async def start_realtime_evaluation(
    ...
    try:
        ...
        websocket_channel = (
            request.websocket_channel or f"quality_metrics_{current_user.id}"
        )
        ...
        background_tasks.add_task(
            realtime_quality_metrics_service.cleanup_old_data, max_age_hours=1
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to start real-time evaluation"
        )
...
async def get_query_history(
    query_id: str, limit: int = 100, current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        history = await realtime_quality_metrics_service.get_query_metrics_history(
            query_id, limit=limit
        )

        return {"query_id": query_id, "history": history, "count": len(history)}

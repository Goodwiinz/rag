...
from ..models.search_schemas import SearchQuery, SearchType
from ..services.multi_agent_search_service import (
...
class MultiAgentSearchRequest(BaseModel):
    ...
    max_agents: int = Field(default=4, ge=1, le=6, description="Maximum number of agents to use")
    timeout: float = Field(default=120.0, ge=30.0, le=300.0, description="Maximum execution time in seconds")
    include_agent_details: bool = Field(default=True, description="Include detailed agent execution information")
    enable_quality_check: bool = Field(default=True, description="Enable quality assurance agent")
    enable_graph_navigation: bool = Field(default=True, description="Enable knowledge graph navigation")
...
class AgentBenchmarkRequest(BaseModel):
    ...
    test_queries: List[str] = Field(..., min_items=1, max_items=10, description="Test queries for benchmarking")
    agent_configurations: List[Dict[str, Any]] = Field(default_factory=list, description="Different agent configurations to test")
    timeout_per_query: float = Field(default=60.0, ge=30.0, le=180.0, description="Timeout per query")
...
async def multi_agent_search(
    ...
    try:
        ...
        logger.info(f"Multi-agent search request: query='{request.query}', user_id={current_user.id}, max_agents={request.max_agents}")
...
async def run_agent_benchmark(
    ...
    try:
        ...
        default_configs = [
            ...
            {"name": "comprehensive", "max_agents": 6, "description": "Comprehensive agent set"}
        ]

        configurations = request.agent_configurations if request.agent_configurations else default_configs
        ...
        for config in configurations:
            ...
            for query in request.test_queries:
                try:
                    ...
                    config_results["failed_queries"] += 1
                    config_results["errors"].append(f"Query '{query[:50]}...': {str(e)}")
            ...
            config_results["average_execution_time"] = total_time / successful_queries if successful_queries > 0 else 0
            config_results["average_confidence"] = total_confidence / successful_queries if successful_queries > 0 else 0

            # Agent success rate (simplified - would need more detailed tracking)
            config_results["agent_success_rate"] = successful_queries / len(request.test_queries)

            configuration_results[config_name] = config_results

        # Generate summary
        if configuration_results:
            best_config = max(configuration_results.items(), key=lambda x: x[1]["average_confidence"])
            fastest_config = min(configuration_results.items(), key=lambda x: x[1]["average_execution_time"])
            ...
            summary_stats = {
                ...
                "overall_success_rate": sum(r["successful_queries"] for r in configuration_results.values()) / (len(request.test_queries) * len(configuration_results))
            }
...
async def get_supported_agent_types():
    ...
    agent_descriptions = {
        "retrieval": {
            ...
            "capabilities": ["Hybrid search", "Document retrieval", "Relevance ranking", "Source diversity"],
            "tools": ["hybrid_search"],
        ...
        "graph_navigation": {
            ...
            "capabilities": ["Entity relationship discovery", "Concept mapping", "Multi-hop relationships", "Graph traversal"],
        ...
        "quality_assurance": {
            ...
            "capabilities": ["Result validation", "Quality assessment", "Gap identification", "Accuracy checking"],
            "tools": [],
            "estimated_duration": 20.0
        },
        "answer_synthesis": {
            ...
            "capabilities": ["Information synthesis", "Answer generation", "Coherence checking", "Response formatting"],
            "tools": [],
        ...
        "query_understanding": {
            ...
            "capabilities": ["Intent analysis", "Query refinement", "Entity extraction", "Query expansion"],
            "tools": [],
        ...
        "result_enrichment": {
            ...
            "capabilities": ["Context addition", "Metadata enrichment", "Categorization", "Summarization"],
            "tools": ["hybrid_search"],
            "estimated_duration": 15.0
        }
    }
...
async def health_check():
    ...
    try:
        ...
        is_healthy = (
            status["crewai_available"] or  # Can work in fallback mode
            len(status["agents_configured"]) > 0 or
            len(status["tools_available"]) > 0
        )
        ...
        health_status = {
            ...
            "capabilities": {
                ...
                "agent_collaboration": status["crewai_available"] and len(status["agents_configured"]) > 1
            }
        }
...
async def compare_search_methods(
    ...
    try:
        ...
        from ..models.search_schemas import SearchQuery
        ...
        comparison = {
            ...
            "standard_search": {
                ...
                "avg_relevance_score": sum(r.relevance_score for r in standard_result.results) / len(standard_result.results) if standard_result.results else 0,
                "search_type": "hybrid"
            },
            "multi_agent_search": {
            ...
            "analysis": {
                ...
                "quality_improvement": multi_agent_result.confidence_score - 0.7,  # Assuming standard search has baseline 0.7
                "agent_benefits": multi_agent_result.recommendations,
                "recommended_usage": _get_recommendation(multi_agent_result.confidence_score, multi_agent_result.execution_time, standard_time)
            }
        }
...
def _get_recommendation(confidence_score: float, multi_agent_time: float, standard_time: float) -> str:
    ...
    if confidence_score > 0.8 and multi_agent_time < standard_time * 1.5:
        ...
        return "Multi-agent search provides improved quality - use for important queries"

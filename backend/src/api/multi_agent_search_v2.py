...
from ..services.multi_agent_search_service_v2 import (
...
class MultiAgentSearchRequest(BaseModel):
    ...
    workflow_type: WorkflowType = Field(default=WorkflowType.FACTUAL_LOOKUP, description="Type of search workflow")
    max_agents: int = Field(default=4, ge=1, le=7, description="Maximum number of agents to use")
    timeout: float = Field(default=180.0, ge=30.0, le=600.0, description="Maximum execution time in seconds")
    enable_learning: bool = Field(default=True, description="Enable query learning")
    user_preferences: Optional[Dict[str, Any]] = Field(default=None, description="User-specific preferences")
...
class AgentBenchmarkRequest(BaseModel):
    ...
    queries: List[str] = Field(..., description="List of test queries", min_items=1, max_items=100)
    workflow_types: List[WorkflowType] = Field(default=[WorkflowType.FACTUAL_LOOKUP], description="Workflow types to test")
    iterations: int = Field(default=3, ge=1, le=10, description="Number of iterations per query")
...
async def enhanced_multi_agent_search(
    ...
    try:
        ...
        logger.info(f"Enhanced multi-agent search request from user {current_user.id}: {request.query}")
...
async def get_service_status(current_user: User = Depends(get_current_user)):
    ...
    try:
        # Get service report
        performance_report = multi_agent_search_service_v2.get_agent_performance_report()
...
async def get_workflow_recommendations(
    ...
    """
    try:
        recommendations = multi_agent_search_service_v2.get_workflow_recommendations(request.query)

        # Enhance with user-specific recommendations
        if hasattr(current_user, 'preferences'):
            user_recs = generate_user_recommendations(current_user.preferences, request.query)
        ...
        if request.parallel:
            ...
            for i, result in enumerate(parallel_results):
                ...
                if isinstance(result, Exception):
                    logger.error(f"Benchmark error for {query} with {workflow.value}: {result}")
            ...
            for query in request.queries:
                for workflow_type in request.workflow_types:
                    result = await run_single_benchmark(query, workflow_type, request.iterations)
                    benchmark_results["results"][f"{query}_{workflow_type.value}"] = result

        # Calculate summary statistics
        benchmark_results["summary"] = calculate_benchmark_summary(benchmark_results["results"])

        # Log benchmark completion
        logger.info(f"Benchmark completed in {benchmark_results['summary'].get('total_time', 0):.2f}s")
    ...
    try:
        ...
        if agent_type:
            if agent_type in report["agent_metrics"]:
                report["agent_metrics"] = {agent_type: report["agent_metrics"][agent_type]}
            else:
                raise HTTPException(
                    status_code=404,
        ...
        report["optimizations"] = generate_optimization_suggestions(report["agent_metrics"])
    ...
    try:
        ...
        for entry in history:
            formatted_entry = {
                ...
                "workflow_type": entry["analysis"].get("workflow_type", {}).value if isinstance(entry["analysis"].get("workflow_type"), WorkflowType) else "unknown"
            }
...
# Helper Functions
async def run_single_benchmark(query: str, workflow_type: WorkflowType, iterations: int) -> Dict[str, Any]:
    ...
    if successful_runs > 0:
        results["statistics"] = {
            ...
            "min_time": min(r["execution_time"] for r in results["iterations"] if r.get("success")),
            "max_time": max(r["execution_time"] for r in results["iterations"] if r.get("success")),
            "std_dev_time": calculate_std_dev([r["execution_time"] for r in results["iterations"] if r.get("success")])
        }
    else:
        results["statistics"] = {
            "success_rate": 0,
...
def calculate_benchmark_summary(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for test_name, test_result in results.items():
        if isinstance(test_result, dict) and "statistics" in test_result:
            ...
            if stats.get("success_rate", 0) > 0:
                ...
                workflow_stats[workflow_type]["total_time"] += stats.get("avg_execution_time", 0)

                if "avg_confidence_score" in stats:
                    total_confidence_sum += stats["avg_confidence_score"]
                    confidence_count += 1
                    workflow_stats[workflow_type]["confidence_sum"] += stats["avg_confidence_score"]
            else:
    ...
    for workflow_type, stats in workflow_stats.items():
        if stats["count"] > 0:
            summary["workflow_performance"][workflow_type] = {
                ...
                "avg_confidence": stats["confidence_sum"] / max(stats["success_count"], 1)
            }
    ...
    if summary["workflow_performance"]:
        ...
        best_workflow = max(summary["workflow_performance"].items(),
        ...
        fastest_workflow = min(summary["workflow_performance"].items(),
...
def generate_optimization_suggestions(metrics: Dict[str, Any]) -> List[str]:
    ...
    for agent_type, agent_metrics in metrics.items():
        if agent_metrics["success_rate"] < 0.8:
            suggestions.append(f"Consider reviewing {agent_type} agent configuration - success rate is {agent_metrics['success_rate']:.1%}")

        if agent_metrics["average_execution_time"] > 60:
            suggestions.append(f"{agent_type} agent is taking {agent_metrics['average_execution_time']:.1f}s on average - consider optimization")

        if agent_metrics["error_rate"] > 0.2:
            suggestions.append(f"{agent_type} agent has high error rate ({agent_metrics['error_rate']:.1%}) - investigate failure patterns")

    return suggestions or ["All agents are performing within acceptable parameters"]


def generate_user_recommendations(user_preferences: Dict[str, Any], query: str) -> Dict[str, Any]:
    ...
    if user_preferences.get("prefers_speed", False):
        recommendations["workflow_adjustments"].append("Consider using FACTUAL_LOOKUP workflow for faster responses")
        recommendations["agent_preferences"].append("Limit to 3 agents for better performance")

    if user_preferences.get("prefers_comprehensive", False):
        recommendations["workflow_adjustments"].append("Use REASONING workflow for detailed analysis")
        recommendations["agent_preferences"].append("Enable all available agents")

    # Based on query length
    if len(query.split()) > 15:
        recommendations["optimization_tips"].append("Consider breaking down complex queries")
...
async def log_search_analytics(
    ...
    logger.info(f"Analytics logged: user={user_id}, query='{query}', time={execution_time:.2f}s, confidence={confidence_score:.2f}, agents={num_agents}")
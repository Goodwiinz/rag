...
async def list_documents(
    ...
    date_from: Optional[datetime] = Query(None, description="Filter documents from date"),
...
async def get_document_entities(
    ...
    try:
        ...
        for entity in entities:
            entity_responses.append(DocumentEntityResponse(
                id=str(entity.id),
                name=entity.name,
...
async def get_document_status(
    ...
    if processing_job and processing_job.started_at and processing_job.progress_percentage < 100:
        # Simple estimation based on current progress
        elapsed_time = (datetime.utcnow() - processing_job.started_at).total_seconds()
        if processing_job.progress_percentage > 0:
            estimated_total_time = elapsed_time / (processing_job.progress_percentage / 100)
            estimated_completion = processing_job.started_at + timedelta(seconds=estimated_total_time)
    ...
    return DocumentStatusResponse(
        ...
        error_message=document.processing_error or (processing_job.error_message if processing_job else None)
    )

@router.post("/search", response_model=DocumentSearchResponse)
async def search_documents(
    query: str = Query(..., description="Search query"),
    document_types: Optional[List[DocumentType]] = Query(None, description="Filter by document types"),
    tags: Optional[List[str]] = Query(None, description="Filter by tags"),
    date_range: Optional[str] = Query(None, description="Date range filter (e.g., 'last_week', 'last_month')"),
    page: int = Query(1, ge=1),
    ...
    try:
        ...
        db_query = db_query.order_by(
            Document.title.ilike(search_pattern).desc(),
...
async def reprocess_document(
    document_id: str,
    force_reprocess: bool = Query(False, description="Force reprocess even if already completed"),
    current_user: User = Depends(get_current_user),
    organization: Organization = Depends(get_current_organization),
    db: Session = Depends(get_db)
):
    ...
    if (document.processing_status == ProcessingStatus.COMPLETED and
        not force_reprocess):
        return {

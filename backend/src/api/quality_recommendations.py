...
async def get_quality_recommendations(
    categories: Optional[List[str]] = Query(None, description="Filter by recommendation categories"),
    priority: Optional[str] = Query(None, regex="^(critical|high|medium|low)$", description="Filter by priority level"),
    status: Optional[str] = Query(None, regex="^(pending|in_progress|completed|rejected|deferred)$", description="Filter by status"),
    days_back: int = Query(30, ge=1, le=365, description="Days of history to analyze"),
    limit: int = Query(50, ge=1, le=200, description="Maximum recommendations to return"),
    ...
    try:
        ...
        focus_areas = None
        if categories:
            focus_areas = [RecommendationCategory(cat) for cat in categories if cat in [c.value for c in RecommendationCategory]]
        ...
        recommendations = await quality_recommendations_service.generate_recommendations(
...
async def update_recommendation_progress(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(status_code=400, detail="Failed to update recommendation progress")
...
async def get_recommendation_effectiveness(
    completed_days: int = Query(30, ge=1, le=365, description="Days to analyze for effectiveness"),
    current_user: User = Depends(get_current_user)
):
    """Get effectiveness analysis of implemented recommendations"""
    try:
        effectiveness = await quality_recommendations_service.get_recommendation_effectiveness(
            organization_id=str(current_user.organization_id),
...
async def get_recommendations_summary(
    ...
    try:
        ...
        for rec in recommendations:
            ...
            summary["by_category"][category]["total_estimated_improvement"] += rec.estimated_improvement
        ...
        for priority in summary["by_priority"]:
            ...
            if priority_recs:
                avg_improvement = sum(r.estimated_improvement for r in priority_recs) / len(priority_recs)
                summary["by_priority"][priority]["avg_estimated_improvement"] = avg_improvement
...
async def get_quality_metrics_for_recommendations(
    ...
    try:
        ...
        from src.services.performance_dashboard_service import MetricTimeRange
        ...
        from src.services.user_behavior_service import MetricTimeRange as BehaviorTimeRange
        ...
        return {
            ...
            "user_behavior": {
                "active_users": len([t for t in user_behavior["engagement_trends"] if t["active_users"] > 0]),
                "search_volume": sum(t["search_count"] for t in user_behavior["search_trends"]),
                "avg_satisfaction": sum(t["avg_satisfaction"] for t in user_behavior["engagement_trends"] if t["avg_satisfaction"]) / len(user_behavior["engagement_trends"]) if user_behavior["engagement_trends"] else 0
            },
            "period_days": days_back,
            "generated_at": datetime.utcnow().isoformat()
        }
...
async def generate_custom_recommendations(
    ...
    try:
        ...
        if focus_areas:
            focus_areas = [RecommendationCategory(area) for area in focus_areas if area in [c.value for c in RecommendationCategory]]
...
async def quality_recommendations_health():
    ...
    return {
        ...
        "cached_recommendations": len(quality_recommendations_service.recommendation_cache),

...
from src.models.evaluation import EvaluationDataset as EvalDataset
...
from .rag_evaluation_service import (
    RAGEvaluationInput,
...
class DeepEvalIntegration:
        ...
        try:
            ...
            if settings.OPENAI_API_KEY:
                models["openai"] = GPTModel(
                    model_name="gpt-4", api_key=settings.OPENAI_API_KEY
                )
    ...
    async def _run_deepeval_evaluation(
        ...
        try:
            ...
            final_overall_score = (
                overall_score / len(deepeval_test_cases) if deepeval_test_cases else 0.0
            )
            success_rate = (
                successful_tests / (len(deepeval_test_cases) * len(metrics))
            ) * 100
    ...
    async def _run_custom_evaluation(
        ...
        try:
            ...
            for test_case in test_cases:
                ...
                individual_metrics[
                    f"{test_case.test_id}_answer_relevancy"
                ] = metrics.answer_relevancy
                individual_metrics[
                    f"{test_case.test_id}_faithfulness"
                ] = metrics.faithfulness
                individual_metrics[
                    f"{test_case.test_id}_contextual_relevancy"
                ] = metrics.contextual_relevancy
                individual_metrics[
                    f"{test_case.test_id}_hallucination_rate"
    ...
    async def _run_hybrid_evaluation(
        ...
        try:
            ...
            deepeval_results = (
                await self._run_deepeval_evaluation(
                    test_cases, query_type, organization_id, user_id
                )
            ...
            if deepeval_results:
                ...
                combined_individual_metrics = {
                    ...
                    **{
                        f"custom_{k}": v
                        for k, v in custom_results.individual_metrics.items()
                    },
                }
    ...
    async def evaluate_query_types(
        ...
        for query_type, test_cases in query_test_suite.items():
            try:
                logger.info(
                    f"Evaluating query type: {query_type.value} with {len(test_cases)} test cases"
                )
    ...
    def generate_evaluation_report(
        ...
        try:
            if isinstance(results, dict):
                ...
                return self._generate_multi_query_report(
                    results, include_recommendations
                )
            else:
                # Single evaluation
    ...
    def _generate_single_report(
        self, results: EvaluationResults, include_recommendations: bool
    ) -> str:
...
"""
    ...
    def _generate_multi_query_report(
        self, results: Dict[str, EvaluationResults], include_recommendations: bool
    ) -> str:
...
"""
            ...
            metrics_sorted = sorted(
                result.individual_metrics.items(), key=lambda x: x[1], reverse=True
            )[:5]
    ...
    def _generate_recommendations(self, results: EvaluationResults) -> str:
        ...
        if results.overall_score < 0.6:
            recommendations.append(
                "- 🚨 **Critical**: Overall system performance requires immediate attention"
            )
        elif results.overall_score < 0.75:
            recommendations.append(
                "- ⚠️ **Moderate**: System performance needs improvement"
            )
        else:
        ...
        for violation in results.threshold_violations[:5]:
            ...
            if "relevancy" in metric.lower():
                recommendations.append(
                    f"- 📝 **Improve {metric}**: Review query understanding and context retrieval"
                )
            elif "faithfulness" in metric.lower():
                recommendations.append(
                    f"- 🔗 **Improve {metric}**: Ensure answers are grounded in retrieved context"
                )
                ...
                recommendations.append(
                    f"- 🔍 **Review {metric}**: Investigate and optimize performance"
                )
...
"""
    ...
    def _generate_multi_query_recommendations(
        ...
        if weak_areas:
            recommendations.append(
                f"- 🚨 **Critical Issues**: Query types needing immediate attention: {', '.join(weak_areas)}"
            )

        if strong_areas:
            recommendations.append(
                f"- ✅ **Strong Performance**: Query types performing well: {', '.join(strong_areas)}"
            )
        ...
        if violation_counts:
            ...
            recommendations.append(
                "- 🔍 **Common Issues**: Metrics that need system-wide attention:"
            )
            for metric, count in top_violations:
                recommendations.append(
                    f"  - **{metric}**: {count} violations across query types"
                )

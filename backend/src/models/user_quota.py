...
class UserQuota(BaseModel):
    ...
    storage_allocated_bytes = Column(
        BigInteger, nullable=False, default=0
    )  # Sum of document sizes
    ...
    monthly_bandwidth_quota_bytes = Column(
        BigInteger, nullable=False, default=100 * 1024 * 1024 * 1024
    )  # 100GB
    ...
    # Quota status
    quota_status = Column(
        Enum(QuotaStatus), nullable=False, default=QuotaStatus.HEALTHY, index=True
    )
    ...
    cleanup_threshold_days = Column(
        Integer, default=365, nullable=False
    )  # Clean docs older than this
    last_cleanup_at = Column(DateTime(timezone=True), nullable=True)

    # Relationships
    user = relationship("User", back_populates="quota")
    organization = relationship("Organization")
    quota_adjustments = relationship(
        "QuotaAdjustment", back_populates="user_quota", cascade="all, delete-orphan"
    )
    quota_alerts = relationship(
        "QuotaAlert", back_populates="user_quota", cascade="all, delete-orphan"
    )
    usage_snapshots = relationship(
        "UsageSnapshot", back_populates="user_quota", cascade="all, delete-orphan"
    )
    ...
    def is_over_quota(self) -> bool:
        ...
        return (
            ...
            or self.document_count > self.document_quota
            or self.current_month_queries > self.monthly_query_quota
            or self.monthly_bandwidth_used_bytes > self.monthly_bandwidth_quota_bytes
        )
    ...
    def can_upload_file(self, file_size_bytes: int) -> bool:
        ...
        return (
            ...
            and self.storage_used_bytes + file_size_bytes <= self.storage_quota_bytes
    ...
    def update_bandwidth_usage(self, delta_bytes: int):
        ...
        self.monthly_bandwidth_used_bytes = max(
            0, self.monthly_bandwidth_used_bytes + delta_bytes
        )
        self._update_quota_status()
    ...
    def adjust_quota(
        ...
        adjustment = QuotaAdjustment(
            ...
            storage_bytes_delta=storage_bytes - self.storage_quota_bytes
            if storage_bytes
            else 0,
            document_count_delta=document_count - self.document_count
            if document_count
            else 0,
            query_quota_delta=query_quota - self.monthly_query_quota
            if query_quota
            else 0,
            reason=reason,
        )
    ...
    def get_usage_summary(self) -> dict:
        ...
        return {
            ...
            "status": {
                ...
                "grace_period_ends": self.grace_period_ends_at.isoformat()
    ...
    @classmethod
    def get_users_nearing_quota(
        cls, organization_id: Optional[uuid.UUID] = None, threshold: float = 80.0
    ) -> list:
        """Get users nearing their quota limit"""
        query = cls.query.filter(
            cls.quota_status == QuotaStatus.WARNING, cls.is_deleted == False
        )
...
class QuotaAlert(BaseModel):
    ...
    )  # warning, critical, exceeded, suspension
...
class UsageSnapshot(BaseModel):
    ...
    def create_daily_snapshot(cls, user_quota_id: uuid.UUID):
        ...
        snapshot_date = datetime.combine(date.today(), datetime.min.time()).replace(

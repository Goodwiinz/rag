...
class EncryptedOrganizationProfile(Base):
    ...
    def get_decrypted_data(self, fields: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        if fields is None:
            fields = [
                ...
                'billing_address', 'shipping_address', 'billing_phone', 'billing_email',
                'bank_account_number', 'bank_routing_number', 'payment_method',
                'legal_contact_name', 'legal_contact_email', 'legal_contact_phone',
                'business_notes', 'custom_attributes'
            ]
    ...
    def mask_sensitive_fields(self) -> Dict[str, Any]:
        ...
        if self.bank_account_number:
            masked_data['bank_account_number'] = mask_bank_account(self.bank_account_number)
...
class EncryptionAuditLog(Base):
    ...
    # Operation details
    operation_type = Column(String(50), nullable=False)  # ENCRYPT, DECRYPT, ROTATE_KEY, etc.
    resource_type = Column(String(50), nullable=False)   # USER_PROFILE, ORG_PROFILE, etc.

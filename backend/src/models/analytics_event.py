...
class AnalyticsEvent(BaseModel):
    ...
    event_category = Column(String(100), nullable=True, index=True)  # Additional categorization
    ...
    # Temporal data
    event_timestamp = Column(DateTime(timezone=True), default=datetime.utcnow, nullable=False, index=True)
    date_hour = Column(String(13), nullable=False, index=True)  # YYYY-MM-DDTHH for time-series queries
    date_day = Column(String(10), nullable=False, index=True)   # YYYY-MM-DD for daily aggregation

    # Metadata
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)
    processed = Column(Boolean, default=False, nullable=False, index=True)  # For async processing
    ...
    __table_args__ = (
        ...
        Index('idx_analytics_events_composite', 'organization_id', 'event_type', 'date_day'),
    )
    ...
    @classmethod
    def create_search_event(cls, user_id, organization_id, session_id, query, results_count, response_time_ms, **kwargs):
    ...
    @classmethod
    def create_document_view_event(cls, user_id, organization_id, session_id, document_id, view_duration_seconds, **kwargs):
    ...
    @classmethod
    def create_performance_event(cls, organization_id, metric_name, value, unit="ms", **kwargs):
        ...
        return cls(
            ...
            value=value,
            unit=unit,
            severity=EventSeverity.WARNING if value > kwargs.get("threshold", 1000) else EventSeverity.INFO,
            **kwargs
        )

    @classmethod
    def create_error_event(cls, user_id, organization_id, error_message, error_code, **kwargs):
        ...
        return cls(
            ...
            severity=EventSeverity.CRITICAL if error_code >= 500 else EventSeverity.ERROR,
            **kwargs
        )

    @classmethod
    def create_quality_metric_event(cls, organization_id, metric_name, value, threshold=None, **kwargs):

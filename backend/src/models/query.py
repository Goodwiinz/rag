...
class QueryRequest(BaseModel):
    ...
    query: str = Field(
        ..., min_length=1, max_length=2000, description="The search query"
    )
    ...
    max_expansion_terms: int = Field(
        10, ge=1, le=50, description="Maximum expansion terms"
    )
...
class QueryHistory(BaseModel):
    ...
    clicked_results: List[str] = Field(
        default_factory=list, description="Clicked result IDs"
    )
    satisfaction_score: Optional[float] = Field(
        None, ge=1.0, le=5.0, description="User satisfaction"
    )
...
class QueryFeedback(BaseModel):
    ...
    result_positions: List[int] = Field(
        default_factory=list, description="Clicked result positions"
    )
    session_duration_ms: Optional[float] = Field(
        None, ge=0.0, description="Session duration"
    )
    created_at: datetime = Field(
        default_factory=datetime.utcnow, description="Creation timestamp"
    )
...
class QueryPattern(BaseModel):
    ...
    intent_distribution: Dict[QueryIntent, float] = Field(
    ...
    avg_satisfaction: Optional[float] = Field(
        None, ge=1.0, le=5.0, description="Average satisfaction"
    )
...
class QueryAnalytics(BaseModel):
    ...
    complexity_distribution: Dict[QueryComplexity, float] = Field(
    ...
    satisfaction_rate: float = Field(
        ..., ge=0.0, le=1.0, description="Satisfaction rate"
    )

...
class GraphSearchRequest(BaseModel):
    ...
    include_metadata: bool = Field(default=True, description="Include entity/relationship metadata")
    document_id_filter: Optional[str] = Field(None, description="Filter by specific document")


class GraphPath(BaseModel):
    """Model representing a path in the graph"""
    entities: List[EntityResponse] = Field(..., description="Entities in the path")
    relationships: List[RelationshipResponse] = Field(..., description="Relationships connecting entities")
...
class GraphSearchResponse(BaseModel):
    ...
    relationships: List[RelationshipResponse] = Field(default_factory=list, description="Found relationships")
    paths: List[GraphPath] = Field(default_factory=list, description="Found paths between entities")
    total_entities: int = Field(default=0, description="Total number of entities found")
    total_relationships: int = Field(default=0, description="Total number of relationships found")
...
class GraphAnalytics(BaseModel):
    ...
    relationship_type_counts: Dict[str, int] = Field(default_factory=dict, description="Count by relationship type")
    ...
    clustering_coefficient: float = Field(default=0.0, description="Average clustering coefficient")

...
class RankingFactors(BaseModel):
    ...
    text_similarity: float = Field(
        0.0, ge=0.0, le=1.0, description="Text similarity score"
    )
    ...
    source_preference: float = Field(
        0.0, ge=0.0, le=1.0, description="Source preference score"
    )
    modality_preference: float = Field(
        0.0, ge=0.0, le=1.0, description="Modality preference score"
    )
    content_length_preference: float = Field(
        0.0, ge=0.0, le=1.0, description="Content length preference score"
    )

    # Behavioral features
    historical_ctr: float = Field(
        0.0, ge=0.0, le=1.0, description="Historical click-through rate"
    )
    recent_interactions: float = Field(
        0.0, ge=0.0, le=1.0, description="Recent interaction score"
    )
    session_relevance: float = Field(
        0.0, ge=0.0, le=1.0, description="Session relevance score"
    )

    # Quality features
    content_quality: float = Field(
        0.0, ge=0.0, le=1.0, description="Content quality score"
    )
    source_authority: float = Field(
        0.0, ge=0.0, le=1.0, description="Source authority score"
    )
    freshness: float = Field(0.0, ge=0.0, le=1.0, description="Freshness score")

    # Temporal features
    time_relevance: float = Field(
        0.0, ge=0.0, le=1.0, description="Time relevance score"
    )
...
class UserBehavior(BaseModel):
    ...
    dwell_time_ms: Optional[float] = Field(
        None, ge=0.0, description="Dwell time in milliseconds"
    )
...
class RankingFeedback(BaseModel):
    ...
    final_position: Optional[int] = Field(
        None, ge=0, description="Final position after user action"
    )
...
class RankingMetrics(BaseModel):
    ...
    strategy_distribution: Dict[RankingStrategy, float] = Field(
        ..., description="Strategy usage distribution"
    )
    user_satisfaction_rate: float = Field(
        ..., ge=0.0, le=1.0, description="User satisfaction rate"
    )
...
class DiversityMetrics(BaseModel):
    ...
    source_diversity: float = Field(
        ..., ge=0.0, le=1.0, description="Source diversity score"
    )
    modality_diversity: float = Field(
        ..., ge=0.0, le=1.0, description="Modality diversity score"
    )
    topic_diversity: float = Field(
        ..., ge=0.0, le=1.0, description="Topic diversity score"
    )
    temporal_diversity: float = Field(
        ..., ge=0.0, le=1.0, description="Temporal diversity score"
    )
    overall_diversity: float = Field(
        ..., ge=0.0, le=1.0, description="Overall diversity score"
    )
    unique_sources_count: int = Field(..., ge=0, description="Number of unique sources")
    unique_modalities_count: int = Field(
        ..., ge=0, description="Number of unique modalities"
    )
...
class RankingDebugInfo(BaseModel):
    ...
    intermediate_scores: List[Dict[str, float]] = Field(
        ..., description="Intermediate scores"
    )
    ...
    debug_metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional debug info"
    )

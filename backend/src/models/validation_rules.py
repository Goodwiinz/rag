...
import bleach
from pydantic import BaseModel as PydanticBaseModel, validator, ValidationError as PydanticValidationError
...
class LengthRule(ValidationRule):
    ...
    def validate(self, value: Any, instance: Any = None) -> bool:
        ...
        if self.min_length is not None and length < self.min_length:
            raise ValueError(f"{self.field_name} must be at least {self.min_length} characters long")

        if self.max_length is not None and length > self.max_length:
            raise ValueError(f"{self.field_name} must be no more than {self.max_length} characters long")
...
class RegexRule(ValidationRule):
    ...
    def validate(self, value: Any, instance: Any = None) -> bool:
        ...
        if not self.pattern.match(value):
            raise ValueError(self.error_message or f"{self.field_name} format is invalid")
    ...
    def validate(self, value: Any, instance: Any = None) -> bool:
        ...
        if enum_value not in self.valid_values:
            raise ValueError(
                ...
                f"{self.field_name} must be one of: {', '.join(self.valid_values)}"
            )
...
class EmailRule(ValidationRule):
    ...
    def validate(self, value: Any, instance: Any = None) -> bool:
        ...
        try:
            ...
            raise ValueError(f"{self.field_name} is not a valid email address: {str(e)}")
...
class URLRule(ValidationRule):
    ...
    def __init__(self, field_name: str, schemes: List[str] = None, error_message: str = None):
        super().__init__(field_name, error_message)
    ...
    def validate(self, value: Any, instance: Any = None) -> bool:
        ...
        if not url_pattern.match(value):
            raise ValueError(self.error_message or f"{self.field_name} is not a valid URL")
...
class FileSizeRule(ValidationRule):
    ...
    def validate(self, value: Any, instance: Any = None) -> bool:
        ...
        if size_bytes > self.max_size_bytes:
            ...
            raise ValueError(
                self.error_message or
                f"{self.field_name} ({size_mb:.1f}MB) exceeds maximum allowed size ({max_mb:.1f}MB)"
            )
...
class DateTimeRule(ValidationRule):
    ...
    def __init__(self, field_name: str, allow_future: bool = True, allow_past: bool = True,
                 min_date: datetime = None, max_date: datetime = None, error_message: str = None):
        super().__init__(field_name, error_message)
...
class SanitizeHTMLRule(ValidationRule):
    ...
    def __init__(self, field_name: str, allowed_tags: List[str] = None, allowed_attributes: List[str] = None):
...
class CrossFieldRule(ValidationRule):
    ...
    def __init__(self, field_name: str, dependent_fields: List[str],
                 validation_func: Callable[[Dict[str, Any]], bool], error_message: str = None):
        super().__init__(field_name, error_message)
    ...
    def validate(self, value: Any, instance: Any = None) -> bool:
        ...
        field_values = {field: getattr(instance, field, None) for field in self.dependent_fields}
        field_values[self.field_name] = value

        try:
            if not self.validation_func(field_values):
                raise ValueError(self.error_message or f"Cross-field validation failed for {', '.join(self.dependent_fields + [self.field_name])}")
...
MODEL_VALIDATION_RULES = {
    ...
    'EnhancedDocument': [
        ...
        CrossFieldRule('processing_status', ['processing_completed_at', 'processing_error'],
    ...
    'UserQuota': [
        ...
        CrossFieldRule('storage_used_bytes', ['storage_quota_bytes'], validate_quota_usage),
    ],
...
class ConsistencyValidator:
    ...
    @staticmethod
    def validate_user_quota_consistency(user_id: uuid.UUID, organization_id: uuid.UUID) -> bool:
    ...
    @staticmethod
    def validate_entity_organization_scope(entity_id: uuid.UUID, organization_id: uuid.UUID) -> bool:
...
class DocumentUploadSchema(PydanticBaseModel):
    ...
    def validate_sharing_level(cls, v):
        ...
        if v not in allowed_levels:
            raise ValueError(f'Sharing level must be one of: {", ".join(allowed_levels)}')
        return v
...
class RAGQuerySchema(PydanticBaseModel):
    ...
    def validate_query_type(cls, v):
        allowed_types = ['factual_lookup', 'reasoning', 'comparison', 'summary', 'analysis', 'recommendation']
    ...
    def validate_search_strategy(cls, v):
        ...
        if v not in allowed_strategies:
            raise ValueError(f'Search strategy must be one of: {", ".join(allowed_strategies)}')
        return v
...
def validate_file_upload(file_size: int, file_name: str, allowed_extensions: List[str] = None) -> ValidationSummary:
    ...
    if file_size > max_size:
        summary.add_error('file_size', f'File size ({file_size} bytes) exceeds maximum ({max_size} bytes)', file_size)
    else:
        summary.add_success()
    ...
    if allowed_extensions:
        ...
        if file_extension not in allowed_extensions:
            summary.add_error('file_extension', f'File extension .{file_extension} not allowed', file_extension)
        else:
            summary.add_success()
    else:
        summary.add_success()
...
__all__ = [
    'ValidationRule', 'LengthRule', 'RegexRule', 'RangeRule', 'EnumRule',
    'EmailRule', 'URLRule', 'FileSizeRule', 'DateTimeRule', 'SanitizeHTMLRule',
    'CrossFieldRule', 'MODEL_VALIDATION_RULES', 'ConsistencyValidator',
    'UserCreateSchema', 'DocumentUploadSchema', 'RAGQuerySchema', 'EntityCreateSchema',
    'ValidationError', 'ValidationSummary', 'validate_model_instance',
    'sanitize_user_input', 'validate_file_upload', 'setup_validation_event_listners'
]
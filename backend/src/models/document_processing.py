...
from sqlalchemy import Column, String, Integer, Float, Boolean, DateTime, Enum, ForeignKey, Text, JSON, CheckConstraint
...
class DocumentVersion(BaseModel):
    ...
    # Version information
    version_label = Column(String(100), nullable=True)  # e.g., "v1.0.0", "draft", "final"
    ...
    parent_version_id = Column(GUID(), ForeignKey("document_versions.id"), nullable=True)
    ...
    parent_version = relationship("DocumentVersion", remote_side="DocumentVersion.id", back_populates="child_versions")
...
class MultimodalContent(BaseModel):
    ...
    # Document reference
    document_id = Column(GUID(), ForeignKey("documents.id"), nullable=False, index=True)
    document_version_id = Column(GUID(), ForeignKey("document_versions.id"), nullable=True)

    # Content identification
    content_type = Column(Enum(ContentType), nullable=False, index=True)
    content_id = Column(String(255), nullable=False, index=True)  # Unique ID within document
    ...
    media_dimensions = Column(JSONB, nullable=True)  # For images/video {"width": 1920, "height": 1080}
    ...
    __table_args__ = (
        ...
        CheckConstraint('extraction_confidence >= 0 AND extraction_confidence <= 1', name='check_extraction_confidence_range'),
        ...
        CheckConstraint('media_duration_seconds >= 0', name='check_media_duration_positive'),
    )
...
class DocumentQualityMetrics(BaseModel):
    ...
    # Document reference
    document_id = Column(GUID(), ForeignKey("documents.id"), nullable=False, index=True)
    content_id = Column(String(255), nullable=True, index=True)  # Reference to specific content

    # Metric information
    metric_type = Column(Enum(QualityMetricType), nullable=False, index=True)
    metric_value = Column(Float, nullable=False)
    metric_unit = Column(String(50), nullable=True)  # e.g., "score", "percentage", "pixels"

    # Assessment information
    assessment_method = Column(String(100), nullable=True)  # e.g., "automated", "manual", "hybrid"
    assessment_version = Column(String(50), nullable=True)  # Version of assessment algorithm
    ...
    assessment_config = Column(JSONB, nullable=True)  # Configuration used for assessment
...
class DocumentAccessLog(BaseModel):
    ...
    # Access information
    access_type = Column(String(50), nullable=False, index=True)  # view, download, edit, delete, share
    access_result = Column(String(20), nullable=False, default="success")  # success, denied, failed
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint('response_size_bytes >= 0', name='check_response_size_positive'),

...
import uuid
from sqlalchemy import Column, String, Integer, Float, Boolean, DateTime, Enum, ForeignKey, Text, JSON, Index, BigInteger, Numeric
...
class EnhancedDocument(BaseModel):
    ...
    document_subtype = Column(String(50), nullable=True)  # Specific document type (e.g., "invoice", "contract")
    ...
    uploaded_by_user_id = Column(GUID(), ForeignKey("users.id"), nullable=False, index=True)
    owner_user_id = Column(GUID(), ForeignKey("users.id"), nullable=True, index=True)

    # Access permissions
    is_public = Column(Boolean, default=False, nullable=False)
    sharing_level = Column(String(20), nullable=False, default="private")  # private, team, organization, public
    ...
    sensitivity_level = Column(String(20), nullable=True, default="normal")  # normal, sensitive, confidential

    # Processing information
    processing_status = Column(Enum(ProcessingStage), nullable=False, default=ProcessingStage.UPLOADED, index=True)
    ...
    # Multimodal content
    multimodal_content = Column(JSON, nullable=True)  # Information about non-text content
    ...
    average_relevance_score = Column(Float, nullable=True)  # Average relevance in search results

    # Version control
    version_number = Column(Integer, default=1, nullable=False)
    is_latest_version = Column(Boolean, default=True, nullable=False)
    parent_document_id = Column(GUID(), ForeignKey("enhanced_documents.id"), nullable=True)
    ...
    uploaded_by_user = relationship("User", foreign_keys=[uploaded_by_user_id], back_populates="uploaded_documents")
    owner_user = relationship("User", foreign_keys=[owner_user_id], back_populates="owned_documents")
    last_accessed_by_user = relationship("User", foreign_keys=[last_accessed_by_user_id])
    parent_document = relationship("EnhancedDocument", remote_side=[id], backref="child_versions")
    ...
    feedback_entries = relationship("DocumentFeedback", back_populates="document", cascade="all, delete-orphan")
    ...
    def is_processing_complete(self) -> bool:
        ...
        return self.processing_status in [ProcessingStage.COMPLETED, ProcessingStage.FAILED]
    ...
    def update_processing_status(self, status: ProcessingStage, error_message: str = None):
        ...
        if status == ProcessingStage.VALIDATED and not self.processing_started_at:
            ...
            if self.processing_started_at:
                self.processing_duration_seconds = (self.processing_completed_at - self.processing_started_at).total_seconds()
    ...
    def add_processing_error(self, stage: ProcessingStage, error_message: str, error_details: Dict[str, Any] = None):
    ...
    def calculate_quality_score(self):
        ...
        elif self.processing_status in [ProcessingStage.INDEXED, ProcessingStage.EMBEDDED]:
        ...
        if self.multimodal_content:
            modality_count = len(self.modalities_present) if self.modalities_present else 1
            content_quality = min(1.0, modality_count / 3)  # Max quality at 3+ modalities
    ...
    def add_access_event(self, user_id: uuid.UUID, action: str, metadata: Dict[str, Any] = None):
    ...
    def can_user_access(self, user_id: uuid.UUID, user_teams: List[str] = None) -> bool:
        ...
        if self.sharing_level == "organization":
            return True  # Assuming user is in same organization (handled at query level)

        # Team-level access
        if self.sharing_level == "team" and user_teams:
            return any(team in self.allowed_teams for team in user_teams) if self.allowed_teams else False
    ...
    def to_dict(self, include_content: bool = False, include_processing_details: bool = False) -> Dict[str, Any]:
    ...
    @classmethod
    def get_documents_by_modality(cls, modality: DocumentModality, organization_id: Optional[uuid.UUID] = None) -> List:
        """Get documents by modality"""
        query = cls.query.filter(
            cls.primary_modality == modality,
    ...
    @classmethod
    def get_high_quality_documents(cls, organization_id: Optional[uuid.UUID] = None, min_quality: QualityLevel = QualityLevel.HIGH) -> List:
        """Get high-quality documents"""
        query = cls.query.filter(
            cls.quality_level >= min_quality,
            cls.is_deleted == False
    ...
    @classmethod
    def search_by_content(cls, search_text: str, organization_id: Optional[uuid.UUID] = None, limit: int = 50) -> List:
        """Full-text search in document content"""
        from sqlalchemy import func

        query = cls.query.filter(
            cls.search_vector.match(search_text),
            cls.is_deleted == False
        )
...
class DocumentQualityMetrics(BaseModel):
    ...
    text_completeness = Column(Float, nullable=True)  # How complete the text extraction is
    text_accuracy = Column(Float, nullable=True)  # Accuracy of extracted text
    formatting_preserved = Column(Float, nullable=True)  # How well formatting was preserved
...
class DocumentFeedback(BaseModel):
    ...
    # Feedback details
    feedback_type = Column(String(50), nullable=False)  # quality, relevance, accuracy, completeness
    ...
    # Feedback metadata
    feedback_timestamp = Column(DateTime(timezone=True), nullable=False, default=datetime.utcnow)

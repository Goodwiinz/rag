...
class GraphAlgorithmRequest(BaseModel):
    ...
    algorithm_type: GraphAlgorithmType = Field(..., description="Type of algorithm to execute")
    parameters: Dict[str, Any] = Field(default_factory=dict, description="Algorithm-specific parameters")
    filters: Optional[Dict[str, Any]] = Field(None, description="Filters to apply to graph data")
...
class CentralityRequest(GraphAlgorithmRequest):
    ...
    algorithm_type: GraphAlgorithmType = Field(..., regex="^(degree_centrality|betweenness_centrality|closeness_centrality|eigenvector_centrality|pagerank)$")
    include_weights: bool = Field(default=True, description="Include edge weights in computation")
...
class CommunityDetectionRequest(GraphAlgorithmRequest):
    ...
    algorithm_type: GraphAlgorithmType = Field(default=GraphAlgorithmType.COMMUNITY_DETECTION)
    community_algorithm: CommunityDetectionAlgorithm = Field(default=CommunityDetectionAlgorithm.LOUVAIN)
    resolution: float = Field(default=1.0, ge=0.1, le=10.0, description="Community resolution parameter")
    min_community_size: int = Field(default=5, ge=1, description="Minimum community size")
    include_overlapping: bool = Field(default=False, description="Allow overlapping communities")
...
class PathfindingRequest(GraphAlgorithmRequest):
    ...
    find_all_paths: bool = Field(default=False, description="Find all paths, not just shortest")
    max_paths: int = Field(default=10, ge=1, description="Maximum number of paths to return")


class GraphVisualizationRequest(BaseModel):
    """Request model for graph visualization data"""
    entity_ids: Optional[List[str]] = Field(None, description="Specific entities to include")
    center_entity_id: Optional[str] = Field(None, description="Entity to center the view on")
    depth: int = Field(default=2, ge=1, le=5, description="Neighborhood depth for expansion")
    max_nodes: int = Field(default=100, ge=1, le=1000, description="Maximum number of nodes")
    layout_algorithm: LayoutAlgorithm = Field(default=LayoutAlgorithm.FORCE_DIRECTED)
    layout_options: Dict[str, Any] = Field(default_factory=dict, description="Layout algorithm options")
...
class GraphEntity(BaseModel):
    ...
    importance_score: Optional[float] = Field(None, description="Computed importance score")
    ...
    centrality_metrics: Optional[Dict[str, float]] = Field(None, description="Centrality metrics")
    community_info: Optional[Dict[str, Any]] = Field(None, description="Community information")
    graph_analytics: Optional[Dict[str, Any]] = Field(None, description="Additional graph analytics")
...
class GraphRelationship(BaseModel):
    ...
    direction: str = Field(default="bidirectional", description="Relationship direction")
    temporal_data: Optional[Dict[str, Any]] = Field(None, description="Time-based information")
    ...
    # Graph analytics
    graph_analytics: Optional[Dict[str, Any]] = Field(None, description="Graph analytics data")
...
class GraphPath(BaseModel):
    ...
    path_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Path ID")
...
class GraphVisualizationData(BaseModel):
    ...
    relationships: List[GraphRelationship] = Field(..., description="Relationships to visualize")
...
class GraphComputationJob(BaseModel):
    ...
    computation_time_ms: Optional[int] = Field(None, description="Computation time in ms")
...
class GraphAlgorithmResponse(BaseModel):
    ...
    estimated_time_seconds: Optional[int] = Field(None, description="Estimated computation time")
    results: Optional[Union[List[CentralityResult], List[CommunityResult], List[GraphPath]]] = Field(None, description="Computation results")
    computation_metadata: Dict[str, Any] = Field(default_factory=dict, description="Computation metadata")
...
class GraphAnalyticsResponse(BaseModel):
    ...
    entity_type_distribution: Dict[EntityType, int] = Field(..., description="Entity type distribution")
    relationship_type_distribution: Dict[RelationshipType, int] = Field(..., description="Relationship type distribution")
    ...
    connectivity_metrics: Dict[str, float] = Field(..., description="Connectivity metrics")
...
class GraphInsight(BaseModel):
    ...
    affected_entity_ids: List[str] = Field(default_factory=list, description="Affected entities")
    affected_relationship_ids: List[str] = Field(default_factory=list, description="Affected relationships")
...
class GraphInsightsResponse(BaseModel):
    ...
    severity_distribution: Dict[str, int] = Field(..., description="Insights by severity")
...
class UserGraphPreferences(BaseModel):
    ...
    default_layout_algorithm: LayoutAlgorithm = Field(default=LayoutAlgorithm.FORCE_DIRECTED)
    ...
    label_threshold: int = Field(default=50, description="Show labels for graphs with < N nodes")
    ...
    preferred_centrality_metric: GraphAlgorithmType = Field(default=GraphAlgorithmType.DEGREE_CENTRALITY)
...
class GraphPerformanceMetrics(BaseModel):
    ...
    relationship_count: int = Field(..., description="Number of relationships processed")

...
class UserAnalytics(BaseModel):
    ...
    avg_session_duration_ms: float = Field(..., ge=0.0, description="Average session duration")
    ...
    satisfaction_score: Optional[float] = Field(None, ge=1.0, le=5.0, description="Satisfaction score")
    top_queries: List[Dict[str, Any]] = Field(default_factory=list, description="Top queries")
    preferred_modalities: List[str] = Field(default_factory=list, description="Preferred modalities")
    expertise_areas: List[str] = Field(default_factory=list, description="Identified expertise areas")
...
class ContentAnalytics(BaseModel):
    ...
    content_id: Optional[UUID] = Field(None, description="Content ID (None for aggregate)")
    period_start: date = Field(..., description="Analytics period start")
    period_end: date = Field(..., description="Analytics period end")
    total_views: int = Field(..., ge=0, description="Total views")
    unique_viewers: int = Field(..., ge=0, description="Unique viewers")
    avg_view_duration_ms: float = Field(..., ge=0.0, description="Average view duration")
    click_through_rate: float = Field(..., ge=0.0, le=1.0, description="Click-through rate")
    ...
    search_queries: List[str] = Field(default_factory=list, description="Search queries that found this content")
...
class SystemAnalytics(BaseModel):
    ...
    response_time_p50_ms: float = Field(..., ge=0.0, description="50th percentile response time")
    response_time_p95_ms: float = Field(..., ge=0.0, description="95th percentile response time")
    response_time_p99_ms: float = Field(..., ge=0.0, description="99th percentile response time")
...
class PerformanceMetrics(BaseModel):
    ...
    throughput_rps: float = Field(..., ge=0.0, description="Throughput requests per second")
    error_rate: float = Field(..., ge=0.0, le=1.0, description="Error rate")
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="Timestamp")
...
class BusinessMetrics(BaseModel):
    ...
    user_retention_rate: float = Field(..., ge=0.0, le=1.0, description="User retention rate")
    total_queries: int = Field(..., ge=0, description="Total queries")
    ...
    customer_satisfaction_score: Optional[float] = Field(None, ge=1.0, le=5.0, description="Customer satisfaction")
...
class ForecastData(BaseModel):
    ...
    historical_period_start: datetime = Field(..., description="Historical period start")
    ...
    confidence_intervals: List[List[float]] = Field(..., description="Confidence intervals")
...
class AnalyticsDashboard(BaseModel):
    ...
    time_window: TimeWindow = Field(TimeWindow.LAST_24_HOURS, description="Default time window")

...
from sqlalchemy import Index, UniqueConstraint, CheckConstraint, ForeignKeyConstraint, text
...
DOCUMENT_INDEXES = [
    ...
    Index('idx_documents_processing_times', 'processing_started_at', 'processing_completed_at'),
...
ENTITY_INDEXES = [
    ...
    Index('idx_entities_external_ids', 'external_ids'),  # JSON index for external lookups
...
QUOTA_INDEXES = [
    ...
    Index('idx_quota_snapshots_daily', 'user_quota_id', 'snapshot_date', 'snapshot_type'),
]
...
UNIQUE_CONSTRAINTS = [
    ...
    # Documents
    UniqueConstraint('organization_id', 'file_hash_sha256', name='uq_documents_file_hash'),
    UniqueConstraint('organization_id', 'stored_filename', name='uq_documents_filename'),

    # Entities
    UniqueConstraint('organization_id', 'entity_uri', name='uq_entities_uri'),
    UniqueConstraint('organization_id', 'canonical_name', 'entity_type', name='uq_entities_canonical_name_type'),
    ...
    # Evaluation runs
    UniqueConstraint('organization_id', 'run_name', 'created_at', name='uq_eval_runs_org_name_date'),
]
...
CHECK_CONSTRAINTS = [
    ...
    CheckConstraint('file_size_bytes <= 5368709120', name='ck_file_size_max_5gb'),  # 5GB max

    # Score and percentage constraints
    CheckConstraint('confidence >= 0.0 AND confidence <= 1.0', name='ck_confidence_range'),
    CheckConstraint('quality_score >= 0.0 AND quality_score <= 1.0', name='ck_quality_score_range'),
    CheckConstraint('progress_percentage >= 0.0 AND progress_percentage <= 100.0', name='ck_progress_percentage_range'),

    # Rating constraints
    CheckConstraint('user_rating >= 1 AND user_rating <= 5', name='ck_user_rating_range'),
    CheckConstraint('rating >= 1 AND rating <= 5', name='ck_rating_range'),

    # Temporal constraints
    CheckConstraint('valid_from <= valid_to', name='ck_valid_date_range'),
    CheckConstraint('evaluation_period_start <= evaluation_period_end', name='ck_evaluation_period_range'),
    ...
    CheckConstraint('processing_priority >= 1 AND processing_priority <= 10', name='ck_processing_priority_range'),
    ...
    CheckConstraint('storage_used_bytes <= storage_quota_bytes', name='ck_storage_usage_limit'),
...
FOREIGN_KEY_CONSTRAINTS = [
    ...
    ForeignKeyConstraint(['parent_document_id'], ['enhanced_documents.id'], ondelete='SET NULL'),
    ForeignKeyConstraint(['document_id'], ['enhanced_documents.id'], ondelete='CASCADE'),

    # Entity relationship constraints
    ForeignKeyConstraint(['source_entity_id'], ['knowledge_entities.id'], ondelete='CASCADE'),
    ForeignKeyConstraint(['target_entity_id'], ['knowledge_entities.id'], ondelete='CASCADE'),
    ForeignKeyConstraint(['merged_into_id'], ['knowledge_entities.id'], ondelete='SET NULL'),

    # Processing constraints
    ForeignKeyConstraint(['document_id'], ['enhanced_documents.id'], ondelete='CASCADE'),
    ForeignKeyConstraint(['evaluation_run_id'], ['evaluation_runs.id'], ondelete='CASCADE'),

    # WebSocket constraints
    ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE'),
    ForeignKeyConstraint(['connection_id'], ['websocket_connections.id'], ondelete='CASCADE'),
]
    ...
    return {
        'query_patterns': {
            'user_document_access': {
                'frequency': 'high',
                'recommended_indexes': ['idx_documents_org_user', 'idx_documents_user_uploaded'],
                'optimization': 'Consider caching frequently accessed user documents'
            },
            'entity_search': {
                'frequency': 'high',
                'recommended_indexes': ['idx_entities_name_variations', 'idx_entities_confidence'],
                'optimization': 'Use trigram indexes for fuzzy name matching'
            },
            'rag_query_execution': {
                'frequency': 'high',
                'recommended_indexes': ['idx_queries_user_session', 'idx_queries_cache_key'],
                'optimization': 'Implement query result caching for common patterns'
            },
            'full_text_search': {
                'frequency': 'medium',
                'recommended_indexes': ['idx_documents_search_vector', 'idx_entities_description'],

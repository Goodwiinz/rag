...
class SearchQuerySanitizer:
    ...
    @classmethod
    def sanitize(
        cls, query: str, strict: bool = True, allow_wildcards: bool = False
    ) -> str:
        """
        ...
        if len(query) > cls.MAX_QUERY_LENGTH:
            ...
            logger.warning(
                f"Query truncated from {len(original_query)} to {cls.MAX_QUERY_LENGTH}"
            )
        ...
        if security_violations:
            if strict:
                ...
                logger.warning(
                    f"Removed {len(security_violations)} security violations from query"
                )

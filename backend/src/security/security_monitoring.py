        ...
        for ip, ip_event_list in ip_events.items():
            if len(ip_event_list) >= self.failed_attempts_threshold:
                ...
                recent_events = [
                    ...
                    if e.timestamp > datetime.utcnow() - timedelta(minutes=self.time_window_minutes)
                ]
    ...
    def evaluate(self, events: List[SecurityEvent]) -> List[SecurityAlert]:
        ...
        for user_id, user_event_list in user_events.items():
            ...
            if len(api_events) > self.unusual_patterns["rapid_api_calls"]:
                time_span = max(e.timestamp for e in api_events) - min(e.timestamp for e in api_events)
...
class DataExfiltrationRule(ThreatDetectionRule):
    ...
    def evaluate(self, events: List[SecurityEvent]) -> List[SecurityAlert]:
        ...
        user_events = defaultdict(list)
        for event in events:
            if event.user_id and event.type in ["file_download", "data_export", "bulk_api_call"]:
                user_events[event.user_id].append(event)
...
class SecurityAlertManager:
        ...
        if self.db_connection:
            try:
                with self.db_connection.cursor() as cursor:
                    ...
                    """, (
                        event.id, event.type, event.severity.value,
                        event.timestamp, event.source_ip, event.user_id,
                        json.dumps(event.details), json.dumps(event.threat_indicators)
                    ))
                ...
                if event_data:
                    ...
                    if timestamp > cutoff_time:
                        event = SecurityEvent(
                            ...
                            threat_indicators=json.loads(event_data['threat_indicators'])
                        )
                        events.append(event)
            except Exception as e:
    ...
    async def _execute_response_actions(self, alert: SecurityAlert) -> None:
        ...
        if alert.severity in [AlertSeverity.HIGH, AlertSeverity.CRITICAL]:
            ...
            for ip in source_ips:
                await self._block_ip(ip, f"Auto-block due to security alert: {alert.id}")

            # Lock user accounts if suspicious
            user_ids = list(set(event.user_id for event in alert.events if event.user_id))
            for user_id in user_ids:
                await self._lock_user_account(user_id, f"Auto-lock due to security alert: {alert.id}")
    ...
    def get_security_metrics(self) -> Dict[str, Any]:
        ...
        # Get threat level
        critical_alerts = sum(1 for alert in self.get_active_alerts()
...
class PagerDutyNotifier(NotificationChannel):
    ...
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.integration_key = config["notification_channels"]["pagerduty"]["integration_key"]
...
class SecurityMonitoringService:
    ...
    async def log_security_event(self, event_type: str, source_ip: str,
                               user_id: Optional[str] = None,
                               details: Dict[str, Any] = None,
                               threat_indicators: List[str] = None) -> str:

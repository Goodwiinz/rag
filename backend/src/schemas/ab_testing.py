...
class ExperimentCreateRequest(BaseModel):
    ...
    description: Optional[str] = Field(
        None, max_length=1000, description="Experiment description"
    )
    hypothesis: str = Field(
        ..., min_length=10, max_length=1000, description="Test hypothesis"
    )

    experiment_type: ExperimentType = Field(..., description="Type of experiment")
    traffic_split_type: TrafficSplitType = Field(
        TrafficSplitType.UNIFORM, description="Traffic split strategy"
    )
    traffic_percentage: float = Field(
        100.0, ge=1.0, le=100.0, description="Percentage of total traffic"
    )
    ...
    start_time: Optional[datetime] = Field(
        None, description="Start time for the experiment"
    )
    end_time: Optional[datetime] = Field(
        None, description="End time for the experiment"
    )
    ...
    statistical_test: StatisticalTest = Field(
        StatisticalTest.Z_TEST, description="Statistical test"
    )
    expected_effect_size: Optional[float] = Field(
        None, gt=0.0, description="Expected minimum effect size"
    )

    target_user_segments: Optional[List[Dict[str, Any]]] = Field(
        None, description="Target user segments"
    )
    target_query_patterns: Optional[List[str]] = Field(
        None, description="Target query patterns"
    )
    target_organization_ids: Optional[List[UUID]] = Field(
        None, description="Target specific organizations"
    )

    primary_metric: MetricType = Field(..., description="Primary success metric")
    success_criteria: SuccessCriterion = Field(
        SuccessCriterion.HIGHER_IS_BETTER, description="Success criteria"
    )
    target_improvement: Optional[float] = Field(
        None, gt=0.0, description="Expected improvement percentage"
    )
    minimum_duration_days: int = Field(
        7, ge=1, le=365, description="Minimum duration in days"
    )

    tags: Optional[List[str]] = Field(None, description="Experiment tags")
    external_references: Optional[Dict[str, Any]] = Field(
        None, description="External resource references"
    )
    ...
    class Config:
        json_json_schema_extra = {
            "example": {
                ...
                "metric_metadata": {"result_count": 10, "response_time_ms": 150},
            }
        }
...
class StatisticalAnalysisRequest(BaseModel):
    ...
    statistical_test: Optional[StatisticalTest] = Field(
        None, description="Statistical test to use"
    )
    segment_analysis: Optional[bool] = Field(
        False, description="Include segment-based analysis"
    )
    time_series_analysis: Optional[bool] = Field(
        False, description="Include time series analysis"
    )
    custom_parameters: Optional[Dict[str, Any]] = Field(
        None, description="Custom analysis parameters"
    )
...
class ExperimentSummaryRequest(BaseModel):
    ...
    segments: Optional[List[str]] = Field(
        None, description="Specific segments to analyze"
    )
...
class UserSegmentCreateRequest(BaseModel):
    ...
    segment_type: str = Field(
        ..., description="Type of segment (behavioral, demographic, technical)"
    )
    segment_criteria: Dict[str, Any] = Field(

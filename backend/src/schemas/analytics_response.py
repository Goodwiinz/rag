...
class ResponseMetadata(BaseModel):
    ...
    actual_start_date: Optional[datetime] = Field(
        None, description="Actual start date of data"
    )
    actual_end_date: Optional[datetime] = Field(
        None, description="Actual end date of data"
    )
    ...
    freshness_minutes: Optional[int] = Field(
        None, description="Data freshness in minutes"
    )

    # Query information
    query_complexity: Optional[str] = Field(None, description="Query complexity level")
    indexes_used: Optional[List[str]] = Field(
        None, description="Database indexes utilized"
    )
...
class TimeSeriesPoint(BaseModel):
    ...
    count: Optional[int] = Field(
        None, description="Number of items aggregated for this point"
    )
    metadata: Optional[Dict[str, Any]] = Field(
        None, description="Additional metadata for this point"
    )
...
class CategoryDataPoint(BaseModel):
    ...
    percentage: Optional[float] = Field(
        None, ge=0, le=100, description="Percentage of total"
    )
...
class CategoricalData(BaseModel):
    ...
    total_value: Optional[float] = Field(
        None, description="Total value across all categories"
    )
...
class DistributionBin(BaseModel):
    ...
    range_start: Union[float, int, Decimal] = Field(
        ..., description="Start of bin range"
    )
    ...
    percentage: Optional[float] = Field(
        None, ge=0, le=100, description="Percentage of items in this bin"
    )
...
class TableData(BaseModel):
    ...
    sortable_columns: Optional[List[str]] = Field(
        None, description="Columns that can be sorted"
    )
...
class MetricSummary(BaseModel):
    ...
    previous_value: Optional[Union[float, int, Decimal]] = Field(
    ...
    trend_direction: Optional[str] = Field(
        None, pattern="^(up|down|stable)$", description="Trend direction"
    )

    # Statistical values
    min_value: Optional[Union[float, int, Decimal]] = Field(
        None, description="Minimum value"
    )
    max_value: Optional[Union[float, int, Decimal]] = Field(
    ...
    confidence_level: Optional[float] = Field(
        None, ge=0, le=1, description="Statistical confidence"
    )
    is_significant: Optional[bool] = Field(
        None, description="Whether change is statistically significant"
    )
...
class PerformanceMetricsResponse(BaseAnalyticsResponse):
    ...
    data: Union[TimeSeriesData, DistributionData, List[MetricSummary]] = Field(
    ...
    system_health_score: Optional[float] = Field(
        None, ge=0, le=100, description="Overall system health score"
    )
...
class UserBehaviorResponse(BaseAnalyticsResponse):
    ...
    data: Union[TimeSeriesData, CategoricalData, TableData] = Field(
    ...
    engagement_score: Optional[float] = Field(
        None, ge=0, le=100, description="Overall engagement score"
    )
...
class QualityMetricsResponse(BaseAnalyticsResponse):
    ...
    overall_quality_score: Optional[float] = Field(
        None, ge=0, le=100, description="Overall quality score"
    )
    queries_analyzed: Optional[int] = Field(
        None, description="Number of queries analyzed"
    )
    below_threshold_count: Optional[int] = Field(
        None, description="Count of metrics below threshold"
    )
...
class AnalyticsEventsResponse(BaseAnalyticsResponse):
    ...
    data: Union[TableData, CategoricalData, TimeSeriesData] = Field(
    ...
    unique_sessions: Optional[int] = Field(
        None, description="Number of unique sessions"
    )
...
class ComparativeAnalyticsResponse(BaseAnalyticsResponse):
    ...
    statistical_significance: Optional[float] = Field(
        None, ge=0, le=1, description="Statistical significance level"
    )
    confidence_interval: Optional[str] = Field(
        None, description="Confidence interval information"
    )
...
class ErrorResponse(BaseAnalyticsResponse):
    ...
    stack_trace: Optional[str] = Field(
        None, description="Technical stack trace (development only)"
    )
...
class PerformanceInfo(BaseModel):
    ...
    serialization_time_ms: Optional[int] = Field(
        None, description="Response serialization time in milliseconds"
    )
    total_time_ms: int = Field(
        ..., description="Total server processing time in milliseconds"
    )
    memory_usage_mb: Optional[float] = Field(
        None, description="Memory usage during request"
    )
    database_rows_examined: Optional[int] = Field(
        None, description="Number of database rows examined"
    )
...
class AnalyticsApiResponse(BaseModel):
    ...
    performance_info: PerformanceInfo = Field(

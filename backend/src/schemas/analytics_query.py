...
class BaseAnalyticsQuery(BaseModel):
    ...
    time_range: Optional[TimeRange] = Field(
        TimeRange.LAST_7_DAYS, description="Predefined time range"
    )
    start_date: Optional[datetime] = Field(
        None, description="Custom start date (required when time_range=CUSTOM)"
    )
    end_date: Optional[datetime] = Field(
        None, description="Custom end date (required when time_range=CUSTOM)"
    )
...
class PerformanceMetricsQuery(BaseAnalyticsQuery):
    ...
    component: Optional[str] = Field(
        None, min_length=1, max_length=100, description="Filter by component name"
    )
    environment: Optional[str] = Field(
        None, min_length=1, max_length=50, description="Filter by environment"
    )
    ...
    aggregation: Optional[AggregationType] = Field(
        AggregationType.AVERAGE, description="Aggregation type for metrics"
    )
    ...
    # Threshold filtering
    show_alerts_only: bool = Field(
        False, description="Show only metrics that triggered alerts"
    )
...
class UserBehaviorQuery(BaseAnalyticsQuery):
    ...
    session_id: Optional[str] = Field(
        None, min_length=10, max_length=255, description="Filter by session ID"
    )

    # User filtering
    include_anonymous: bool = Field(
        False, description="Include anonymous/anonymized users"
    )
    user_segments: Optional[List[str]] = Field(
        None, description="Filter by user segments"
    )

    # Behavioral metrics
    min_engagement_score: Optional[float] = Field(
        None, ge=0, le=100, description="Minimum engagement score filter"
    )
    min_session_duration: Optional[int] = Field(
        None, ge=0, description="Minimum session duration in seconds"
    )
    bounced_sessions_only: bool = Field(False, description="Show only bounced sessions")

    # Search behavior
    search_query_contains: Optional[str] = Field(
        None, min_length=1, max_length=100, description="Filter by search query content"
    )
    has_downloads: bool = Field(
        None, description="Filter sessions with/without downloads"
    )
...
class QualityMetricsQuery(BaseAnalyticsQuery):
    ...
    metric_name: Optional[str] = Field(
        ...
        min_length=1,
        max_length=100,
        description="Filter by specific quality metric",
    )
    query_type: Optional[str] = Field(
        None, pattern="^(semantic|hybrid|keyword)$", description="Filter by query type"
    )
    ...
    max_score: Optional[float] = Field(
        None, ge=0, le=1, description="Maximum quality score filter"
    )
    below_threshold: bool = Field(
        False, description="Show only metrics below threshold"
    )

    # Comparison and trends
    compare_with_previous: bool = Field(
        False, description="Compare with previous period"
    )
    trend_direction: Optional[str] = Field(
        None, pattern="^(up|down|stable)$", description="Filter by trend direction"
    )
...
class AnalyticsEventsQuery(BaseAnalyticsQuery):
    ...
    component: Optional[str] = Field(
        None, min_length=1, max_length=100, description="Filter by component"
    )
    error_code: Optional[int] = Field(
        None, ge=100, le=599, description="Filter by HTTP error code"
    )

    # Text-based filtering
    description_contains: Optional[str] = Field(
        None, min_length=1, max_length=200, description="Filter by description content"
    )

    # Aggregation
    group_by: Optional[List[str]] = Field(
        None, description="Fields to group by for aggregation"
    )
        ...
        if v:
            ...
            for field in v:
                if field not in allowed_fields:
                    raise ValueError(
                        f"Invalid group_by field: {field}. Allowed: {allowed_fields}"
                    )
        return v
...
class ComparativeAnalyticsQuery(BaseAnalyticsQuery):
    ...
    confidence_level: float = Field(
        0.95, ge=0.8, le=0.99, description="Statistical confidence level"
    )
    show_significant_only: bool = Field(
        False, description="Show only statistically significant changes"
    )
    ...
    def validate_comparison_periods(self):
        ...
        for period_type in ["current", "comparison"]:
            ...
            if start_date and end_date:
                if start_date >= end_date:
                    raise ValueError(
                        f"{period_type.title()} period start must be before end"
                    )
                ...
                if end_date - start_date > max_range:
                    raise ValueError(
                        f"{period_type.title()} period cannot exceed 90 days"
                    )
...
class AnalyticsResponse(BaseModel):
    ...
    query_params: Optional[Dict[str, Any]] = Field(
        None, description="Processed query parameters"
    )

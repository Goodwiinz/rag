...
class DatabaseOptimizationManager:
    ...
    async def apply_all_optimizations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.postgresql_optimizer:
                try:
                    pg_result = (
                        await self.postgresql_optimizer.apply_production_optimizations()
                    )
                    optimization_results["results"]["postgresql"] = pg_result
                except Exception as e:
                    optimization_results["results"]["postgresql"] = {
                        "success": False,
                        "error": str(e),
                    }
            ...
            if self.neo4j_optimizer:
                try:
                    neo4j_result = (
                        await self.neo4j_optimizer.apply_production_optimizations()
                    )
                    optimization_results["results"]["neo4j"] = neo4j_result
                except Exception as e:
                    optimization_results["results"]["neo4j"] = {
                        "success": False,
                        "error": str(e),
                    }
            ...
            if self.redis_optimizer:
                try:
                    redis_result = (
                        await self.redis_optimizer.apply_production_optimizations()
                    )
                    optimization_results["results"]["redis"] = redis_result
                except Exception as e:
                    optimization_results["results"]["redis"] = {
                        "success": False,
                        "error": str(e),
                    }
            ...
            if self.pool_manager:
                try:
                    ...
                    optimization_results["results"]["connection_pools"] = {
                        "success": False,
                        "error": str(e),
                    }
            ...
            if optimization_results["success"]:
                ...
                logger.warning(
                    f"Some optimizations failed: {optimization_results['errors']}"
                )
    ...
    async def start_monitoring(self, monitoring_interval_seconds: int = 60) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Database monitoring started with {monitoring_interval_seconds}s interval"
            )
            return True
    ...
    async def get_comprehensive_metrics(self) -> Dict[str, Any]:
        ...
        if self.postgresql_optimizer:
            try:
                metrics["databases"][
                    "postgresql"
                ] = await self.postgresql_optimizer.get_optimization_report()
            except Exception as e:
        ...
        if self.neo4j_optimizer:
            try:
                metrics["databases"][
                    "neo4j"
                ] = await self.neo4j_optimizer.get_graph_statistics()
            except Exception as e:
        ...
        if self.redis_optimizer:
            try:
                metrics["databases"][
                    "redis"
                ] = await self.redis_optimizer.get_redis_metrics()
            except Exception as e:
        ...
        if self.cross_db_integration:
            try:
                metrics[
                    "cross_db_metrics"
                ] = await self.cross_db_integration.get_comprehensive_metrics()
            except Exception as e:

...
async def get_api_key_data(
    ...
    try:
        ...
        # Hash the raw key to look up directly in the database
        # This prevents the memory exhaustion/DoS vulnerability caused by loading all keys with the same prefix
        key_hash = hashlib.sha256(raw_key.encode()).hexdigest()

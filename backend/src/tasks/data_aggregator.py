...
from src.models.analytics_event import AnalyticsEvent, EventSeverity, EventType
...
class AnalyticsDataAggregator:
    ...
    async def aggregate_events(
        self, config: AggregationConfig, event_types: Optional[List[EventType]] = None
    ) -> Dict[str, Any]:
        ...
        if config.organization_id:
            query = query.filter(
                AnalyticsEvent.organization_id == config.organization_id
            )
    ...
    async def aggregate_sessions(self, config: AggregationConfig) -> Dict[str, Any]:
        ...
        if config.metric_type == MetricType.COUNT:
            ...
            raise ValueError(
                f"Unsupported metric type for sessions: {config.metric_type}"
            )
    ...
    async def aggregate_performance(
        ...
        if config.organization_id:
            query = query.filter(
                PerformanceLog.organization_id == config.organization_id
            )
        ...
        if config.metric_type == MetricType.AVERAGE:
            ...
            raise ValueError(
                f"Unsupported metric type for performance: {config.metric_type}"
            )

        return result

    async def _aggregate_count(
        self, query, config: AggregationConfig
    ) -> Dict[str, Any]:
        ...
        if config.group_by:
            ...
            results = (
                base_query.with_entities(
                    *group_fields, func.count(AnalyticsEvent.id).label("count")
                )
    ...
    async def _aggregate_sum(self, query, config: AggregationConfig) -> Dict[str, Any]:
        ...
        if config.group_by:
            group_fields = [getattr(AnalyticsEvent, field) for field in config.group_by]
            results = (
                base_query.with_entities(*group_fields, func.sum(field).label("sum"))
                .group_by(*group_fields)
                .all()
            )
    ...
    async def _aggregate_average(
        ...
        if config.group_by:
            ...
            results = (
                base_query.with_entities(
                    *group_fields, func.avg(field).label("average")
                )
                .group_by(*group_fields)
                .all()
            )
            ...
            if hasattr(AnalyticsEvent, field):
                if isinstance(value, list):
                    success_query = success_query.filter(
                        getattr(AnalyticsEvent, field).in_(value)
                    )
                else:
                    success_query = success_query.filter(
                        getattr(AnalyticsEvent, field) == value
                    )
    ...
    async def _aggregate_session_count(
        self, query, config: AggregationConfig
    ) -> Dict[str, Any]:
        ...
        if config.group_by:
            ...
            results = (
                query.with_entities(
                    *group_fields, func.count(UserSession.id).label("count")
                )
                .group_by(*group_fields)
                .all()
            )
    ...
    async def _aggregate_session_average(
        self, query, config: AggregationConfig
    ) -> Dict[str, Any]:
    ...
    async def _aggregate_session_sum(
        self, query, config: AggregationConfig
    ) -> Dict[str, Any]:
    ...
    async def _aggregate_performance_average(
        self, query, config: AggregationConfig
    ) -> Dict[str, Any]:
    ...
    async def _aggregate_performance_count(
        self, query, config: AggregationConfig
    ) -> Dict[str, Any]:
        ...
        if config.group_by:
            ...
            results = (
                query.with_entities(
                    *group_fields, func.count(PerformanceLog.id).label("count")
                )
                .group_by(*group_fields)
                .all()
            )
    ...
    async def _aggregate_performance_rate(
        self, query, config: AggregationConfig
    ) -> Dict[str, Any]:
    ...
    async def aggregate_time_series(
        self, config: AggregationConfig, data_source: str = "events"
    ) -> Dict[str, Any]:

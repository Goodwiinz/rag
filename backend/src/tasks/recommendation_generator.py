...
class AnalyticsRecommendationGenerator:
    ...
    async def generate_recommendations(
        ...
        if not recommendation_types or RecommendationType.PERFORMANCE_OPTIMIZATION in recommendation_types:
            perf_recs = await self._generate_performance_recommendations(organization_id, time_range)
            recommendations.extend(perf_recs)

        if not recommendation_types or RecommendationType.USER_ENGAGEMENT in recommendation_types:
            engagement_recs = await self._generate_engagement_recommendations(organization_id, time_range)
            recommendations.extend(engagement_recs)

        if not recommendation_types or RecommendationType.SEARCH_QUALITY in recommendation_types:
            search_recs = await self._generate_search_quality_recommendations(organization_id, time_range)
            recommendations.extend(search_recs)

        if not recommendation_types or RecommendationType.SYSTEM_HEALTH in recommendation_types:
            health_recs = await self._generate_system_health_recommendations(organization_id, time_range)
    ...
    async def _generate_performance_recommendations(
        ...
        if avg_response_time > 2000:  # > 2 seconds
            ...
            priority = RecommendationPriority.CRITICAL if avg_response_time > 5000 else RecommendationPriority.HIGH
        ...
        if total_logs > 0:
            ...
            if error_rate > 5:  # > 5% error rate
                ...
                priority = RecommendationPriority.CRITICAL if error_rate > 15 else RecommendationPriority.HIGH
        ...
        # Check memory usage
        avg_memory_usage = self.db.query(func.avg(PerformanceLog.memory_usage_percent)).filter(
            and_(
                PerformanceLog.organization_id == organization_id,
                PerformanceLog.timestamp >= start_date,
                PerformanceLog.timestamp < end_date,
        ...
        if avg_memory_usage > 80:  # > 80% memory usage
            ...
            priority = RecommendationPriority.HIGH if avg_memory_usage > 90 else RecommendationPriority.MEDIUM
    ...
    async def _generate_engagement_recommendations(
        ...
        if avg_duration < 120:  # Less than 2 minutes
            ...
            priority = RecommendationPriority.HIGH if avg_duration < 60 else RecommendationPriority.MEDIUM
        ...
        short_sessions = self.db.query(UserSession).filter(
            and_(
                UserSession.organization_id == organization_id,
                UserSession.started_at >= start_date,
                UserSession.started_at < end_date,
        ...
        if total_sessions > 0:
            ...
            if bounce_rate > 40:  # > 40% bounce rate
                ...
                priority = RecommendationPriority.CRITICAL if bounce_rate > 60 else RecommendationPriority.HIGH
        ...
        if search_events:
            ...
            if total_searches > 0:
                ...
                if zero_result_rate > 15:  # > 15% zero results
                    ...
                    priority = RecommendationPriority.HIGH if zero_result_rate > 25 else RecommendationPriority.MEDIUM
    ...
    async def _generate_system_health_recommendations(
        ...
        recent_alerts = self.db.query(PerformanceLog).filter(
            and_(
                PerformanceLog.organization_id == organization_id,
                PerformanceLog.timestamp >= start_date,
                PerformanceLog.timestamp < end_date,
        ...
        if recent_alerts > 10:  # More than 10 alerts in the period
            ...
            priority = RecommendationPriority.CRITICAL if recent_alerts > 50 else RecommendationPriority.HIGH
    ...
    def format_recommendations(self, recommendations: List[Recommendation]) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "avg_impact_score": sum(r.impact_score for r in recommendations) / len(recommendations) if recommendations else 0
            },

...
from src.core.database import get_database_info, check_database_health
from src.migrations.data_migration_utils import run_migration, validate_migration
from src.migrations.database_optimization import DatabaseOptimizer, analyze_database_performance
...
class MigrationRunner:
    ...
    def __init__(self, database_url: Optional[str] = None):
        ...
        """
        self.database_url = database_url or "postgresql://postgres:postgres@localhost:5432/multimodal_rag"
        self.engine = create_engine(self.database_url)
        self.SessionLocal = sessionmaker(bind=self.engine)

    def run_complete_migration(self, batch_size: int = 100, optimize: bool = True, benchmark: bool = True) -> Dict[str, Any]:
        """
        ...
        """
        logger.info("Starting complete migration process for enhanced document processing")

        migration_results = {
            'migration_info': {
                'start_time': datetime.utcnow().isoformat(),
                'database_url': self.database_url.replace(self.database_url.split('@')[0].split('//')[1].split(':')[0], '***'),
        ...
        try:
            ...
            if not migration_results['database_migration']['migration_applied']:
                logger.error("Database schema migration has not been applied. Please run: alembic upgrade head")
            ...
            if benchmark:
                ...
                migration_results['performance_benchmark'] = self._run_performance_benchmark()
            ...
            migration_results['summary'] = self._generate_migration_summary(migration_results)
            migration_results['migration_info']['end_time'] = datetime.utcnow().isoformat()
    ...
    def _run_consistency_checks(self, db) -> Dict[str, Any]:
        ...
        try:
            ...
            checks['processed_documents_without_history'] = processed_documents_without_history
            ...
            checks['consistency_score'] = 'Excellent' if total_issues == 0 else 'Good' if total_issues < 10 else 'Poor'
    ...
    def _generate_migration_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for step, result in results.items():
            ...
            if isinstance(result, dict) and result.get('success') is False:
                ...
                summary['warnings'].append(f"{step} failed: {result.get('error', 'Unknown error')}")
        ...
        if 'data_migration' in results and results['data_migration'].get('statistics'):
            ...
            summary['key_metrics'] = {
                ...
                'processing_history_created': stats.get('processing_history_created', 0),
                'multimodal_content_created': stats.get('multimodal_content_created', 0),
        ...
        if summary['key_metrics'].get('migration_errors', 0) > 0:
            summary['recommendations'].append("Review migration errors and consider re-running migration for failed documents")

        if 'validation' in results:
            consistency = results['validation'].get('consistency_checks', {})
            if consistency.get('total_issues', 0) > 0:
                summary['recommendations'].append("Address data consistency issues found during validation")
        ...
        if 'performance_benchmark' in results and results['performance_benchmark'].get('success'):
            ...
            avg_time = benchmark.get('aggregate_summary', {}).get('avg_execution_time_ms', 0)
            if avg_time > 1000:
                summary['recommendations'].append("Consider additional database optimization as query times are above optimal")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run enhanced document processing migration')
    parser.add_argument('--batch-size', type=int, default=100, help='Batch size for data migration')
    ...
    parser.add_argument('--output', type=str, help='Output file for migration results (JSON)')

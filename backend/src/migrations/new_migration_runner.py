...
class MigrationRunner:
    ...
    def discover_migrations(self) -> List[Dict]:
        ...
        for file_path in sorted(self.migrations_dir.glob("*.sql")):
            ...
            name = "_".join(file_path.stem.split("_", 1)[1:]) if "_" in file_path.stem else file_path.stem
            ...
            async with self.connection_pool.acquire() as conn:
                async with conn.transaction():
                    ...
                    await conn.execute("""
                        ...
                        VALUES ($1, $2, $3, $4)
                    """, migration['version'], migration['name'], execution_time, migration['checksum'])

            console.print(f"✅ Migration {migration['version']} executed successfully ({execution_time}ms)", style="green")
            return True

        except Exception as e:
            console.print(f"❌ Migration {migration['version']} failed: {e}", style="red")
    ...
    async def rollback_migration(self, version: str) -> bool:
        ...
        if not rollback_file.exists():
            console.print(f"❌ No rollback file found for migration {version}", style="red")
            return False
    ...
    async def show_status(self):
        ...
        console.print(f"\n📈 Summary: {applied_count} applied, {pending_count} pending", style="bold")
    ...
    async def migrate(self, target_version: Optional[str] = None):
                ...
                if target_version is None or migration['version'] <= target_version:
                    ...
                    if migration['version'] in executed_migrations:
                        ...
                        if exec_info['checksum'] != migration['checksum']:
                            console.print(f"⚠️  Migration {migration['version']} has changed - manual intervention required", style="yellow")
                            continue
        ...
        with Progress(
            ...
            task = progress.add_task("Running migrations...", total=len(migrations_to_run))
            ...
            for migration in migrations_to_run:
                ...
                if not success:
                    console.print(f"❌ Migration stopped at {migration['version']}", style="red")
                    return
    ...
    async def rollback(self, version: str):
        ...
        versions_to_rollback = [
            v for v in sorted(executed_migrations.keys(), reverse=True)
            if v > version
        ]
    ...
    async def validate_schema(self):
        ...
        async with self.connection_pool.acquire() as conn:
            ...
            expected_tables = {
                'organizations', 'users', 'user_sessions', 'documents',
                'document_processing_jobs', 'search_queries', 'search_results',
                'entities', 'entity_relationships', 'concepts',
                'rag_evaluations', 'performance_metrics', 'system_health',
                'error_logs', 'schema_migrations'
            }
            ...
            if missing_tables:
                console.print(f"❌ Missing tables: {', '.join(missing_tables)}", style="red")
                return False

            if extra_tables:
                console.print(f"⚠️  Extra tables: {', '.join(extra_tables)}", style="yellow")

            # Check if RLS is enabled on sensitive tables
            rls_required_tables = {
                'organizations', 'users', 'documents', 'search_queries',
                'entities', 'rag_evaluations', 'error_logs'
            }
            ...
            for table_info in rls_status:
                if not table_info['rowsecurity']:
                    console.print(f"⚠️  RLS not enabled on table: {table_info['tablename']}", style="yellow")
...
async def main():
    ...
    if not args.database_url:
        console.print("❌ DATABASE_URL environment variable or --database-url argument required", style="red")
        return

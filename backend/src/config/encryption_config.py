...
from ..core.encryption import EncryptionKeyType, EncryptionAlgorithm
...
class EncryptionConfig(BaseSettings):
    ...
    auto_encrypt_request_fields: bool = Field(
        default=True,
        description="Automatically encrypt sensitive fields in requests"
    )
    ...
    max_key_versions: int = Field(
        ...
        ge=3,
        le=50,
        description="Maximum number of key versions to retain"
    )
    ...
    key_backup_retention_days: int = Field(
        ...
        ge=365,
        description="Days to retain key backups"
    )

    # Audit settings
    audit_encryption_operations: bool = Field(
        default=True,
        description="Audit all encryption operations"
    )
    ...
    encryption_cache_enabled: bool = Field(
        default=True,
        description="Enable encryption operation caching"
    )
    ...
    require_key_rotation_alerts: bool = Field(
        default=True,
        description="Alert when keys need rotation"
    )
    ...
    key_rotation_warning_days: int = Field(
        ...
        ge=1,
        le=30,
        description="Days before key rotation to send warning"
    )
    ...
    integrate_with_rbac: bool = Field(
        default=True,
        description="Integrate encryption with RBAC system"
    )

    integrate_with_audit: bool = Field(
        default=True,
        description="Integrate encryption with audit system"
    )
    ...
    def validate_encryption_algorithm(cls, v):
        ...
        if v not in valid_algorithms:
            raise ValueError(f"Invalid encryption algorithm. Must be one of: {valid_algorithms}")
        return v

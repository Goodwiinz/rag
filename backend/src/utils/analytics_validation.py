...
class SecurityValidator:
    ...
    def validate_list_input(
        ...
        Args:
            ...
            item_type: Optional required type for list entries
        ...
        # Backward-compatible signature: validate_list_input(value, max_items, field_name)
        ...
        if item_type is not None:
            for idx, item in enumerate(value):
                if not isinstance(item, item_type):
                    raise AnalyticsValidationError(
                        f"Item at index {idx} in {field_name} must be of type {item_type.__name__}"
                    )
    ...
    def validate_pagination(limit: int, offset: int) -> tuple[int, int]:
        """Backward-compatible pagination validator on SecurityValidator."""
        return QueryParameterValidator.validate_pagination(limit, offset)

...
class FileUploadSecurityService:
    ...
    def __init__(self):
        ...
        self.blocked_extensions = {
            ...
            '.app', '.deb', '.pkg', '.dmg', '.msi', '.msp', '.msm', '.sh', '.ps1',
            '.vb', '.vbe', '.jse', '.ws', '.wsf', '.wsc', '.wsh', '.ps1xml', '.ps2',
            '.ps2xml', '.psc1', '.psc2', '.msh', '.msh1', '.msh2', '.mshxml', '.msh1xml',
            '.msh2xml', '.scf', '.lnk', '.inf', '.reg', '.docm', '.dotm', '.xlsm',
            '.xltm', '.xlam', '.pptm', '.potm', '.ppam', '.ppsm', '.sldm', '.ade', '.adp',
            '.cpl', '.mdb', '.accdb', '.accdc', '.accde', '.accdr', '.accdt', '.accda',
            '.mda', '.mdw', '.mdt', '.mdz', '.xll', '.xlw', '.wks', '.wiz', '.wps',
            '.xtp', '.xnk', '.ins', '.isp', '.onetoc2', '.tmp', '.url', '.vsmacros',
            '.vsw', '.vsd', '.vsdx', '.vss', '.vst', '.vdx', '.vsx', '.vtx', '.vsdm',
            '.vssm', '.vstm', '.xdw', '.xps'
        }
    ...
    async def validate_file_security(
        ...
        try:
            ...
            if self._is_archive_file(mime_result.get('detected_mime', '')):
                archive_result = await self._detect_archive_bomb(file_content, file.filename)
                validation_results.update(archive_result)

            # 6. Image-specific validation
            if mime_result.get('detected_mime', '').startswith('image/'):
                image_result = await self._validate_image_security(file_content, file.filename)
    ...
    async def _read_file_content(self, file: UploadFile, max_size: int = 100 * 1024 * 1024) -> bytes:
    ...
    def _validate_file_size(self, content: bytes, organization: Organization) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if scan_result is None:
                    ...
                    # Threat detected
                    result['errors'].append(
                        f"Security threat detected: {scan_result}"
                    )
    ...
    async def _detect_archive_bomb(self, content: bytes, filename: str) -> Dict[str, Any]:
                    ...
                    try:
                        with zipfile.ZipFile(temp_file_path, 'r') as zip_file:
                            ...
                            for file_info in zip_file.infolist():
                                ...
                                if file_ext in ['.zip', '.rar', '.tar', '.gz', '.bz2', '.7z']:
                                    result['warnings'].append(
    ...
    async def _validate_image_security(self, content: bytes, filename: str) -> Dict[str, Any]:
                ...
                if matches:
                    ...
                    logger.warning(
                        ...
                        extra={
                            ...
                            'severity': 'high' if pattern_name in ['ssn', 'credit_card', 'private_key'] else 'medium'
                        }
                    )
    ...
    async def _extract_and_sanitize_metadata(self, content: bytes, mime_type: str) -> Dict[str, Any]:

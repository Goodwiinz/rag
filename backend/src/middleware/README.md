...
| `analytics_auth.py`        | `require_analytics_permission`, `AnalyticsAccessValidator`                | FastAPI `Depends`-style guards for analytics endpoints. Enforces minimum role (ANALYST+), active-org check, optional session token validation, and audit logging. Pre-built instances: `require_analytics_access`, `require_admin_analytics_access`.                                                        |

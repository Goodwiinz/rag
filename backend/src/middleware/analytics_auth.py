...
def get_analytics_user_context(
    request: Request,
    access_context: dict = Depends(validate_analytics_access)
) -> dict:
    """
...
class AnalyticsAccessValidator:
    ...
    def __call__(self,
        ...
        if not self.allow_cross_org:
            target_org_id = request.path_params.get("organization_id") or \
...
require_content_manager_plus_access = AnalyticsAccessValidator(min_role=UserRole.CONTENT_MANAGER)